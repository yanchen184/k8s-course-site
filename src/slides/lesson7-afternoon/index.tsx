...
// ============================================================
// 第七堂下午 Loop 4 — 從零建完整系統（任務排程系統）
// 影片：7-8（系統設計）、7-9（邊建邊解釋）、7-10（QA + 學員題目）
// 架構：Frontend → Backend API → Redis Queue → Task Runner → MySQL
// 以 public/docs/day7-loop4-deploy.md 為手稿來源，45 張一一對應
// ============================================================
...
[▶ PPT 12/44：MySQL 為什麼 StatefulSet]`,
  },

  // ── [12/44] MySQL 為什麼 StatefulSet ──
  {
    title: '[12/44] MySQL — 為什麼不用 Deployment',
    subtitle: '有穩定身份的三件事',
    section: '7-9 第一段：基礎層',
    duration: '3',
    content: (
...
[▶ PPT 14/44：StatefulSet YAML]`,
  ...
  {
    title: '[14/44] MySQL StatefulSet YAML',
      ...
      <div className="space-y-3 text-xs">
        ...
        <div className="bg-slate-800/40 border-l-4 border-green-500 p-3 rounded">
          ...
          <ul className="text-slate-300 space-y-1 list-disc list-inside">
            <li><b>DB</b>：MySQL 或 Redis 都合理。選 Redis 要持久化就要用 StatefulSet（跟 Loop 4 Redis 用 Deployment 的判斷原則相同：資料不能丟就 StatefulSet）</li>
...
解答：DB 選 MySQL 或 Redis 都合理。這裡最有趣的判斷是——如果你選 Redis 做持久化，這時候要用 StatefulSet。因為資料不能丟了。這跟 Loop 4 的 Redis 用 Deployment，判斷原則完全相同：資料能不能自動重建？能就 Deployment，不能就 StatefulSet。
...
看到這張圖你會發現：Loop 4 學到的判斷原則，直接套到新的系統也能解。從需求出發、一個一個組件決定用什麼——這就是真正的 K8s 工程師在做的事情，背指令不是工程，做判斷才是。
