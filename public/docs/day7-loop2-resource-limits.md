...
但現在有另一個問題：你的 Pod 健康了，不代表它是個好公民。一個 Pod 可以不受限制地吃 CPU 和記憶體，把整台 Node 的資源吃光，其他 Pod 全部受影響。Linux 的 OOM Killer 會出動選一個 process 殺掉，**它不一定殺出問題的 Pod**，可能殺的是你的 API。

---

📄 7-5 第 2 張

K8s 用 **requests 和 limits** 兩個欄位來解這個問題：

| 欄位 | 說明 | 超過怎樣 |
|:-----|:-----|:---------|
| requests | 保底，給 Scheduler 排程用 | — |
| limits（CPU） | 天花板 | 限速（throttling），服務變慢但不死 |
| limits（Memory） | 天花板 | 直接殺容器，狀態叫 **OOMKilled** |

Docker 只有 `--memory` / `--cpus` 這種上限，沒有 requests 的概念。K8s 多了 requests 是因為 Scheduler 要決定 Pod 放到哪台 Node，需要知道每個 Pod 的最低需求。詳細的單位和 YAML 寫法進實作再說。
...
**requests 和 limits 詳細說明**

**requests = 保底（Scheduler 排程依據）**：Scheduler 在決定把 Pod 放到哪台 Node 時，會看每個 Node 還剩多少可用資源。Pod requests 是 500MB，Node A 剩 1GB，Node B 剩 300MB，Scheduler 把 Pod 放 Node A，因為 Node B 放不下。requests 就像預約座位，K8s 保證留給你。

**limits = 天花板（硬限制）**：
- **CPU 超過 limits**：K8s 限速（throttling），暫停 CPU 執行時間，程式變慢但服務不死。API 回應從 100ms 拉到 500ms，請求還是有回應。
- **Memory 超過 limits**：K8s 直接殺容器（OOMKilled）。記憶體不能排隊等，沒辦法「等一下再寫」，只能強制終止。Exit Code 137（SIGKILL）。

**資源單位**

CPU 單位是 **millicores（毫核）**：`1000m` = 1 核、`100m` = 0.1 核、`500m` = 0.5 核。

記憶體單位用 **Mi / Gi**（1024 進位）：`64Mi` ≈ 67MB、`128Mi` ≈ 134MB、`1Gi` ≈ 1.07GB。
...
**QoS 三個等級**：K8s 根據你的 requests/limits 自動計算，Node 資源不夠時決定誰先被殺。
...
生產環境至少要設 requests（讓 Pod 是 Burstable），才不會第一個被犧牲；而且後面的 HPA 也需要 requests 才能運作。
