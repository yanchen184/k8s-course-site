...
[▶ 下一頁]`,
  ...
  // ── 6-3（1/2）：確認 Controller + 部署應用 + Path-based routing ──
  {
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">Step 1：確認 Ingress Controller（k3s 會看到兩個）</p>
          <pre className="text-xs font-mono text-slate-300 bg-slate-900 p-2 rounded mt-1">{`svclb-traefik-xxx   2/2  Running  ← Klipper LB（k3s 特有）
traefik-xxx         1/1  Running  ← Ingress Controller`}</pre>
