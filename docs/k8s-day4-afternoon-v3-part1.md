...
第一個，把 kind 打錯。複製一個 pod.yaml，把 kind 從 Pod 改成 Podd。

指令：cp pod.yaml pod-kind-wrong.yaml
指令：（編輯 pod-kind-wrong.yaml，把 kind: Pod 改成 kind: Podd）
指令：kubectl apply -f pod-kind-wrong.yaml

你會看到錯誤訊息：error: unable to recognize "pod-kind-wrong.yaml": no matches for kind "Podd" in version "v1"。K8s 告訴你：我不認識 Podd 這個資源類型。這個錯誤在 apply 的時候就會報，Pod 根本不會被建立。

指令：rm pod-kind-wrong.yaml

第二個，把 apiVersion 改錯。複製一個 pod.yaml，把 apiVersion 從 v1 改成 v2。

指令：cp pod.yaml pod-version-wrong.yaml
指令：（編輯 pod-version-wrong.yaml，把 apiVersion: v1 改成 apiVersion: v2）
指令：kubectl apply -f pod-version-wrong.yaml

你會看到：error: unable to recognize "pod-version-wrong.yaml": no matches for kind "Pod" in version "v2"。K8s 告訴你：v2 這個 API 版本裡面沒有 Pod。因為 Pod 只存在 v1 裡面。

指令：rm pod-version-wrong.yaml

注意看，這兩個錯誤訊息的格式很像，都是 no matches for kind "xxx" in version "yyy"。差別在於一個是 kind 錯、一個是 version 錯。以後在生產環境碰到這種訊息就不會慌了，你知道要去檢查 kind 和 apiVersion 有沒有寫對。
