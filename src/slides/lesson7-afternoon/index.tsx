...
// 以 public/docs/day7-loop4-deploy.md 與 public/docs/day7-loop4-urlshortener.md 為手稿來源，48 張一一對應
// ============================================================
...
我自己寫完會回頭看一遍，六個指令一次打完：Namespace 是 Active、Secret DATA 是 3、ConfigMap DATA 是 7、StatefulSet READY 1/1、PVC Bound、mysql-0 Running 1/1。
...
Job 裡的 init container 會先等 mysql 的 service 可達，再跑 migrate.js，所以不會搶在 DB 之前跑。
    ...
    content: (
      <div className="space-y-3 text-xs">
        <div className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border border-cyan-500/50 p-3 rounded">
          <p className="text-cyan-300 font-semibold mb-1">產品功能</p>
          <p className="text-slate-300">
            使用者輸入長網址 → 系統產生 <span className="font-mono text-cyan-300">short.local/r/abc123</span> → 使用者打短網址 → API 查 DB → redirect 到原始網址
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-green-300 font-semibold">驗收非常清楚</p>
    ...
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          {[
            ['00', 'Namespace', '隔離整個產品'],
            ['01', 'Secret', 'DB password'],
            ['02', 'ConfigMap', 'DB host / port / database / user'],
            ['03', 'PostgreSQL', 'StatefulSet + PVC 保存短網址資料'],
            ['04', 'Migration Job', '建立 short_links table'],
            ['05', 'API', 'Deployment + Service + probe + resources'],
            ['06', 'Frontend', '靜態網站 Deployment + Service'],
            ['07', 'HPA', 'API CPU 自動擴縮'],
            ['08', 'Ingress', 'short.local 對外入口'],
            ['驗收', 'Browser + kubectl', '功能與 K8s 狀態都要過'],
          ].map(([step, name, desc]) => (
...
00 建 Namespace，隔離產品。01 建 Secret，放 DB password。02 建 ConfigMap，放 DB host、port、database、user。03 建 PostgreSQL StatefulSet 和 PVC，保存短網址資料。04 跑 Migration Job，建立 short_links table。05 部署 API，有 Service、probe、resources。06 部署 Frontend。07 掛 HPA 到 API。08 建 Ingress，讓 short.local 對外。
  ...
  // ── [47/48] 驗收清單 ──
  {
    ...
    content: (
      <div className="space-y-3 text-xs">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">K8s 狀態</p>
...
kubectl get hpa -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">故障測試</p>
...
K8s 驗收看三個指令：get all、get pvc、get hpa。學生要能說出每個物件在這個產品裡負責什麼。
...
[▶ PPT 48/48：Helm 一個指令]`,
  ...
  // ── [48/48] Helm 一個指令 ──
  {
    title: '[48/48] 最後收尾：Helm 一個指令',
    subtitle: '手動做一次是為了理解；工作上要能重複部署、調參、升級回滾',
...
前面一步一步 apply，是為了理解每個 K8s 元件負責什麼。但正式工作不會每天手動 apply 八個 YAML。通常會用 Helm，把這些 YAML 變成 chart。
...
最後帶走一句話：K8s 的價值不是讓你背一堆 YAML，而是讓你知道一個產品由哪些運行元件組成；Helm 的價值，是把這些元件打包成可以重複部署、可以調參、可以升級回滾的一個產品。
