...
**Q1：Pod 被砍掉之後，emptyDir 裡的資料為什麼會消失？emptyDir 跟 PVC 最根本的差別是什麼？**

A：emptyDir 的生命週期跟 Pod 綁在一起，Pod 刪了 emptyDir 也跟著刪。PVC 的生命週期跟 Pod 無關，PVC 綁的是 PV（叢集層級的儲存資源），Pod 刪了重建，一樣掛到同一個 PVC，資料就還在。

**Q2：下面這個情境應該用哪種 AccessMode？**
- 一個 MySQL 資料庫，只在一個 Node 上跑
- 一份靜態網站的 HTML 檔，10 個 Pod 都要唯讀掛載

A：MySQL 用 `ReadWriteOnce (RWO)`，資料庫需要讀寫，且只允許一個 Node 同時掛載（同時多個 Node 寫同一份資料庫檔案會造成 corruption）。靜態網站用 `ReadOnlyMany (ROX)`，多個 Pod 需要同時讀，唯讀掛載不會有衝突。

**Q3：PV 的 ReclaimPolicy 設 Retain 和 Delete 有什麼差別？生產環境建議用哪個？為什麼？**

A：`Retain`：PVC 刪掉後，PV 和資料保留，需要管理員手動決定如何處理。`Delete`：PVC 刪掉後，PV 和資料一起刪除。生產環境建議用 Retain，避免誤刪 PVC 導致資料永久消失，保留人工確認的機會。
...
實際建立 PV、PVC、MySQL Deployment，確認 Binding 狀態，寫入資料後砍 Pod，驗證資料在 Pod 重建後還在。最後示範 PVC Pending 的排查方法。
...
帶做確認 PV/PVC 狀態、排查三個常見坑，銜接下一個 Loop（StorageClass 動態佈建）。
...
坑一：`storageClassName` 打錯或不一致 → PVC 永遠 Pending，K8s 找不到符合的 PV。

坑二：PVC 的 `accessModes` 跟 PV 不一致 → 同樣 Pending，兩邊要完全吻合。

坑三：PV 的容量不夠 → PVC 申請 2Gi，PV 只有 1Gi，配不上。
...
**Q：學習環境用 hostPath，生產環境通常用什麼？兩者最大的差別在哪？**

A：生產環境通常用 NFS、AWS EBS、GCP Persistent Disk，或透過 StorageClass 動態佈建。最大差別是：hostPath 資料只存在特定 Node 上，那個 Node 壞了資料就沒了；NFS 或雲端硬碟是獨立於 Node 的儲存，Node 換了還可以重新掛載。

---

### ④ 學員實作

確認自己的 PV/PVC 都是 Bound 狀態：

```bash
kubectl get pv,pvc
```

如果有任何 Pending，用 `kubectl describe pvc <名稱>` 查 Events，根據錯誤訊息修正 storageClassName、accessModes 或容量設定。

---

### ⑤ 學員實作解答

如果做完實作後想清理資源，順序很重要：先刪 Deployment，再刪 PVC，最後刪 PV（因為 ReclaimPolicy 是 Retain，刪 PVC 後 PV 還在，需要手動刪）：
