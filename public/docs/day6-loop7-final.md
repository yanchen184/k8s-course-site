...
但如果你只是想跑一個 WordPress，不想自己組 MySQL + WordPress + PVC + Ingress，Bitnami 的 Helm Chart 幫你把這一切都包好了（用 OCI registry 方式，免費）：
...
> ⚠️ 注意：`helm repo add bitnami + helm install bitnami/wordpress` 這種方式從 2025/8/28 起 image 改付費，會出現 `Init:ImagePullBackOff`。務必使用上面的 OCI 方式。
...
> ⚠️ **Bitnami repo 方式（`helm repo add bitnami`）從 2025/8/28 起 image 改付費**
> `helm install bitnami/wordpress` 會出現 `Init:ImagePullBackOff`！
> 改用 **OCI registry 方式**（免費，不需要 helm repo add）。

**第一步：安裝 WordPress（OCI 方式）**
...
- `helm install my-blog oci://registry-1.docker.io/bitnamicharts/wordpress`：用 OCI registry 方式安裝，不需要 helm repo add，直接從 Docker Hub 拉
