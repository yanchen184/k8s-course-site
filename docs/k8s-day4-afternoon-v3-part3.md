# 第四堂下午逐字稿 v3 Part 3 — Loop 5：Deployment 入門（4-24 ~ 4-26）

> 承接 Loop 4 結尾（4-23）：MySQL 回頭操作完成，銜接 Deployment
...
這就是 Deployment 的威力。再讓你感受一個更實用的功能。假設雙十一流量暴增，你需要從 3 個 Pod 擴到 5 個。
...
| `--restart always` | `restartPolicy: Always`（Pod 層級，容器掛了自動重啟） |
| `docker compose --scale` | Deployment `replicas` + `kubectl scale`（自動補 Pod；多節點可分散） |
...
先做一件事。如果你前面的 Deployment 沒跟上，螢幕上有完整的步驟，照著做就好，我就不再逐行念了。已經做過的同學，趁這個時間做自由練習或整理筆記。
...
Docker 對照表也更新了，多了兩行。Docker 的 --restart always 對應 K8s 的 restartPolicy: Always，都是容器掛了自動重啟。docker compose --scale 對應 Deployment 的 replicas 加上 kubectl scale，不只能維持 Pod 數量，多節點叢集時 Pod 還會分散到不同的 Node。大家截圖存起來當速查卡。
...
第三，團隊要怎麼自動加人？雙十一流量暴增，你要從 3 個 Pod 擴到 10 個。今天用的 kubectl scale 是手動的，第七堂課會教 HPA，根據 CPU 使用率自動擴縮容。
...
好，第四堂課到這裡全部結束。今天從完全不認識 K8s 到能夠操作 Pod 和 Deployment，大家真的辛苦了。回去好好消化，把四個回家作業做一做。下堂課見。
