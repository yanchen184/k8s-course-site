...
這一段的目的不是把大家帶去學第二套工具，而是先直接回答一個很實際的問題：**既然業界有人用 Podman，為什麼這門課第二天主線還是先教 Docker？**

答案不是因為 Podman 不重要，而是因為對初學者教學來說，Docker 的教材、社群範例、Compose 文件、跨平台安裝路徑和搜尋到的解法最一致。你今天回家自己複習、上網查資料、跟著部落格做練習，看到的大多數內容都還是以 Docker 為主。這會讓學習摩擦最低。

換句話說，課程主線選 Docker，是一個**教學設計選擇**，不是在宣告 Podman 比較次要。

業界會選 Podman，通常不是因為它「比較潮」，而是因為它在 Linux 原生整合、權限模型和 `systemd` 管理上，對某些團隊來說更順手。

Podman 是 Red Hat 推出的容器引擎，它跟 Docker 的功能幾乎一樣，指令也幾乎一樣。你可以把 docker 命令直接換成 podman，大部分情況下都能用。

如果你把兩者放在一起看，可以先記住三件事。

先抓住概念差異就夠了，今天不做安裝示範，也不改變後面課程的操作主線。

第一，**兩者都能跑 OCI 容器，日常命令很接近**。像 `run`、`ps`、`images`、`exec` 這些常見操作，概念幾乎都一樣。所以你會用 Docker，再學 Podman 不會太痛苦。這也是為什麼課程可以放心先用 Docker 帶大家建立核心概念。
...
第三，**生態重心也不一樣**。Docker 的社群最大、教材最多、Compose 範例最多，所以學習和查資料通常比較方便。Podman 則在 Red Hat 生態系特別常見，尤其是 RHEL、CentOS Stream、Fedora 這些環境。所以如果目標是把整班學員快速帶上手，Docker 會是比較穩定的共同語言。
...
如果你是做**教學、跨平台開發、查社群文件、快速跟著範例操作**，Docker 通常最方便。因為多數教材、範例、團隊共識都在 Docker 這邊。尤其當你要跟著 Compose 文件、部落格教學或課堂 lab 一步一步操作時，Docker 的語境還是最統一。
...
不過，Docker 的生態系統還是最完整的，Docker Hub 上的 Image 最多，社群資源最豐富。所以我們這門課後續所有操作還是以 Docker 為主，不會在 lab 中途切換到 Podman，避免初學者同時背兩套路線。

如果你今天先把 Docker 的 Image、Container、Volume、Network、Dockerfile 這些核心概念學穩，之後再去看 Podman，通常會發現轉換成本沒有那麼高。反過來說，如果一開始就把 Docker 和 Podman 兩條工具線並排講，很多初學者會在「這個指令到底要用哪一套」上分心，反而抓不到容器技術真正的主軸。

今天提 Podman，是讓你知道實務上還有另一個常見選項。未來你看到團隊文件、主機環境或面試題提到 Podman，能立刻知道它和 Docker 很接近，只是預設哲學與常見使用場景不同。
