...
## 第 1.5 頁 | 多節點環境搭建：Multipass + k3s（15min）
...
好，在正式進入 Deployment 之前，我們先來升級一下實驗環境。
...
好，環境準備好了，接下來我們正式進入今天的主題 — Deployment。
...
好，概念講完了，我們馬上來動手。請大家打開終端機，確認 minikube 還在跑。
...
你應該看到 nginx 的歡迎頁面。也可以直接在瀏覽器打開 `http://<minikube-ip>:30080`。
...
- 搭配 RBAC 可以限制「誰能存取哪個 Namespace」（第七堂教）
