...
▶ **PPT 5/47：修復 worker NotReady（IP 飄移）**
...
▶ **PPT 11/47：ConfigMap 說明 — 為什麼是 Service 名稱**
...
▶ **PPT 12/47：PostgreSQL — 為什麼不用 Deployment**
...
▶ **PPT 14/47：PostgreSQL StatefulSet YAML**
...
▶ **PPT 18/47：第二段過場 — 資料層（04~07）**

### 第二段：資料層（04~07）

---

▶ **PPT 19/47：Redis — 為什麼用 Deployment**
...
▶ **PPT 21/47：Job — 一次性 DB Migration YAML**
...
▶ **PPT 23/47：RBAC — Backend 最小權限 YAML**
...
▶ **PPT 25/47：Backend Deployment + Service YAML**
...
▶ **PPT 26/47：Running 在騙你 — 為什麼需要 Probe**

### Probe 專題（先講完再回來看 Backend 四個重點）

這個 YAML 最下面有個 `readinessProbe`，先停下來講這是什麼、為什麼存在。

你怎麼確認服務正常？九成的人會說：`kubectl get pods` 看 STATUS 是 `Running` 就好。但 **Running 在騙你**。Running 只代表一件事——容器裡面的主行程還在跑。process 活著，K8s 就認為你是 Running。

三種會騙你的情境：

1. **API 死鎖**：process 還活著，但不處理任何請求
2. **連線池滿**：回 500 錯誤，但 K8s 照樣顯示 Running
3. **啟動還沒完成**：Pod 剛 Running 就接流量，前幾十秒請求全炸

K8s 預設不知道你的服務真的好了沒，它只會問 process 在不在。你必須給它一個方法——「這樣打我，打通才算好」。這個方法就是 **Probe**。

---

▶ **PPT 27/47：三種 Probe — 各問各的問題**

### 三種 Probe 各司其職

| Probe | 問的問題 | 失敗怎麼辦 | 什麼時候用 |
|:------|:---------|:-----------|:-----------|
| `livenessProbe` | 你還活著嗎？ | 重啟容器 | 解死鎖、掛掉回不了 |
| `readinessProbe` | 準備好接流量了嗎？ | 從 Service 移除 | 啟動中、暫時不能服務 |
| `startupProbe` | 啟動完了沒？ | 重啟容器 | 慢啟動（Java 啟動 60 秒） |

**livenessProbe**：失敗就把容器整個重啟（重啟容器，不是刪 Pod）。解死鎖、解掛死那種回不來的狀況。

**readinessProbe**：失敗 K8s 不重啟容器，而是把 Pod 從 Service 後端列表拔掉，不再給流量。恢復了自動加回來。適合「暫時不能服務但會自己恢復」。

**startupProbe**：專門給啟動特別慢的應用。通過之前 liveness/readiness 都不執行，通過後 startupProbe 退場。沒有它的話 liveness 會在啟動中判定失敗，Pod 一直被重啟起不來。
...
| `tcpSocket`：連 port，連上成功 | PostgreSQL、Redis、無 HTTP 的服務 |
| `exec`：執行指令，exit 0 成功 | 自訂檢查邏輯 |

對照一下：Docker HEALTHCHECK 只有一種，失敗只貼 unhealthy 標籤，不重啟也不切流量。K8s 三種 Probe 各管各的，而且會**真的採取動作**。

---

▶ **PPT 28/47：Probe 的四個關鍵參數**
...
readinessProbe:
  ...
  timeoutSeconds: 1           # 每次戳等幾秒沒回應算超時
```

- **initialDelaySeconds**：設太小啟動中誤判失敗；設太大出問題反應慢
- **periodSeconds**：readiness 通常比 liveness 短（好了趕快接流量）
- **failureThreshold**：不要設 1，網路抖一下就誤判。一般 3 次比較穩
- **timeoutSeconds**：超時算失敗。預設 1 秒對慢服務可能不夠，可以調到 3~5

### 回到 Loop 3：每個組件該用哪種？

| 組件 | Probe 建議 | 說明 |
|:-----|:-----------|:-----|
| **Backend** | readiness + httpGet `/health` | Web API 標準做法（下一頁就會看到） |
| **PostgreSQL** | readiness + tcpSocket `5432` | 沒 HTTP，連上就算好 |
| **Redis** | readiness + exec `redis-cli ping` | TCP 連上不代表有回應，要確認回 PONG |
| **Task Runner** | 不用（或只配 liveness） | 沒人連它，readiness 沒意義 |

**判斷原則**：Probe 不是每個 Pod 都要配滿，看它的角色決定要哪幾種。有人要連你 → 需要 readiness；可能死鎖 → 需要 liveness；啟動特別慢 → 需要 startup。
...
▶ **PPT 32/47：第三段過場 — 應用層（08~12）**

### 第三段：應用層（08~12）

---

▶ **PPT 33/47：Frontend Deployment + Service**
...
▶ **PPT 34/47：Task Runner 用 Deployment 就好 — 兩個判斷**
...
▶ **PPT 35/47：Task Runner Deployment YAML**
...
▶ **PPT 38/47：concurrencyPolicy 三選項**
...
▶ **PPT 39/47：Ingress — k3s 用 Traefik**
