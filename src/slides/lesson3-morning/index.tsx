      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-slate-800/50 p-3 rounded-lg">
            ...
            <p className="text-slate-300 text-xs">容器暫停，程序被 freezer cgroup 凍結但仍在記憶體中。</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-lg">
...
然後是 Paused 狀態，這個狀態比較特殊。當你執行 docker pause 的時候，Docker 會透過 freezer cgroup 把容器內的程序整體凍結，程序不再繼續往前執行，但是它們還在記憶體裡面。就像你按下暫停鍵，程序停在那一刻，等到你 docker unpause 的時候，程序繼續從中斷的地方執行。這個功能在你需要暫時停止容器又不想讓它完全關閉的時候很有用，比如你想為容器做一個快照。
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="bg-slate-800/50 p-3 rounded-lg">
              ...
              <p className="font-mono text-green-400 text-sm mt-1">docker create --name mybox nginx:alpine</p>
            </div>
            <div className="bg-slate-800/50 p-3 rounded-lg">
...
docker pause 和 docker unpause 這組指令很有趣。pause 不是送出 SIGSTOP，而是由 Docker 透過 freezer cgroup 把容器的程序凍結，記憶體不釋放。unpause 則讓程序繼續執行。這個功能最常見的使用場景是在做容器快照或備份的時候，暫時把容器凍結，確保資料一致性，做完之後再繼續。
...
最後要說一下重啟策略。在生產環境，我們通常希望容器在崩潰後能夠自動重啟，這就是 --restart 選項的用途。常用的值有：always（容器退出後會重啟；如果你手動 stop，會先保持停止，等 Docker daemon 重啟或你再手動 start 才恢復策略）、on-failure（只在非正常退出時重啟，可以加冒號指定最大重試次數）、unless-stopped（除非手動停止，否則會持續重啟，Docker daemon 重啟後也維持停止狀態）。對於生產環境的服務，我通常推薦使用 unless-stopped 或 always。大家來試試看，我們一起實際執行這些指令。
...
Q：為什麼你一直強調自訂 bridge 網路？預設 bridge 不行嗎？

A：預設 bridge 在可管理性與命名解析上都較弱，容易在多服務場景變成隱性風險。自訂網路可讓服務命名更清楚、隔離更可控，也更接近後續 Kubernetes 心智模型。

Q：host network 效能比較好，那是不是都用 host 就好？

A：不建議。host 模式會犧牲隔離與可移植性，埠衝突與安全邊界也更難控管。除非有明確效能或特殊網路需求，否則預設應選擇橋接與服務化路徑。

Q：Dockerfile 裡 EXPOSE 了，為什麼外部還是連不到？

A：EXPOSE 是文件宣告，不會自動開主機埠。你仍需要在執行時做埠映射或讓平台幫你建立對外通道。這是初學者最常見誤解。

Q：Bind Mount 和 Named Volume 到底怎麼選？

A：開發期重即時同步與可見性，常選 Bind Mount；生產期重穩定與生命週期管理，通常選 Named Volume。不要把開發便利策略直接搬進正式環境。

Q：restart=always 很保險，為什麼你說也可能有風險？

A：因為它可能掩蓋根因，讓錯誤在背景無限重試。沒有監控與退避機制時，會造成資源耗盡與噪音。重啟策略要和告警、log 一起設計。

Q：CPU 和記憶體限制應該抓多高才合理？

A：先看基準負載與尖峰，再留安全緩衝，不要拍腦袋。過低會頻繁限流或 OOM，過高又浪費資源並影響排程。這是持續觀測後迭代的參數，不是一次定終身。

Q：env-file 很方便，但會不會把機密帶進 Git？

A：會，所以必須把敏感檔案排除版本控制，並區分範例檔與真實檔。正式機密要交給秘密管理系統，不要依賴本機檔案習慣。

Q：容器顯示 Running 但服務連不到，排查優先順序是什麼？

A：先確認程序是否真的在容器內監聽正確埠，再確認映射與網路路徑，最後看防火牆與存取來源。Running 只代表程序活著，不代表業務路徑通。`,
    duration: "10"
  },
]
