...
sudo sh -c 'echo "<NODE-IP> short.local" >> /etc/hosts'`}</code></pre>
          </div>
        </div>
...
hosts 也要講。學生的瀏覽器要知道 short.local 指向哪個 node IP，所以要加 hosts。Windows 環境要改 Windows hosts；Linux 操作環境可以改 /etc/hosts。這裡投影片給的是概念，實際 IP 要換成自己的 node IP。
