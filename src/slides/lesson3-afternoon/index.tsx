...
Q：Dockerfile 的層快取要怎麼排才最有效？

A：把低變動成本步驟放前面，高變動步驟放後面。依賴安裝通常在前，應用程式碼通常在後。這樣改一行程式不會重建整個世界。

Q：多階段建構是不是所有專案都該用？

A：多數服務都值得用，但不是教條。核心目標是減少產物體積與攻擊面，如果你的流程已足夠精簡，也可先不複雜化。選擇要看效益與維護成本。

Q：Docker Compose 能直接上正式環境嗎？

A：小型單機服務可以，但擴展性、容錯、治理能力有限。團隊一旦進入多節點與多環境，通常會轉向 Kubernetes 或其他編排平台。Compose 很好用，但有邊界。

Q：Compose 裡放密碼真的不行嗎？

A：測試可暫用，正式不建議。配置檔容易外流且權限控管粗糙，應改用 secret 管理或外部機密注入。方便與安全要分場景取捨。

Q：Named Volume 要怎麼備份與搬遷？

A：要把備份流程做成標準作業，而不是事故後才想。常見做法是臨時容器掛載 volume 後匯出，並定期驗證可還原性。只有可還原的備份才算備份。

Q：BUILD ARG 和 ENV 常混淆，差異最關鍵是什麼？

A：ARG 主要在建構期使用，ENV 會進入執行期環境。把機密放 ARG 也不一定安全，因為可能留在建構痕跡。設計時先分清楚建構需求與執行需求。

Q：多環境是不是要維護多份 compose 檔？

A：建議主檔加覆蓋檔，避免完全複製造成漂移。共通設定放 base，環境差異放 override。這樣能兼顧可讀性與一致性。

Q：depends_on 設了就代表資料庫一定先可用嗎？

A：不代表。depends_on 只保證啟動順序，不保證服務就緒。真正穩定做法是健康檢查加重試邏輯，避免偶發開機競態。`,
    duration: "10"
  },
]
