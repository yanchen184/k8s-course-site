...
這個 Loop 不複習定義，不背指令清單。從 Docker 扛不住的那一刻開始，走一遍完整的因果鏈。每一個概念出現都是因為上一步有問題，解決了問題又冒出新問題，一環扣一環，這就是這四堂課的學習邏輯。

走完這條鏈，你的腦袋裡就有一張完整的 K8s 知識地圖。
...
四堂課之前，你學完了 Docker。docker run 跑容器、Dockerfile build Image、Docker Compose 管多服務，在筆電上跑得好好的。

然後第一個問題來了。上了生產環境，一台伺服器跑所有容器，某天伺服器掛了，全部服務一起掛。流量暴增，一台機器扛不住。你想加第二台、第三台，但手動管太痛苦。

因為 Docker 管不了一群機器上的一群容器 → 所以學了 **Kubernetes**：容器編排平台，把一群機器組成叢集，你只要告訴 K8s 你想要什麼狀態，K8s 幫你實現。
...
每一個概念都不是憑空出現的。它出現是因為上一步有問題。這條因果鏈就是你的 K8s 知識地圖。遇到問題的時候，沿著鏈走就知道該用什麼工具。
