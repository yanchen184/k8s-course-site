# 第四堂（上）— K8s 全貌 + 環境搭建（完整投影片 + 逐字稿）

> 13 頁投影片，含 PPT 提詞內容 + 可直接念的逐字稿
> 對象：已學完 3 堂 Docker 課的容器初學者
...
- 這個章節：概念 + 架構
- 下個章節：環境搭建 + kubectl 探索（動手）
- 再下個章節：Pod 實作（寫 YAML、部署、排錯）

**先裝起來！可以先暫停影片安裝，或邊聽邊跑：**
```bash
# 1. 安裝 minikube（Ubuntu / Debian）
...
歡迎來到第四堂課。前三堂課我們從 Linux 基礎開始，學會了 `cd`、`mkdir`、`ls` 這些終端機操作，還有 `chmod`、`curl`、`ssh` 等檔案和網路指令；接著進入 Docker 的世界，學會了用 `docker run` 跑容器、用 `-p` 做 Port Mapping、用 `-v` 掛 Volume；第三堂課我們更進一步，用 Dockerfile 打包自己的應用程式，還用 Docker Compose 一次管理多個容器。

到目前為止，我們所有的操作都是在一台機器上完成的。這在開發和測試的時候完全沒問題，但如果要把應用程式上線到生產環境，只靠一台機器、靠 Docker Compose 是不夠的。從今天開始，我們要進入 Kubernetes 的領域。

這個章節我會帶大家把 Kubernetes 的全貌走一遍：它為什麼存在、核心概念有哪些、架構長什麼樣子。下個章節會動手搭建環境、用 kubectl 探索叢集。之後才開始正式寫 YAML、部署 Pod。

在繼續之前，建議大家先把 minikube 裝起來。螢幕上有完整的安裝指令，就是這四個步驟：下載 minikube、下載 kubectl、啟動叢集、驗證。你可以先暫停影片把它裝好，也可以讓它在背景跑，等我們講到實作的時候就可以直接用了。

---

## 第 2 頁 | 為什麼需要 Kubernetes（10min）
...
| 容器數量 | 管理方式 | 問題 |
|:---:|---------|------|
| 1-5 個 | `docker run` | 沒問題 |
| 5-20 個 | Docker Compose | 單機極限 |
| 成百上千 | ？？？ | 腳本管不動了 |

**容器多了、機器多了之後的五大問題：**

1. **調度** — 容器該放哪台機器？哪台有空位？
2. **故障恢復** — 某台機器掛了，上面的容器怎麼辦？
3. **彈性擴縮** — 流量暴增，怎麼快速加容器？退了怎麼縮回來？
4. **滾動更新** — 更新版本的時候，怎麼不中斷服務？
5. **服務發現** — 容器之間怎麼互相找到對方？

**Docker Compose 能解決嗎？**
- Docker Compose 只能管一台機器上的容器
- 跨機器？它做不到
- 自動故障恢復？它做不到
- 自動擴縮容？它做不到
...
## 第 3 頁 | K8s 是什麼 + 能力（5min）

### PPT 上的內容

**一句話：容器的管理平台（容器編排引擎）**

- Google 內部用 **Borg** 系統管理數十億個容器
- 2014 年 Google 把經驗開源 → **Kubernetes**
- 捐給 CNCF（Cloud Native Computing Foundation）
- K8s = K + 8個字母 + s = Kubernetes
- 目前是容器編排領域的**行業標準**
...
| 問題 | K8s 怎麼解決 |
|------|------------|
| 調度 | Scheduler 自動把容器分配到最適合的機器 |
| 故障恢復 | 機器掛了 → 自動把容器搬到其他機器重建 |
| 彈性擴縮 | 一行指令或自動根據負載增減容器數量 |
| 滾動更新 | 逐步用新版本替換舊版本，零停機 |
| 服務發現 | 內建 DNS，容器用名字就能找到對方 |

**其他能力：** 災難恢復、負載均衡、配置管理、密碼管理、儲存管理

### 逐字稿

那 Kubernetes 到底是什麼？用一句話來說，它就是一個容器的管理平台，學術一點的說法叫「容器編排引擎」。
...
## 第 4 頁 | 核心概念：Node + Pod + Service（15min）
...
- 容器在 Pod 裡共享網路和儲存
- 最佳實踐：一個 Pod 放一個容器
- 多容器情況 → Sidecar 模式（主程式 + 輔助容器，如日誌收集）
- 對照 Docker：`docker run` 跑一個容器 ≈ 一個 Pod

**為什麼需要 Service：**
```
問題 1：Pod IP 是叢集內部的 → 外面連不到
問題 2：Pod 會被銷毀重建 → IP 會變
```

**Service = 穩定的存取入口**
- Pod 掛了換新的 → Service 地址不變
- 自動轉發到健康的 Pod
- 對照 Docker：`docker run -p 8080:80` 做 port mapping

**Service 類型：**
| 類型 | 作用 | 存取範圍 |
|------|------|---------|
| ClusterIP（預設）| 叢集內部存取 | 內部 |
| NodePort | 在 Node 上開 Port | 外部 |
| LoadBalancer | 外部負載均衡器 | 外部 |

### 逐字稿

好，現在我們來認識 Kubernetes 的核心概念。
...
## 第 5 頁 | 核心概念：Ingress + ConfigMap + Secret（10min）

### PPT 上的內容

**Ingress — HTTP 路由器：**
- NodePort 用 IP + Port 存取 → 開發測試 OK
- 生產環境要用域名（`www.example.com`）
- Ingress = 管理外部 HTTP 存取的入口
  - 不同域名 → 不同 Service
  - 不同路徑（`/api`、`/web`）→ 不同 Service
  - 可配置 SSL 憑證（HTTPS）
- 對照 Docker：在 Docker 裡面用 Nginx 做反向代理

**ConfigMap — 設定檔管理：**
- 問題：資料庫地址寫死在 Image 裡 → 改地址就要重新 build
- ConfigMap = 把設定抽出來，跟 Image 分離
  - 修改設定 → 不用重新 build Image
  - 修改 ConfigMap → 重新載入 Pod 即可
- 注意：明文儲存，不放密碼
- 對照 Docker：`docker run -e DB_HOST=xxx`

**Secret — 敏感資料管理：**
- 密碼、API Key、憑證 → 不該放 ConfigMap
- Secret = 跟 ConfigMap 類似，但做了 Base64 編碼
- 注意：Base64 是編碼不是加密，需搭配其他安全機制
- 對照 Docker：`.env` 檔案

### 逐字稿

繼續往下看三個概念：Ingress、ConfigMap 和 Secret。
...
這就是 Ingress 的作用。Ingress 是一個 HTTP 層的路由器，可以根據域名或路徑把請求轉發到不同的 Service。比如 `www.example.com` 導到前端的 Service，`api.example.com` 導到後端的 Service，還可以配置 SSL 憑證來支援 HTTPS。用 Docker 的經驗來對照，Ingress 的角色就像你在 Docker 環境裡面架一個 Nginx 做反向代理，只不過在 K8s 裡面這是一個內建的資源物件，設定起來更方便。不過要注意，Ingress 本身只是一組規則定義，要讓它真正運作，還需要安裝一個叫做 Ingress Controller 的東西，比如 nginx-ingress-controller。這個我們在第五堂課會詳細教。
...
K8s 提供了 ConfigMap 來解決這個問題。你可以把所有的設定資訊，像是資料庫地址、Port、各種參數，全部放到 ConfigMap 裡面。應用程式的 Image 裡面完全不包含這些設定。當設定需要修改的時候，只要改 ConfigMap 然後重啟 Pod 就好了，不需要重新 build Image。如果是用 Volume 掛載的方式，修改後會自動更新，但會有一點延遲；如果是用環境變數注入的方式，就需要重啟 Pod 才會生效。
...
## 第 6 頁 | 核心概念：Volume + Deployment + StatefulSet（10min）
...
- Volume = 把資料存在容器外面
  - 可掛載到本地磁碟或遠端儲存（NFS、雲端硬碟）
  - 容器重啟 → 資料不遺失
- 對照 Docker：`docker volume`、`-v /host:/container`

**Deployment — 管理無狀態應用：**
- 問題：只跑一個 Pod → 掛了就停服務
- 解決：多跑幾個副本（replicas）
- Deployment = 管理 Pod 副本的控制器
  - 副本控制：定義要跑 3 個 → 掛了一個 → 自動補一個
  - 滾動更新：逐步替換新版本，不中斷服務
  - 三層關係：`Deployment → ReplicaSet → Pod`
  - ReplicaSet 是自動建立的，你只管 Deployment
- 對照 Docker：`docker compose up --scale web=3`

**StatefulSet — 管理有狀態應用：**
- Deployment 適合：前端、API（無狀態，每個副本一樣）
- 資料庫不適合用 Deployment → 每個副本有自己的資料
- StatefulSet = 給有狀態應用用的
  - 穩定的網路標識（`mysql-0`、`mysql-1`、`mysql-2`）
  - 有序部署和刪除（先 0 再 1 再 2）
  - 每個 Pod 有自己獨立的儲存
- 實務建議：資料庫可以不放 K8s，在外面單獨跑

### 逐字稿

接下來是 Volume、Deployment 和 StatefulSet 這三個概念。
...
| 概念 | 做什麼 | Docker 對照 |
|------|--------|-----------|
| **Node** | 一台機器，叢集的組成單位 | 你的那台 Linux 主機 |
| **Pod** | 容器的包裝，最小調度單位 | `docker run` |
| **Service** | 穩定的存取入口，負載均衡 | `-p 8080:80` + `--network` DNS |
| **Ingress** | HTTP 路由器，域名和路徑轉發 | Nginx 反向代理 |
| **ConfigMap** | 設定檔管理（明文） | `-e ENV_VAR=value` |
| **Secret** | 敏感資料管理（Base64） | `.env` 檔案 |
| **Volume** | 資料持久化 | `docker volume` / `-v` |
| **Deployment** | 管理無狀態應用的副本 | `docker compose up --scale web=3` |
| **StatefulSet** | 管理有狀態應用（資料庫） | 手動管理，Docker 沒有對應 |
...
→ 後面四堂課會一個一個展開來教、一個一個動手做

### 逐字稿

好，我們來做一個小結。剛剛一口氣講了 8 個核心概念，我們用一張表來整理一下，並且對照你已經會的 Docker 來加深記憶。
...
好，概念的部分到這裡。接下來我們來看 K8s 的架構，也就是這些概念在底層是怎麼運作的。

---

## 第 8 頁 | K8s 架構：Worker Node（15min）

### PPT 上的內容

**K8s = Master-Worker 架構**
- Master Node → 管理整個叢集（公司管理層）
- Worker Node → 跑你的應用程式（實際幹活的員工）

**Worker Node 上的三個組件：**

**1. Container Runtime（容器執行時期）**
- 負責：拉映像檔、建容器、啟動/停止容器
- 每個 Worker Node 都必須裝
- Docker 用的是 Docker Engine
- K8s 主流用 **containerd**（Docker 的底層也是用它）
- 對照：你之前裝的 Docker，其實底層就是 containerd

**2. kubelet（節點管家）**
- 負責：管理這個 Node 上的所有 Pod
- 從 API Server 接收指令：「要在你這邊跑一個 nginx Pod」
- 確保 Pod 按照預期運行
- 定期回報 Node 狀態給 API Server
- 對照：像 Docker Daemon，但只管這一台 Node

**3. kube-proxy（網路代理）**
- 負責：網路代理和負載均衡
- 請求進來 → 轉發到正確的 Pod
- 在叢集內實現負載均衡，將流量分配到各個 Pod
- 對照：Docker 的 port mapping + 網路橋接

### 逐字稿

好，接下來我們看 Kubernetes 的架構，也就是底層是怎麼運作的。
...
第三個是 kube-proxy，負責網路代理和負載均衡。當一個請求進來的時候，kube-proxy 負責把它轉發到正確的 Pod。它會在叢集內建立轉發規則，把流量均勻分配到所有健康的 Pod 上。用 Docker 來對照，kube-proxy 的角色就像 Docker 的 port mapping 加上網路橋接，但功能更強大。

---

## 第 9 頁 | K8s 架構：Master Node（15min）
...
- 持續監控所有資源的「現在狀態」vs「期望狀態」
- 發現不一致 → 自動修復
- 例：你說要 3 個 Pod，掛了一個只剩 2 → 自動補一個

**完整流程範例：**
```
你輸入：kubectl create deployment nginx --replicas=3

1. kubectl 把請求送到 → API Server
2. API Server 驗證你的權限 → 通過
3. API Server 把「要 3 個 nginx Pod」記錄到 → etcd
4. Scheduler 發現有 3 個 Pod 還沒分配
   → 看各 Node 資源 → 分配到合適的 Node
5. 對應 Node 的 kubelet 收到通知
   → 呼叫 Container Runtime → 把容器跑起來
6. Controller Manager 持續監控
   → Pod 掛了就告訴 Scheduler 重新分配
```

### 逐字稿

看完 Worker Node，我們來看 Master Node。Master Node 上有四個核心組件，它們協同合作來管理整個叢集。
...
**一句話版本：**
- 你用 `kubectl` → 跟 API Server 說話 → API Server 指揮整個叢集
- 等一下實作就會在 `kube-system` namespace 裡親眼看到這些組件

### 逐字稿

好，我們把剛剛講的內容用一張架構圖來整理。大家看螢幕上這張圖。
...
|------|---------|:---:|:---:|:---:|
| **minikube** | 個人學習 | 單節點 | 一行指令 | 今天（第四堂） |
| **k3s** | 輕量生產 / 多節點學習 | 多節點 | 一行指令 | 第五堂 |
| **RKE / kubeadm** | 企業生產叢集 | 多節點 | 較複雜 | 待定 |

**minikube：**
- 在一台電腦上模擬一個完整的 K8s 叢集
- Master + Worker 合在同一台
- 底層跑在 Docker 或虛擬機裡面
- 適合學習和本地測試，不適合生產

**kubectl = K8s 的命令列工具：**
- 不管用 minikube、k3s 還是雲端 K8s → kubectl 指令都一樣
- 學一次，到處用
...
| `docker logs <id>` | `kubectl logs <pod>` | 看日誌 |
| `docker exec -it <id> bash` | `kubectl exec -it <pod> -- bash` | 進容器 |
| `docker stop/rm <id>` | `kubectl delete pod <pod>` | 停止/刪除 |
| `docker compose up -f` | `kubectl apply -f xxx.yaml` | 用檔案部署 |
| `docker images` | `kubectl get deployments` | 看部署的應用 |

### 逐字稿

概念和架構都講完了，接下來要解決一個很實際的問題：我們的 K8s 叢集要用什麼來跑？
...
## 第 12 頁 | 實作：minikube + kubectl（50min）
...
**Step 3：驗證叢集**
```bash
kubectl get nodes                   # 應該看到一個 Ready 的節點
kubectl get nodes -o wide           # 更多資訊：IP、OS、Container Runtime
...
kubectl get ns                      # 看命名空間：default、kube-system、kube-public...
```
→ 對照剛剛的架構圖，每個組件都在這裡
...
# 重點看：
#   - Roles: control-plane（Master + Worker 合一）
#   - Container Runtime: containerd 或 docker
#   - CPU / Memory 容量和已分配量
#   - 上面跑了哪些 Pod
```

**Step 6：打開 Dashboard（圖形介面）**
```bash
minikube dashboard
# 會自動開瀏覽器
# 可以用圖形介面看到叢集的所有資源
...
**Step 7：基本 kubectl 探索**
```bash
kubectl get all                     # 看 default namespace 的所有資源
kubectl get all -n kube-system      # 看系統層的所有資源
kubectl api-resources               # K8s 支援的所有資源類型
```

### 逐字稿

好，終於到了動手的時間！大家打開終端，我們一步一步來。
...
| 主題 | 內容 |
|------|------|
| 為什麼需要 K8s | Docker 單機的五個極限 → K8s 全部解決 |
| K8s 是什麼 | Google 開源的容器編排引擎，行業標準 |
| 8 個核心概念 | Pod / Service / Ingress / ConfigMap / Secret / Volume / Deployment / StatefulSet |
| K8s 架構 | Master 4 組件 + Worker 3 組件 |
| 環境方案 | minikube（第四堂）/ k3s（第五堂）/ RKE |
| 實作 | minikube 安裝 + kubectl 探索 + 親眼看到架構組件 |

**下一章要做的事：**

1. YAML 基本格式（apiVersion / kind / metadata / spec）
2. 寫第一個 Pod YAML（nginx）→ 完整生命週期操作
   - `apply → get → describe → logs → exec → delete`
3. 故意搞壞 Pod → 學會用 `describe` 排錯
   - 打錯 image 名 → `ImagePullBackOff` → 找原因 → 修正
4. 多容器 Pod（Sidecar 模式）
   - nginx + busybox 共享 Volume
5. 自由練習時間
