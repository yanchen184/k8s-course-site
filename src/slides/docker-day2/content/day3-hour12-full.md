...
> 以下是原本 Hour 12 的講課內容（Node.js+TS 實戰、Spring Boot 實戰、常見問題排查），保留作為補充教學素材。講師可視時間和學生程度，選擇性地在 Hour 10-11 的講課中穿插使用。

### A. Node.js + TypeScript 生產級專案
...
- npm ci 嚴格按 package-lock.json，適合 Dockerfile 和 CI/CD
- npm install 可能更新版本，適合本地開發

**dumb-init — PID 1 信號問題：**
- Node.js 不是設計來當 PID 1 的
- 沒有 dumb-init：docker stop → SIGTERM → Node.js 沒反應 → 10 秒 SIGKILL
- 有 dumb-init：docker stop → SIGTERM → dumb-init 轉發 → Node.js 優雅關閉（1-2 秒）
...
- 舊版 Java 8 JVM 讀宿主機記憶體而非容器限制
- 宿主機 32GB → 容器限制 512MB → JVM 以為有 32GB → 分配 8GB → OOM Kill
- Java 10+ 加入容器感知，讀 cgroup 限制

**`-XX:MaxRAMPercentage=75.0` vs `-Xmx`：**
- MaxRAMPercentage 按比例，容器記憶體改變時自動調整
- -Xmx 是絕對值，改了容器記憶體還得改 Dockerfile
- 留 25% 給 Metaspace、Code Cache、Thread Stack、GC Overhead

**`-Djava.security.egd=file:/dev/./urandom`：**
- 容器環境熵池補充慢，`/dev/random` 可能阻塞
- Spring Boot 啟動卡住數十秒的常見原因

**HEALTHCHECK start-period：**
- Node.js：5s（啟動很快）
- Spring Boot：30s（JVM 初始化 + Spring Context 掃描需要 10-30 秒）

### C. 常見 Dockerfile 問題排查
...
**第一步：永遠先看日誌** `docker logs my-container`

常見原因：
- Alpine 沒有 bash → 用 sh 或直接跑
- 程式找不到 → `docker run -it --entrypoint sh my-image` 進去看
- 缺少環境變數 → `-e DATABASE_URL=...`
...
- 路徑相對於 Build Context，不是 Dockerfile 位置
- 被 .dockerignore 排除了
- Linux 大小寫敏感（Mac 開發推到 CI/CD 失敗的經典坑）

#### 問題五：權限問題

切換到非 root 使用者後，COPY 的檔案預設是 root 擁有。
```dockerfile
# 解法：COPY 時指定擁有者
