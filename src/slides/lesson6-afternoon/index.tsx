...
Q：PVC 一直 Pending，最有效率的排查順序是什麼？

A：先看 PVC 事件，再對照 StorageClass、容量、存取模式、selector。不要一開始就懷疑平台壞掉，多數是規格不匹配。

Q：StorageClass 的 reclaimPolicy 應該用 Delete 還是 Retain？

A：看資料重要性與治理流程。生產資料通常偏 Retain，避免誤刪；測試環境可用 Delete 控制成本。策略要和回收流程一起設計。

Q：雲端磁碟常見只支援 RWO，那多副本資料庫怎麼辦？

A：這是儲存架構選型題。可選用支援 RWX 的儲存系統，或改採資料庫層複寫而非共用同一卷。不要強行用不相容模式。

Q：PVC 可以在線擴容，那能不能縮小？

A：一般不支援縮小。實務上要透過新卷搬遷或資料重建流程處理。容量規劃時要預留成長空間，避免反覆搬遷。

Q：StatefulSet 改名或重建時，PVC 會不會跟著亂掉？

A：有機會，因為 PVC 命名與 StatefulSet 物件綁得很緊。任何重構前都要先設計資料遷移步驟，不要假設平台會自動對應。

Q：備份只做 snapshot 就夠嗎？

A：通常不夠。你還要考慮應用一致性、交易邊界與還原演練。能備份不代表能正確還原。

Q：RPO 和 RTO 在儲存策略裡要怎麼落地？

A：先把業務可接受損失與停機時間量化，再反推備份頻率、複寫架構與演練頻率。沒有量化目標，策略就會流於口號。

Q：刪 namespace 時如何避免把重要資料一起刪掉？

A：先盤點 PVC 與回收策略，再執行刪除。關鍵資料應使用 Retain 或外部備份保護，並把刪除流程做成可審核清單。`,
    duration: "10"
  },
]
