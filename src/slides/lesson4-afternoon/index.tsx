...
export const slides: Slide[] = [
  ...
  {
    title: 'Pod 生命週期狀態流程',
    subtitle: 'Pod phase vs kubectl STATUS',
    section: 'Loop 1：Pod 生命週期 + 排錯',
    duration: '7',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-3">Pod phase（Kubernetes API 的高層摘要）</p>
          ...
          <p className="text-slate-400 text-xs mt-3">
            <code className="text-blue-300">ContainerCreating</code> 常出現在 <code className="text-yellow-300">kubectl get pods</code> 的 STATUS，
            通常表示容器仍在 Waiting，例如正在拉 image 或建立 container。
          </p>
        </div>

        <div className="bg-red-900/30 border border-red-500/30 p-4 rounded-lg">
          <p className="text-red-400 font-semibold mb-2">kubectl 常見 STATUS / Reason</p>
          <table className="w-full text-sm">
            ...
            <tbody className="text-slate-300">
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4"><code className="text-blue-300">ContainerCreating</code></td>
                <td className="py-2 pr-4">容器仍在 Waiting</td>
                <td className="py-2 pr-4">正在拉 Image、掛 Volume、套用 Secret / ConfigMap</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              ...
              <ol className="text-slate-300 text-xs space-y-0.5 list-decimal list-inside">
                ...
                <li>YAML 四大欄位</li>
                <li>Pod CRUD 六指令</li>
                <li>Pod phase + kubectl 常見 STATUS</li>
                <li>排錯三兄弟</li>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="text-slate-300">
              ...
              <tr className="border-t border-slate-700">
                ...
                <td className="py-1">Deployment <code>replicas</code>（自動補 Pod；多節點時可分散）</td>
              </tr>
            </tbody>
          </table>
        </div>
...
Docker 對照表也更新了，多了兩行。docker compose --scale 對應 kubectl scale deployment。Docker 的 --restart always 只能管同一台機器，Deployment 的 replicas 會幫你維持 Pod 數量；如果是多節點叢集，Pod 還可能分散到不同的 Node，但那是 Scheduler 決定的。大家截圖存起來當速查卡。 [▶ 下一頁]`,
  },
