...
到目前為止 Ingress、ConfigMap、Secret 都學了。接下來是今天上午的最終 Boss——把三個東西合在一起。
...
[▶ 下一頁]`,
  ...
  {
    title: '整合實作引導：Namespace + 三服務 + 九步驟',
    subtitle: '目標：一個 namespace 裡跑完整的 MySQL + 前端 + API，用 Ingress 對外',
    section: '6-8：整合實作',
    duration: '5',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          ...
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-3 rounded">
            <p className="text-slate-500"># namespace: my-app</p>
            <p>Ingress（traefik）</p>
            <p className="text-slate-500 ml-4">├── /frontend → frontend-svc → frontend-deploy（k8s-demo-app）</p>
            <p className="text-slate-500 ml-4">├── /api     → api-svc     → api-deploy（k8s-demo-app）</p>
            <p className="text-slate-500 ml-4">└── mysql   → mysql-svc   → mysql Deployment（ConfigMap + Secret）</p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          ...
          <ol className="text-slate-300 text-sm space-y-1 list-decimal list-inside">
            <li>建 Namespace <code className="text-cyan-400">my-app</code></li>
            <li>建 Secret（MySQL root 密碼）</li>
            <li>建 ConfigMap（前端訊息設定）</li>
            <li>deploy MySQL + mysql-svc（ClusterIP）</li>
            <li>deploy frontend-deploy（注入 ConfigMap + Secret）</li>
            <li>deploy api-deploy</li>
            <li>建 Ingress（path-based，traefik）</li>
            <li>curl 驗收三個端點</li>
            <li>清理整個 namespace</li>
          </ol>
        </div>

        <div className="bg-yellow-900/30 border border-yellow-500/30 p-3 rounded-lg">
          <p className="text-yellow-400 text-xs font-semibold mb-1">新概念：Namespace</p>
          <p className="text-slate-300 text-xs">Namespace 是 K8s 的「資料夾」，同一個叢集裡邏輯隔離不同環境或專案。<code className="text-green-400">-n my-app</code> 或 <code className="text-green-400">--namespace my-app</code> 指定 namespace。清理時直接刪整個 namespace，一行搞定。</p>
        </div>
      </div>
    ),
    code: `# 今天的目標：三個服務在同一個 namespace 跑起來
# 驗收標準：
curl http://<NODE-IP>/frontend   # → Hello from frontend
curl http://<NODE-IP>/api        # → Hello from api

# Namespace 是 K8s 的「資料夾」
kubectl create namespace my-app

# 所有後續指令加 -n my-app，或 kubectl config set-context --current --namespace=my-app`,
    notes: `好，兩個 Loop 都走完了——Ingress 讓外面的人連進來，ConfigMap 和 Secret 把設定和密碼抽出來。接下來是今天上午的最終整合。

這次我們要在一個乾淨的 namespace 裡面，從零建起一個完整的系統：MySQL 資料庫、前端服務、API 服務，全部用 Ingress 串起來。

先介紹一個新概念：Namespace。K8s 的 Namespace 就是一個資料夾，同一個叢集裡面可以有很多個 Namespace，彼此邏輯隔離。你的公司可能用 dev、staging、prod 三個 Namespace，同一個叢集跑三個環境，互不干擾。今天我們建一個 my-app，把所有東西都放在裡面，最後清理只要刪這個 Namespace，一行指令全部消失，不用一個一個刪。

整個流程九個步驟，我來示範一遍，你們跟著看。
...
[▶ 下一頁 — 開始九步示範]`,
  ...
  {
    title: '整合示範：九步驟走完',
    subtitle: 'Namespace → Secret → ConfigMap → MySQL → frontend → api → Ingress → curl → 清理',
    section: '6-9：整合示範',
    duration: '15',
    content: (
      <div className="space-y-3">
        <div className="bg-slate-800/50 p-3 rounded-lg">
          <p className="text-cyan-400 font-semibold text-sm mb-2">Step 1-3：基礎設定</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-0.5">
            <p className="text-slate-500"># 1. 建 Namespace</p>
            <p>kubectl create namespace my-app</p>
            <p className="text-slate-500 mt-1"># 2. 建 Secret（MySQL root 密碼）</p>
            <p>kubectl create secret generic mysql-secret \</p>
            <p>{'  '}--from-literal=root-password=rootpass123 -n my-app</p>
            <p className="text-slate-500 mt-1"># 3. 建 ConfigMap</p>
            <p>kubectl create configmap app-config \</p>
            <p>{'  '}--from-literal=MESSAGE="Hello from frontend" \</p>
            <p>{'  '}--from-literal=USERNAME=demo -n my-app</p>
          </div>
        </div>

        <div className="bg-slate-800/50 p-3 rounded-lg">
          <p className="text-cyan-400 font-semibold text-sm mb-2">Step 4-7：部署服務</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-0.5">
            <p className="text-slate-500"># 4-7. apply 所有 YAML</p>
            <p>kubectl apply -f mysql-deploy.yaml -n my-app</p>
            <p>kubectl apply -f frontend-deploy.yaml -n my-app</p>
            <p>kubectl apply -f api-deploy.yaml -n my-app</p>
            <p>kubectl apply -f app-ingress.yaml -n my-app</p>
          </div>
        </div>

        <div className="bg-slate-800/50 p-3 rounded-lg">
          <p className="text-cyan-400 font-semibold text-sm mb-2">Step 8：驗收</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-0.5">
            <p>kubectl get all -n my-app</p>
            <p>curl http://{'<NODE-IP>'}/frontend  <span className="text-green-400"># → Hello from frontend</span></p>
            <p>curl http://{'<NODE-IP>'}/api       <span className="text-green-400"># → Hello from api</span></p>
          </div>
        </div>
      </div>
    ),
...
# Step 9：清理（刪整個 namespace，一行搞定）
kubectl delete namespace my-app
kubectl get all -n my-app   # → No resources found`,
    notes: `好，我來示範九步驟。

Step 1：建 Namespace。kubectl create namespace my-app。

Step 2：建 Secret。kubectl create secret generic mysql-secret，--from-literal=root-password=rootpass123，-n my-app。注意每個指令都要加 -n my-app，告訴 K8s 這個資源要建在哪個 namespace。

Step 3：建 ConfigMap。三個 key，MESSAGE 是前端的訊息，USERNAME 是 demo。

Step 4 到 7：一次 apply 四個 YAML。mysql-deploy.yaml 包含 MySQL Deployment 和 ClusterIP Service。frontend-deploy.yaml 的 Deployment 裡面用 envFrom 注入了 app-config 和 mysql-secret。api-deploy.yaml 是另一個服務。app-ingress.yaml 是 path-based routing，/frontend 導前端，/api 導 API。

Step 8 驗收。kubectl get all -n my-app，確認三個 Pod 都在 Running。curl /frontend 看到 Hello from frontend，curl /api 看到 Hello from api。

Step 9 清理。kubectl delete namespace my-app，一行指令，namespace 裡面所有的 Deployment、Service、ConfigMap、Secret、Ingress 全部消失。這就是用 Namespace 管理資源的最大好處。
...
[▶ 下一頁 — 換你了]`,
  ...
  // ── 6-9（2/2）：學員實作 ──
  {
    ...
    subtitle: '自己的 namespace、自己的訊息、自己的密碼',
    section: '6-9：整合示範',
    duration: '15',
    content: (
      <div className="space-y-4">
        <div className="bg-green-900/30 border border-green-500/30 p-4 rounded-lg">
          ...
          <ol className="text-slate-300 text-sm space-y-1.5 list-decimal list-inside">
            <li>建 namespace <code className="text-cyan-400">my-app</code></li>
            <li>建 Secret，密碼換成你自己的</li>
            <li>建 ConfigMap，MESSAGE 換成你的名字</li>
            <li>apply mysql-deploy.yaml + frontend-deploy.yaml + api-deploy.yaml + app-ingress.yaml（都加 <code className="text-cyan-400">-n my-app</code>）</li>
            <li>curl /frontend 看到自己的 Username；curl /api 看到 Hello from api</li>
            <li>刪整個 namespace</li>
          </ol>
        </div>
        <div className="bg-yellow-900/30 border border-yellow-500/30 p-4 rounded-lg">
          <p className="text-yellow-400 font-semibold mb-2">挑戰題</p>
          <ol className="text-slate-300 text-sm space-y-1 list-decimal list-inside">
            <li>不用 namespace，把三個服務直接建在 default namespace</li>
            <li>改 Ingress 加 host-based routing：<code className="text-cyan-400">myapp.local</code> → frontend-svc，<code className="text-cyan-400">api.myapp.local</code> → api-svc</li>
            <li>修改 /etc/hosts，curl 兩個域名驗證</li>
          </ol>
        </div>
      </div>
    ),
...
kubectl get all -n my-app   # 確認三個 Pod Running
...
必做：建自己的 namespace，Secret 密碼換成自己的，ConfigMap MESSAGE 換成自己的名字，apply 四個 YAML，curl 兩個端點驗收，最後刪整個 namespace。

挑戰：改用 host-based routing，/etc/hosts 加兩條，curl 域名驗收。
...
[▶ 下一頁 — 對答案 + 上午總結]`,
  ...
  {
    title: '上午總結：三個 Loop 因果鏈',
    subtitle: 'NodePort → Ingress → ConfigMap/Secret → 整合 → 下午繼續',
    section: '6-10：上午總結',
    duration: '5',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-3">三個 Loop 因果鏈</p>
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              <span className="bg-blue-900/50 text-blue-300 text-xs px-2 py-0.5 rounded mt-0.5 shrink-0">Loop 1</span>
              <p className="text-slate-300 text-sm">NodePort 醜且難記 → <strong className="text-cyan-400">Ingress</strong>：一個 IP，path/host 路由，80/443 標準 Port</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="bg-green-900/50 text-green-300 text-xs px-2 py-0.5 rounded mt-0.5 shrink-0">Loop 2</span>
              <p className="text-slate-300 text-sm">設定寫死在 YAML → <strong className="text-cyan-400">ConfigMap + Secret</strong>：設定和密碼分離，env 注入 vs Volume 掛載</p>
            </div>
            <div className="flex items-start gap-2">
              <span className="bg-purple-900/50 text-purple-300 text-xs px-2 py-0.5 rounded mt-0.5 shrink-0">整合</span>
              <p className="text-slate-300 text-sm">Namespace 隔離環境 → 三個服務合在一起 → curl 驗收 → 一行清理</p>
            </div>
          </div>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">學了什麼 vs 還沒學什麼</p>
          <table className="w-full text-sm">
            <tbody className="text-slate-300">
              <tr className="border-b border-slate-700">
                <td className="py-1.5 pr-4 text-green-400">✅ 路由</td>
                <td className="py-1.5">Ingress Path-based + Host-based</td>
              </tr>
              <tr className="border-b border-slate-700">
                <td className="py-1.5 pr-4 text-green-400">✅ 設定管理</td>
                <td className="py-1.5">ConfigMap env 注入 + Volume 掛載、Secret</td>
              </tr>
              <tr className="border-b border-slate-700">
                <td className="py-1.5 pr-4 text-red-400">❌ 資料持久化</td>
                <td className="py-1.5">Pod 死掉 MySQL 資料全消失 → 下午 PV/PVC</td>
              </tr>
              <tr>
                <td className="py-1.5 pr-4 text-red-400">❌ 有狀態服務</td>
                <td className="py-1.5">MySQL 重啟名字不固定 → 下午 StatefulSet</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-red-900/30 border border-red-500/30 p-3 rounded-lg">
          <p className="text-red-400 text-sm font-semibold">下午解決：Pod 死掉，資料怎麼辦？</p>
        </div>
      </div>
    ),
    code: `# 上午總結：驗收清單
kubectl get all                    # 確認乾淨（只剩 kubernetes Service）
kubectl get configmap              # 確認 ConfigMap 清乾淨
kubectl get secret                 # 確認 Secret 清乾淨
kubectl get ingress                # 確認 Ingress 清乾淨
...
# 資料全消失！← 下午 PV/PVC 解決這個問題`,
    notes: `好，上午的內容到這裡。我們來回顧一下今天做了什麼。

三個 Loop。

第一個 Loop，Ingress。NodePort 的問題是 Port 醜、有限、沒域名、沒 HTTPS，每個服務一個 Port。Ingress 是 K8s 世界裡的 Nginx 反向代理，一個 IP，標準的 80 Port，用路徑或域名區分不同服務。

第二個 Loop，ConfigMap 和 Secret。設定寫死在 YAML，推到 Git 就洩漏了，換環境要改很多地方。ConfigMap 存不敏感的設定，Secret 存敏感的密碼，兩種注入方式的差異：env 注入改了要 rollout restart，Volume 掛載 30 秒自動同步。

然後我們把三個合在一起，在 my-app 這個 namespace 裡面跑了完整的系統。最後刪 namespace，一行全清。

那我問大家一個問題。我們剛才跑的 MySQL，裡面存了一些資料。如果這個 Pod 掛掉了，K8s 把它重啟，或者你 kubectl delete pod，資料會怎樣？

答案是：全消失。Pod 是無狀態的，裡面的檔案系統是臨時的，Pod 一死資料就清空。這對無狀態的服務沒問題，但 MySQL 是有狀態的服務，資料消失就完蛋了。

下午我們解決這個問題。PV 和 PVC 讓資料可以持久化，StatefulSet 讓有狀態服務有固定的名字和獨立的儲存，Helm 讓複雜的部署變成一行指令。

休息一下，下午繼續。

[▶ 下午內容見 lesson6-afternoon]`,
  },

  // （6-11 以後為下午內容，見 lesson6-afternoon/index.tsx）
]
