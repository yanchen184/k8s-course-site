...
這叫**橫向移動**（Lateral Movement），是資安攻擊裡最常見的手法之一。意思是攻擊者入侵了一台機器之後，利用內網裡機器之間的信任關係，從一台跳到另一台，一路打到高價值目標。在 K8s 裡就是：一個 Pod 被攻破了，因為預設所有 Pod 都能互連，攻擊者就能從這個 Pod 橫向移動到資料庫、快取、甚至其他 Namespace 的服務。
...
這邊的 Layer 是 OSI 網路模型的分層。Layer 3 是 IP 層，管的是「誰跟誰連」；Layer 4 是 Transport 層，管的是 TCP/UDP port；Layer 7 是 Application 層，看的是 HTTP 路徑、Host header 這些應用層資訊。NetworkPolicy 只能控制到 Layer 3/4（IP + Port），沒辦法看 HTTP 路徑；要做到 Layer 7 的控制，需要用 Service Mesh（例如 Istio）。
