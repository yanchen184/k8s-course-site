...
K8s 的世界不一樣。K8s 管的不是一台機器，是一群機器。這群機器在 K8s 裡面叫做叢集，cluster。叢集裡面的每一台機器叫做 Node，節點。
...
其實一個 Pod 中是可以運行多個容器的，但是一般也僅限於這些容器是高度耦合的情況，它們之間為了實現某種功能或者共享某些資源而不得不放到同一個 Pod 中。比如大家經常聯到的 Sidecar，也就是邊車模式，就是將一個應用容器和一個輔助容器放到同一個 Pod 中。這個輔助容器就是 Sidecar，通常用來實現一些輔助的功能，比如日誌收集、監控或者配置管理等等。
...
[▶ 下一頁]`,
  ...
  // ── 4-2（2/3）：Pod IP 會變（10.244.0.5→10.244.0.8）→ Service + 三種類型 ──
  {
    ...
    notes: `現在我們已經將應用程式和資料庫分別放到兩個 Pod 中。應用程式要存取資料庫的話，就需要知道資料庫的 IP 位址。這個 IP 位址是在 Pod 建立的時候自動分配的，它是一個叢集內部的 IP 位址，Pod 之間可以通過這個 IP 位址來進行叢集通訊。這樣我們的應用程式就可以通過這個 IP 位址來存取資料庫了。
...
舉個具體例子。K8s 給後端 Pod 分配了 10.244.0.5 這個 IP，前端 Pod 就寫 10.244.0.5 去連。跑了兩天，後端掛了，K8s 重建了一個新的 Pod，新 IP 變成 10.244.0.8。前端還在傻傻地連 10.244.0.5，連不上了，使用者看到一片空白。

為了解決這個問題，Kubernetes 提供了一個叫做 Service 的資源物件。Service 翻譯過來也就是服務，它可以將一組 Pod 封裝成一個服務，這個服務可以通過一個統一的入口來存取。拿剛剛的例子來說，我們可以將應用程式和資料庫的 Pod 分別封裝成兩個 Service。這樣的話，應用程式就可以通過 Service 的 IP 位址來存取資料庫了。這樣當 Pod 發生故障的時候，即使它的 IP 位址發生了變化，但是 Service 的 IP 位址是不會發生變化的。Service 會自動將請求轉發到其他健康的 Pod 上，這樣就解決了 Pod 的 IP 位址不穩定的問題。就像公司的總機號碼，不管接線員換了幾個人，總機號碼永遠不變。

服務也分為內部服務和外部服務。內部服務是一些我們不想暴露給外部或者沒有必要暴露給外部的服務，比如 MongoDB 資料庫就是一個例子，其他類似的還有快取、訊息佇列等等。這些服務只需要在叢集內部被存取就可以了，這種類型的服務我們稱之為內部服務，在 K8s 裡對應的就是 ClusterIP 類型。K8s 內建了 CoreDNS 服務，你的 API 直接用 Service 的名字，像 mysql-service，DNS 自動解析成正確的 IP。跟 Docker Compose 用服務名稱互連是一樣的概念，只是 K8s 可以跨 Node 運作。

相應地，有些服務是需要暴露給外部的，比如一些微服務的後端 API 介面，或者是一些給使用者使用的前端介面等等。這種類型的服務我們稱之為外部服務。外部服務有幾種常用的類型，其中一種類型就是 NodePort，它會在節點上開放一個 Port（30000-32767），然後將這個 Port 映射到 Service 的 IP 位址和 Port 上。這樣我們就可以通過節點的 IP 位址和 Port 來存取 Service 了。這就像 Docker 的 -p 8080:80，差別在於 NodePort 在叢集裡每個 Node 上都開同一個 Port。

還有一種是 LoadBalancer，向雲端服務商申請一個負載均衡器，自動分配外部 IP。AWS、GCP 那些雲環境比較常用，之後再細講。

OK，叢集內部的 Pod 互相找得到了，外部也可以通過 NodePort 連進來了。但是你想想，你會讓使用者用 http://192.168.1.100:30080 來存取你的網站嗎？又長又醜又難記。使用者要的是域名，www.myshop.com。
...
[▶ 下一頁]`,
  ...
  // ── 4-2（3/3）：192.168.1.100:30080 太醜 → Ingress + 流量路徑圖 ──
  {
    ...
    notes: `在開發或者測試階段，使用這種 IP 和 Port 的方式是沒有問題的，但是在生產環境中通常都是通過域名來存取服務的。這個時候就用到了另一個資源物件 — Ingress。

Ingress 是用來管理從叢集外部存取叢集內部服務的入口和方式的，可以通過 Ingress 來配置不同的轉發規則，從而根據不同的規則來存取叢集內部不同的 Service，以及 Service 所對應的後端 Pod。還可以通過 Ingress 來配置域名，這樣就可以將原本使用 IP 位址和 Port 的方式轉換成使用域名的方式來存取 Service 了。另外 Ingress 也可以配置一些其他的功能，比如負載均衡、SSL 憑證等等，這些功能我們會在後面的課程中詳細介紹。

舉個具體例子。你的電商網站有前端、API、管理後台三個 Service。在 Ingress 裡設定：www.myshop.com 轉到前端，www.myshop.com/api 轉到後端，admin.myshop.com 轉到管理後台。一個域名底下不同路徑導到不同服務，這在實際專案非常常見。
...
[▶ 下一頁]`,
  ...
  // ── 4-3（1/2）：設定寫死→ConfigMap / 密碼明文→Secret（+Base64 警告）──
  {
    ...
    notes: `現在我們的應用程式和資料庫已經可以在 Kubernetes 中運行並且對外提供服務了，但是這個時候我們還沒有考慮到一個問題，就是應用程式和資料庫之間的耦合問題。比如我們的應用程式需要存取資料庫的話，通常的做法是把資料庫的位址和 Port 等連線資訊寫到配置檔或者環境變數中，然後在應用程式中讀取這些配置資訊。但是這樣的話，配置資訊就和應用程式耦合在一起了。一旦資料庫的位址或者 Port 發生了變化，那麼我們就需要重新編譯應用程式，然後再重新部署到叢集中。這樣不但非常麻煩，而且還會造成應用程式的停機時間。這對於一些需要 7x24 小時不間斷運行的系統或者服務來說，顯然是不能接受的。

具體一點說，開發環境的資料庫是 dev-db:3306，測試環境是 test-db:3306，上線是 prod-db:3306。三個環境，三個不同的地址。設定寫死在 Image 裡面，每次換環境就要改設定、重新 build Image、重新部署。更要命的是，上線的 Image 跟測試的 Image 不一樣了，你怎麼確保上線的就是你測過的那個版本？這會帶來很大的風險。
...
ConfigMap 有兩種使用方式。第一種，環境變數注入。Pod 啟動的時候，K8s 把 ConfigMap 裡的值設成環境變數，程式直接讀環境變數。跟 Docker 的 -e 一樣。簡單直覺，適合少量設定。但有一個限制：後來改了 ConfigMap，正在跑的 Pod 不會自動更新，你要重啟 Pod 才能拿到新值。第二種，Volume 掛載。K8s 把 ConfigMap 的內容變成檔案掛到 Pod 裡。好處是 ConfigMap 改了之後，掛載的檔案會自動更新，不用重啟 Pod。適合複雜的設定檔，比如 nginx.conf。ConfigMap 的大小上限是 1 MB。

有一點需要注意的是，ConfigMap 中的配置資訊都是明文的。如果配置資訊中包含一些敏感資訊，比如資料庫的使用者名稱或者密碼等等，那麼這些敏感資訊就不建議儲存在 ConfigMap 中，因為這樣會帶來一些安全風險和問題。你的叢集裡有十個開發者都有權限看 ConfigMap，那他們都能看到資料庫密碼、API 金鑰、SSL 憑證。這不行。

為了解決這個問題，Kubernetes 提供了另一個叫做 Secret 的組件。和 ConfigMap 類似，它可以將一些敏感資訊封裝起來，然後就可以在應用程式中讀取和使用了。需要注意的是，這些敏感資訊雖然不是以明文的形式來儲存的，但是也只是做了一層 Base64 的編碼而已。Base64 編碼只是一種編碼方式，並不是一種加密方式，所以僅僅只靠 Secret 並不能完全保證敏感資訊的安全。你拿到一個 Base64 字串，終端機裡執行一個 base64 decode 指令，一秒鐘就解回原文了。千萬不要以為放進 Secret 就安全了。

那 Secret 的意義在哪裡？意義在於 K8s 把 Secret 和 ConfigMap 分成兩種不同的資源類型。這樣你就可以用 RBAC 來分別控制權限。你可以設定規則：一般開發者只能看 ConfigMap，不能看 Secret，只有運維人員才能存取 Secret。真正的安全還需要配合一些其他的手段來提高安全性，比如 Kubernetes 提供的一些其他安全機制，包括網路安全、存取控制、身份認證等等。第七堂課會講 RBAC 的實際操作。

補充一個進階的東西，叫 Sealed Secret。如果你想把 Secret 的 YAML 放進 Git 做版本控制，直接 commit 的話，Base64 一解碼密碼就曝光了。Sealed Secret 用 RSA 非對稱加密，公鑰加密、私鑰解密，私鑰只存在叢集裡，外面拿到也解不開。GitOps 團隊必備。課程不深入，知道有就好。
...
所以生產環境更常用遠端儲存。這是 K8s 的 Volume 比 Docker 強大的地方。Docker 的 volume 基本上就存在本機磁碟。K8s 的 Volume 可以掛到各種地方：NFS 網路儲存、AWS 的 EBS、GCP 的 Persistent Disk、分散式儲存 Ceph。好處是 Pod 被調度到不同 Node，還是可以掛載到同一個遠端儲存，資料不會丟。比如 MySQL Pod 本來在 Node 1，資料存在 AWS EBS 上。Node 1 掛了，K8s 把 MySQL Pod 調度到 Node 2，Node 2 的新 Pod 可以重新掛載同一個 EBS，資料完全沒丟。
...
[▶ 下一頁]`,
  ...
  // ── 4-4（1/3）：單點故障 → Deployment + 三層關係 ──
  {
    ...
    notes: `到目前為止，我們的程式已經可以在 Kubernetes 中運行並且對外提供服務了，但是這個時候我們還沒有考慮到一個問題，那就是應用程式的高可用性。比如如果應用程式所在的節點發生了故障，或者需要對節點進行升級和更新維護的時候，應用程式就會停止服務，這樣就有可能造成一些不必要的麻煩和損失。
...
副本控制就是指可以定義和管理應用程式的副本數量。比如我們可以定義一個應用程式的副本數量為 3 個，那麼如果有副本發生故障的時候，它會自動建立一個新的副本來替代它，始終保持有 3 個副本在叢集中運行。不管白天半夜，不管你有沒有在看，它自動維護。而且這 3 個副本會被分散到不同的 Node，一整台 Node 掛了，其他 Node 上的副本繼續服務。

滾動更新就是指可以定義和管理應用程式的更新策略，可以輕鬆地升級應用程式的版本，逐漸使用新的版本替換掉舊的版本，確保應用程式的平滑升級。
...
除了副本管理，滾動更新也是 Deployment 的殺手級功能。
...
[▶ 下一頁]`,
  ...
  // ── 4-4（3/3）：StatefulSet + 八概念因果鏈總結圖 ──
  {
    ...
    notes: `除了應用程式之外，資料庫也會存在類似的問題。比如資料庫的節點發生了故障，或者需要升級和更新維護的時候，也會停止服務影響使用者的正常使用。所以資料庫一般也是需要採取類似的多副本的方式來保證它的高可用性的。但是和應用程式不同的是，我們一般不使用 Deployment 來實現資料庫的多副本，因為資料庫的多副本之間是有狀態的。簡單來說就是每個副本都有自己的狀態，也就是資料。這些資料都是需要持久化儲存的，而且也需要保證多個副本之間的資料是一致的，這就需要一些額外的機制。

舉個具體例子。三個 API 的 Pod 是一模一樣的，請求隨便導到哪個都行。但三個 MySQL 的 Pod 完全不一樣。MySQL 主從架構裡，mysql-0 是主節點負責寫入，mysql-1 和 mysql-2 是從節點只負責讀取。名字不能亂、順序不能錯、資料不能混。
...
具體來說有三個不同。第一，穩定的網路標識。Deployment 建的 Pod 名稱是隨機的，像 nginx-abc123。StatefulSet 建的名稱是有序且穩定的：mysql-0、mysql-1、mysql-2。重啟之後名稱不變。第二，有序的部署和刪除。先建 mysql-0，確保跑起來才建 mysql-1，像搭積木。第三，每個 Pod 有自己獨立的儲存，不會搞混。

StatefulSet 並不是萬能的，部署的過程還是比較複雜和繁瑣的，而且並不是所有的有狀態應用都適合使用 StatefulSet 來部署。一種更加通用和簡單的方式是把資料庫這種有狀態的應用程式從 Kubernetes 叢集中剝離出來，在叢集外單獨部署。用雲端的託管服務，像 AWS 的 RDS、GCP 的 Cloud SQL。K8s 裡只跑無狀態的應用。這是一個非常常見的最佳實踐，特別是剛開始導入 K8s 的團隊。

怎麼判斷用 Deployment 還是 StatefulSet？問自己三個問題。第一，需要持久化資料嗎？不需要就用 Deployment。第二，每個副本資料獨立嗎？共享就用 Deployment 加外部 DB。第三，需要穩定身份和順序？是就用 StatefulSet。簡單說，大部分用 Deployment，只有 DB 類才考慮 StatefulSet。

順帶提一個 DaemonSet。Deployment 是「我要 N 個 Pod」，DaemonSet 是「每台 Node 都要一個 Pod」。適合監控 agent、日誌收集器。第五堂詳教。
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="text-slate-300">
              <tr className="border-t border-slate-700">
                ...
                <td className="py-2">叢集中的一個節點，可以是實體機也可以是虛擬機</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Pod</td>
                <td className="py-2">最小調度單元，容器的抽象，一般一個 Pod 一個容器</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Service</td>
                <td className="py-2">將一組 Pod 封裝成服務，提供統一入口</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Ingress</td>
                <td className="py-2">將請求路由轉發到叢集內部的 Service</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">ConfigMap</td>
                <td className="py-2">封裝配置資訊，避免重新編譯和部署</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Secret</td>
                <td className="py-2">封裝敏感資訊，搭配 RBAC 控制權限</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Volume</td>
                <td className="py-2">掛載到本地磁碟或遠端儲存，實現資料持久化</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">Deployment</td>
                <td className="py-2">部署無狀態應用，副本控制 + 滾動更新</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-blue-400 font-semibold">StatefulSet</td>
                <td className="py-2">部署有狀態應用，穩定身份 + 有序部署 + 獨立儲存</td>
              </tr>
            </tbody>
          </table>
        </div>
...
這八個概念，每一個都是因為前一步沒解決的問題才出現的。這條因果鏈就是 K8s 的核心。後面幾堂課我們會一個一個深入，實際寫 YAML、部署、操作。
...
[▶ 下一頁]`,
  ...
  // ── 4-5（2/2）：Worker 三組件 ──
  {
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">1. Container Runtime（containerd）— 工具</p>
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            <li>負責：拉 Image、建容器、啟動/停止容器</li>
            <li>Docker 分兩層：上層 CLI + 下層 <strong className="text-white">containerd</strong></li>
            <li>K8s 1.24+ 直接用 containerd，不需要裝 Docker</li>
            <li>Docker build 的 Image 在 containerd <strong className="text-green-400">照樣能跑</strong>（OCI 標準）</li>
          </ul>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">2. kubelet — 工頭</p>
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            <li><strong className="text-white">接收指令</strong>：Master 說「跑一個 nginx Pod」→ kubelet 叫 containerd 建容器</li>
            <li><strong className="text-white">持續監控</strong>：盯著這台 Node 上所有 Pod 的狀態（跑著/掛了/CPU/記憶體）</li>
            <li><strong className="text-white">定期回報</strong>：把狀態回報給 Master。kubelet 不回報 → Master 懷疑這台 Node 掛了</li>
          </ul>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">3. kube-proxy — 交通指揮</p>
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            <li>請求進來 → 轉發到正確的 Pod</li>
            <li>Service 後面有 3 個 Pod → kube-proxy 幫你負載均衡</li>
            <li>用 <strong className="text-white">DaemonSet</strong> 部署：每台 Node 都有一個 kube-proxy</li>
          </ul>
        </div>

        <div className="bg-amber-900/30 border border-amber-500/40 p-3 rounded-lg">
          <p className="text-amber-400 font-semibold mb-1">缺一不可</p>
          <p className="text-slate-300 text-xs">沒 Runtime → 容器跑不了。沒 kubelet → 指令傳不到。沒 kube-proxy → 流量到不了正確的 Pod。</p>
        </div>
      </div>
    ),
...
[▶ 下一頁]`,
  ...
  // ── 4-6（1/3）：Master 四組件 ──
  {
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-green-900/30 border border-green-500/30 p-3 rounded-lg">
          <p className="text-green-400 font-semibold text-sm mb-1">用一行指令認識四個組件</p>
          <code className="text-cyan-400 text-sm">kubectl create deployment nginx --image=nginx --replicas=3</code>
          <p className="text-slate-400 text-xs mt-1">這行指令發出去後，會依序經過下面四個組件 ↓</p>
        </div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-400 border-b border-slate-600">
                <th className="pb-2 pr-4">順序</th>
                <th className="pb-2 pr-4">組件</th>
                <th className="pb-2 pr-4">比喻</th>
                <th className="pb-2">這行指令到它這裡發生什麼</th>
              </tr>
            </thead>
            <tbody className="text-slate-300">
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-cyan-400 font-bold">①</td>
                <td className="py-2 pr-4 text-blue-400 font-semibold">API Server</td>
                <td className="py-2 pr-4">大門警衛</td>
                <td className="py-2">收指令 → 驗證身份和權限 → 通過</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-cyan-400 font-bold">②</td>
                <td className="py-2 pr-4 text-blue-400 font-semibold">etcd</td>
                <td className="py-2 pr-4">檔案室</td>
                <td className="py-2">記下「要 3 個 nginx Pod」→ 叢集的記憶</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-cyan-400 font-bold">③</td>
                <td className="py-2 pr-4 text-blue-400 font-semibold">Controller Manager</td>
                <td className="py-2 pr-4">24h 監工</td>
                <td className="py-2">發現期望 3 個但實際 0 個 → 觸發建 Pod</td>
              </tr>
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-cyan-400 font-bold">④</td>
                <td className="py-2 pr-4 text-blue-400 font-semibold">Scheduler</td>
                <td className="py-2 pr-4">HR</td>
                <td className="py-2">看哪台 Node 空閒 → 分配 Pod 到合適的 Node</td>
              </tr>
            </tbody>
          </table>
        </div>
...
假設你坐在電腦前面，在終端機裡敲了一行指令：kubectl create deployment nginx --image=nginx --replicas=3。意思是「我要三個 nginx 的 Pod」。好，你按了 Enter。這個指令發出去了，第一個問題：誰收的？
...
[▶ 下一頁]`,
  ...
  // ── 4-6（2/3）：完整流程 ──
  {
    ...
    subtitle: 'kubectl create deployment nginx --image=nginx --replicas=3',
    section: '4-6：K8s 架構（下）Master Node',
    duration: '5',
    content: (
      <div className="space-y-4">
...
我們來把完整流程從頭走一遍。你在終端機輸入 kubectl create deployment nginx --image=nginx --replicas=3，按 Enter。
...
為什麼需要兩個減號？因為 kubectl 需要一個分隔符號來區分「給 kubectl 的參數」和「要在容器裡執行的指令」。沒有這個分隔符號的話，kubectl 可能會把 /bin/sh 當成自己的參數來處理。Docker 的 exec 不需要這個雙減號，是因為 Docker 的參數解析方式不同。K8s 多了這一步，但背後的設計其實更嚴謹。這是很容易忘記的一個點，待會常見踩坑我還會再提醒一次。
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <table className="w-full text-sm">
            ...
            <tbody className="text-slate-300">
              ...
              <tr className="border-t border-slate-700">
                <td className="py-2 pr-4 text-cyan-400 font-bold">Loop 5</td>
                <td className="py-2">Deployment 入門（三層關係 + 自我修復）</td>
              </tr>
            </tbody>
          </table>
        </div>
