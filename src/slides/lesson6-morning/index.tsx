...
[▶ 下一頁]`,
  ...
  // ============================================================
  {
    ...
    notes: `這節先做一個實驗。MySQL 跑起來，進去建 testdb 寫一筆資料，然後把 Pod 砍掉，等新 Pod 起來再進去看。你猜資料還在嗎？不在了。原因是容器有自己的 overlay filesystem，寫入的資料在最上層的 writable layer，容器刪了那層就不見了。解法是 PV 和 PVC。PV 是管理員規劃的儲存空間，可以是本機硬碟、NFS、雲端硬碟。PVC 是開發者的申請書，說我要多大、什麼 AccessMode，K8s 自動找符合的 PV 配對，這叫 Binding。AccessMode 三種：RWO 只有一個 Node 可以讀寫，ROX 多個 Node 唯讀，RWX 多個 Node 可以讀寫（需要 NFS）。ReclaimPolicy：Retain 是 PVC 刪了 PV 資料保留，生產用；Delete 是 PVC 刪了 PV 一起刪，開發環境用。好，開始做。[▶ 下一頁]`,
  },
