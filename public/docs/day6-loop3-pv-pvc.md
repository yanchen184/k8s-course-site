...
> 📢 **講師引導**：這份 YAML 有三個設定錯誤，導致 PVC 無法和 PV 綁定。管理員已經建好 PV（不要動它），你的任務是找出 PVC 裡的三個 bug，修好後讓兩個都是 `Bound`。
>
> Hint：仔細比對 PV 和 PVC 的 `accessModes`、`storageClassName`、`storage` 容量三個欄位。
...
local-pv 已經被 local-pvc 綁定了。現在再建一個 `local-pvc2`（storageClassName: manual，requests: 1Gi），觀察 STATUS，說說看為什麼是那個狀態？

```bash
kubectl get pvc
# 觀察 local-pvc2 的 STATUS 是什麼
```
