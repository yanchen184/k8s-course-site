...
> # 打開瀏覽器訪問 http://localhost:8080，應該看到自訂的頁面
> # 清理：docker rm -f my-web-container
> ```
...
> 建構與驗證：
> ```bash
> # 第一次：不帶 --build-arg（BUILD_ENV 使用預設值 production）
...
> # 可以看到 APP_VERSION=1.0.0（ENV 定義的，容器內看得到）
> # 看不到 BUILD_ENV（ARG 定義的，建構完就消失了）
>
> docker run --rm env-test:v1 sh -c "echo BUILD_ENV=$BUILD_ENV APP_VERSION=$APP_VERSION"
> # 輸出：BUILD_ENV= APP_VERSION=1.0.0
> # BUILD_ENV 是空的，因為 ARG 在容器執行時已經不存在了
> ```
>
> 結論：**ENV 在容器裡看得到，ARG 看不到。** ARG 只存在於建構過程中（貼紙），ENV 會一直留到容器執行時（刺青）。
...
> # 輸出：Hello Docker
>
> # 測試二：帶參數 "I am a student"
> docker run --rm demo-cmd "I am a student"
> # 輸出：docker: Error（或執行 "I am a student" 這個指令）
> # → CMD 被完全覆蓋，"I am a student" 變成要執行的指令，而不是 echo 的參數
>
> docker run --rm demo-ep "I am a student"
> # 輸出：Hello Docker I am a student
> # → ENTRYPOINT 保留，"I am a student" 被附加在後面
> ```
>
