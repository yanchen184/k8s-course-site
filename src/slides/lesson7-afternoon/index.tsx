      ...
      <div className="space-y-4">
        <div className="bg-green-900/30 border border-green-500/50 p-4 rounded-lg">
          ...
          <div className="bg-slate-900 p-2 rounded font-mono text-xs text-slate-300">
            ...
            <p className="text-green-400">NAME　　　　　READY　STATUS　　RESTARTS</p>
            <p className="text-green-400">nginx-xxx　　1/1　　Running　　0</p>
          </div>
        </div>
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="bg-slate-800/50 p-3 rounded-lg">
          ...
          <p className="text-sm text-slate-300">一旦套了 NetworkPolicy → 預設<span className="text-red-400 font-bold">全部拒絕</span> → 只有明確允許的才能通過（白名單邏輯）</p>
        </div>

        <div className="bg-amber-900/30 border border-amber-500/50 p-3 rounded-lg text-sm">
          <p className="text-amber-400 font-semibold">CNI 支援注意</p>
          <div className="text-xs text-slate-300 mt-1 space-y-1">
            <p>✗ Flannel（k3s 預設）、kindnet（minikube 預設）→ apply 不報錯但不生效</p>
            <p>✓ <span className="text-green-400">Calico</span>、<span className="text-green-400">Cilium</span>、Weave → 生產環境用這些</p>
          </div>
        </div>
      </div>
    ),
...
這裡有一個非常重要的觀念。一旦你在某個 Pod 上套了 NetworkPolicy，預設行為就變成「全部拒絕」。只有你在 ingress 或 egress 裡面明確允許的流量才能通過。沒有被明確允許的，一律擋掉。這跟傳統防火牆的白名單邏輯一模一樣。

最後提一個很重要的實務注意事項。NetworkPolicy 這個東西是 K8s 的 API 規格，但實際執行是靠 CNI 插件來做的。不是所有 CNI 都支援 NetworkPolicy。k3s 預設使用的 Flannel 不支援。minikube 預設使用的 kindnet 也不支援。你 apply 了 NetworkPolicy 的 YAML，K8s 不會報錯，但它也不會生效。流量照樣全通。

支援 NetworkPolicy 的 CNI 有 Calico、Cilium、Weave 這幾個。生產環境通常會用 Calico 或 Cilium。如果你想在 minikube 上測試，需要用 minikube start --cni=calico 重新建叢集。在我們的課堂環境上，我們主要是理解概念和 YAML 怎麼寫，如果 CNI 不支援，你 apply 之後用指令確認 NetworkPolicy 有建好就行，理解它要做的事情。
        ...
        <div className="bg-red-900/30 border border-red-500/50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm text-slate-300">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p>CNI 不支援</p>
                <p className="text-xs text-slate-400">apply 不報錯但不生效 → 換 Calico：minikube start --cni=calico</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-red-400 font-bold shrink-0">2.</span>
