      ...
      <div className="space-y-3">
        ...
        <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-3 rounded text-xs">
          ...
          <p className="text-slate-400 mt-2">
            為什麼不直接讓 Backend 收 /api/tasks？因為 /api 這個前綴是 Ingress 路由層的決定，不應該滲透到 Backend 程式裡。Backend 只管自己的邏輯，不管外面怎麼路由，以後換路由規則不用改 Backend 程式。
          </p>
        </div>
      </div>
    ),
...
[▶ PPT 46/54：Local image fallback]`,
  },

  // ── [46/54] Local image fallback 準備 ──
  {
    title: '[46/54] Local image fallback',
    subtitle: 'Public pull 失敗時：下載 tar，匯入每台 node，再回到部署流程',
    section: '7-11 Image 準備',
    duration: '5',
    content: (
      <div className="space-y-1.5 text-[10px] leading-snug">
        <div className="bg-gradient-to-r from-amber-900/30 to-red-900/30 border border-amber-500/50 p-1.5 rounded">
          <p className="text-amber-300 font-semibold">這是 Docker Hub pull 不過時才用的 fallback：把 API、Frontend、PostgreSQL、BusyBox 匯入 control plane 和 worker。</p>
        </div>

        <div className="grid grid-cols-[1.05fr_0.95fr] gap-1.5">
          <div className="bg-slate-800/40 border border-amber-500/40 p-1.5 rounded">
            <p className="text-amber-200 font-semibold">0. 每台 VM 貼上 setup block</p>
            <p className="text-slate-400">control plane 和 worker 都要貼；看到 OK 才繼續。</p>
            <pre className="bg-slate-950 text-slate-100 p-1.5 rounded mt-1 overflow-x-auto text-[9px] leading-tight"><code>{`set -eu
USER_NAME="$(whoami)"
...
echo "OK: sudo -n k3s is ready for $USER_NAME ($K3S_PATH)"`}</code></pre>
          </div>

          <div className="space-y-1.5">
            <div className="bg-slate-800/40 border border-slate-700 p-1.5 rounded">
              <p className="text-cyan-300 font-semibold">1. Windows 下載 tar</p>
              <p className="text-slate-400">用瀏覽器開 Google Drive；檔案會在 Downloads。</p>
              <pre className="bg-slate-950 text-slate-100 p-1.5 rounded mt-1 whitespace-pre-wrap break-all text-[9px]"><code>{`drive.google.com/file/d/1LAvKkpENmTtQjvxxrivgoHDbuJWzcJH-`}</code></pre>
            </div>
            <div className="bg-slate-800/40 border border-slate-700 p-1.5 rounded">
              <p className="text-cyan-300 font-semibold">2. 在 Windows PowerShell 用 scp 傳檔</p>
              <pre className="bg-slate-950 text-slate-100 p-1.5 rounded mt-1 overflow-x-auto text-[9px] leading-tight"><code>{`ssh user@<control-plane-ip> "mkdir -p ~/Downloads"
scp "$env:USERPROFILE\\Downloads\\url-shortener-k3s-images.tar" \\
  user@<control-plane-ip>:~/Downloads/url-shortener-k3s-images.tar`}</code></pre>
              <p className="text-slate-400">右邊直接指定遠端檔名，不只寫資料夾。</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-[0.85fr_1.15fr] gap-1.5">
          <div className="bg-slate-800/40 border border-slate-700 p-1.5 rounded">
            <p className="text-cyan-300 font-semibold">3. 在 Linux VM 確認檔案</p>
            <pre className="bg-slate-950 text-slate-100 p-1.5 rounded mt-1 overflow-x-auto text-[9px]"><code>{`cd ~/k8s-course-labs/lesson7/url-shortener
sha256sum ~/Downloads/url-shortener-k3s-images.tar`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-1.5 rounded">
            <p className="text-cyan-300 font-semibold">4. 匯入並檢查每台 node</p>
            <pre className="bg-slate-950 text-slate-100 p-1.5 rounded mt-1 overflow-x-auto text-[9px] leading-tight"><code>{`IMAGE_TAR=~/Downloads/url-shortener-k3s-images.tar \\
K3S_NODES="user@<control-plane-ip> user@<worker-ip>" \\
  ./scripts/load-images-to-k3s-ssh.sh
K3S_NODES="user@<control-plane-ip> user@<worker-ip>" \\
  ./scripts/check-k3s-images-ssh.sh`}</code></pre>
            <p className="text-slate-400">把 &lt;control-plane-ip&gt; / &lt;worker-ip&gt; 換成自己的 VM IP。</p>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-1.5">
          <div className="bg-cyan-900/20 border border-cyan-500/40 p-1.5 rounded text-cyan-300 text-center">
            tar 已包含 4 個 image，不用現場 pull Docker Hub。
          </div>
          <div className="bg-amber-900/20 border border-amber-500/40 p-1.5 rounded text-amber-200 text-center">
            第一次 sudo 可能要輸入 Linux 密碼；看到 OK 才完成。
          </div>
          <div className="bg-emerald-900/20 border border-emerald-500/40 p-1.5 rounded text-emerald-300 text-center font-semibold">
            Pod 被排到哪台 node，那台 node 就要有 image。
          </div>
        </div>
      </div>
    ),
    notes: `這張要讓學生先完成部署前置作業：把 image 放進每台 k3s node。

以前上課可能遇過，全班同時從 Docker Hub 拉 image，結果被 rate limit，大家的 Pod 都卡在 ImagePullBackOff。這不是 YAML 寫錯，而是外部 registry 限流。

這堂課先讓學生知道有兩條路。public image path 先試，直接從 Docker Hub pull 講師準備好的 image。local image fallback 是當 public pull 不過、遇到 rate limit 或 ImagePullBackOff 時才用。local fallback 時，學生從雲端空間下載 url-shortener-k3s-images.tar，檢查 sha256，再透過 SSH 把 tar 傳進 VMware 裡的 control plane 和 worker node，最後用 k3s ctr images import 把 image 放進每個 k3s node 的 containerd。

下載連結放在講義裡，目前是 Google Drive。建議學生先用 Windows 瀏覽器下載，再用 Windows PowerShell 的 scp 傳到 control plane VM。scp 要在 tar 所在的機器執行，也就是 Windows。傳檔前先用 ssh 建立 ~/Downloads，然後 scp 右邊不要只寫遠端資料夾，要直接寫完整遠端檔名，例如 ~/Downloads/url-shortener-k3s-images.tar，這樣可以避開 scp: ... Is a directory 這類現場問題。Linux VM 可以連外時，也可以用 gdown 直接下載，但這需要 Python/pip 和網路，現場不一定每台 VM 都準備好。

K3S_NODES 不是 Kubernetes 指令，它只是給腳本讀的 node 清單。像 K3S_NODES="user@<control-plane-ip> user@<worker-ip>"，意思是用 user 這個帳號 SSH 到 control plane 和 worker 兩台 VM。學生要把 angle bracket 裡的內容換成自己真的能 SSH 進去的 VM IP，也要換成自己的 Linux 帳號。如果學生自己有設定 hostname 或 hosts，才可以改用 user@control-plane 這種名字。
...
[▶ PPT 47/54：手動部署流程]`,
  ...
  // ── [47/54] 手動部署流程 ──
  {
    title: '[47/54] 先選 image 路徑，再開始部署',
    subtitle: 'Public image 先試；如果 pull 不過，再切 Local image fallback',
    section: '7-12 手動部署',
    duration: '6',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-cyan-500/40 p-3 rounded">
            <p className="text-cyan-300 font-semibold">A. Public image path（先試）</p>
            <p className="text-slate-400 mt-1">用 Docker Hub image，指令最短；如果全班同時 pull 被限流，再切 B。</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`MANIFEST_DIR=k8s
# 後面 03~06 都用 k8s/*.yaml`}</code></pre>
          </div>

          <div className="bg-slate-800/40 border border-amber-500/40 p-3 rounded">
            <p className="text-amber-300 font-semibold">B. Local image path（pull 不過才用）</p>
            <p className="text-slate-400 mt-1">先完成 46/54 的 tar import；03~06 改用本地 image YAML。</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`MANIFEST_DIR=k8s-local
# 後面 03~06 都用 k8s-local/*.yaml`}</code></pre>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2">
          {[
            ['48', '先確認環境', 'node / kube-system / namespace / config'],
            ['49', '先讓資料可用', 'PostgreSQL ready，再跑 migration Job'],
            ['50', '部署產品服務', 'API ready，再讓 Frontend ready'],
            ['51', '打開入口', 'HPA / Ingress / Windows hosts'],
            ['52', '對照 YAML', '確認每份檔案到底建了什麼'],
            ['53', '產品驗收', 'Windows browser、redirect、自我修復'],
          ].map(([slide, title, desc]) => (
            <div key={slide} className="bg-slate-900/60 border border-slate-700 p-2 rounded">
              <p className="text-amber-300 font-semibold">PPT {slide}</p>
              <p className="text-cyan-300 mt-1">{title}</p>
              <p className="text-slate-400 mt-1">{desc}</p>
            </div>
          ))}
        </div>

        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded text-amber-300 text-center font-semibold">
          後面 03~06 會用 `$MANIFEST_DIR`；public pull 失敗時，只要改成 `MANIFEST_DIR=k8s-local` 重新跑該段。
        </div>
      </div>
    ),
    notes: `這張要改成兩條 image 路徑，不要只講 local。

第一條是 public image path。先讓學生用 k8s 目錄，也就是 Docker Hub 上的 public image。這條路徑指令最少，學生比較容易理解整個 deployment flow。

第二條是 local image path。當現場出現 Docker Hub rate limit、ImagePullBackOff、pull image 太慢或失敗時，才切到 local image path。切到 local path 前，要先完成 46/54 的 tar download、VM sudo setup、image import 和 check。然後把 MANIFEST_DIR 設成 k8s-local，後面 03 到 06 就用 k8s-local 裡的 YAML。

這裡要講清楚：00 namespace、01 secret、02 configmap、07 hpa、08 ingress 不分 public/local，都還是用 k8s 目錄。會切換的是 03 PostgreSQL、04 migration、05 API、06 Frontend，因為這幾份直接牽涉 imagePullPolicy 和 image 名稱。

流程分成九個檔案。每個檔案只負責一層，這樣學生比較不會迷路。
...
[▶ PPT 48/54：環境檢查與基礎資源]`,
  ...
  // ── [48/54] 環境檢查與基礎資源 ──
  {
    title: '[48/54] 先確認環境，再建立基礎資源',
    subtitle: '貼上第一段：確認環境，建立 namespace、secret、configmap',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 1：確認叢集狀態</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl get nodes
kubectl get pods -n kube-system
kubectl get storageclass`}</code></pre>
            <p className="text-slate-400 mt-2">看到 node 是 Ready，再繼續下一段。</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 2：建立產品邊界與設定</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s/00-namespace.yaml
kubectl apply -f k8s/01-secret.yaml
kubectl apply -f k8s/02-configmap.yaml
kubectl get ns url-shortener
kubectl get secret,configmap -n url-shortener`}</code></pre>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2">
          {[
            ['Namespace', '產品邊界', '後面所有資源都放在 url-shortener。'],
            ['Secret', '敏感設定', '放 postgres-password，正式環境不要提交真密碼。'],
            ['ConfigMap', '非敏感設定', '放 DB host、port、database、user。'],
          ].map(([kind, role, desc]) => (
            <div key={kind} className="bg-slate-900/60 border border-slate-700 p-3 rounded">
              <p className="text-amber-300 font-semibold">{kind}</p>
        ...
        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300 text-center">
          看到 `url-shortener` namespace、Secret、ConfigMap 都存在，才進下一張。
        </div>
      </div>
    ),
    notes: `這張補上原本投影片缺掉的前置流程。

image 已經準備好之後，不要一口氣全部 apply。先檢查環境。kubectl get nodes 看 control plane 和 worker 是否 Ready。kubectl get pods -n kube-system 看系統元件，尤其 Traefik、metrics-server、local-path provisioner 是否正常。kubectl get storageclass 看有沒有預設 StorageClass，否則 PostgreSQL PVC 會 Pending。

接著才建立三個基礎資源。Namespace 是產品邊界，Secret 放敏感設定，ConfigMap 放非敏感設定。

這一步的重點是讓學生知道，產品不是從 Deployment 開始。真正的部署流程要先有邊界、設定和敏感資料，後面的 DB、API、Frontend 才能靠這些設定啟動。

[▶ PPT 49/54：資料庫與 Migration]`,
  },

  // ── [49/54] PostgreSQL 與 Migration ──
  {
    title: '[49/54] 先讓資料層可用：PostgreSQL + Migration',
    subtitle: '貼上第二段：先等 DB ready，再跑建表 Job',
    section: '7-12 手動部署',
    duration: '6',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 3：啟動 PostgreSQL</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`: "\${MANIFEST_DIR:=k8s}"
...
kubectl get pod postgres-0 -n url-shortener`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 4：跑 Migration Job</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`: "\${MANIFEST_DIR:=k8s}"
echo "Using $MANIFEST_DIR"
...
kubectl logs job/db-migrate -n url-shortener`}</code></pre>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            <p className="text-amber-300 font-semibold">StatefulSet</p>
            <p className="text-slate-400 mt-1">讓 DB 有穩定名稱，例如 postgres-0。</p>
          </div>
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            <p className="text-amber-300 font-semibold">PVC</p>
            <p className="text-slate-400 mt-1">Pod 重建後仍掛回同一份資料。</p>
          </div>
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            <p className="text-amber-300 font-semibold">Job</p>
            <p className="text-slate-400 mt-1">用一次性任務建立資料表，不靠手動進 DB。</p>
          </div>
        </div>

        <div className="bg-emerald-900/20 border border-emerald-500/40 p-3 rounded text-emerald-300 text-center">
          看到 `job/db-migrate condition met`，且 logs 顯示 migration 完成，才進下一張。
        </div>
      </div>
    ),
    notes: `這張把資料層流程補完整。先提醒學生：47/54 選 public image path 時，MANIFEST_DIR=k8s；如果 pull image 失敗後改 local image path，MANIFEST_DIR=k8s-local。這張不要讓學生手動改檔名，照著 MANIFEST_DIR 跑。

先跑 PostgreSQL，因為 API 和 migration 都依賴 DB。public path 會用 k8s/03-postgres.yaml；local fallback 會用 k8s-local/03-postgres.yaml，讓 postgres:15 從已匯入的 node image store 啟動。

PostgreSQL 用 StatefulSet，不是 Deployment。理由是資料庫需要穩定身份和持久化儲存。postgres-0 這個名字要穩定，PVC 也要穩定。

接著跑 migration Job。migration Job 先用 busybox init container 等 postgres-service:5432 開好，再用 API image 執行 node migrate.js 建資料表。

學生要帶走的觀念是：資料庫和 migration 都是部署流程的一部分，不是手動進 DB 做。

[▶ PPT 50/54：API 與 Frontend]`,
  },

  // ── [50/54] API 與 Frontend ──
  {
    title: '[50/54] 部署產品服務：API + Frontend',
    subtitle: '貼上第三段：API ready，再讓 Frontend ready',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 5：部署 API</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`: "\${MANIFEST_DIR:=k8s}"
echo "Using $MANIFEST_DIR"
...
kubectl get pods -l app=url-api -n url-shortener`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 6：部署 Frontend</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`: "\${MANIFEST_DIR:=k8s}"
echo "Using $MANIFEST_DIR"
kubectl apply -f $MANIFEST_DIR/06-frontend.yaml
kubectl rollout status deployment/url-frontend -n url-shortener --timeout=180s
kubectl get deploy,svc -n url-shortener
kubectl get pods -l app=url-frontend -n url-shortener`}</code></pre>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            <p className="text-amber-300 font-semibold">API 要看什麼？</p>
            <ul className="text-slate-400 mt-2 space-y-1 list-disc list-inside">
              <li>ConfigMap / Secret 注入 DB 設定</li>
              <li>liveness/readiness probe</li>
              <li>resources requests，給 HPA 計算 CPU</li>
            </ul>
          </div>
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            <p className="text-amber-300 font-semibold">Frontend 要看什麼？</p>
            <ul className="text-slate-400 mt-2 space-y-1 list-disc list-inside">
              <li>靜態網站無狀態，用 Deployment</li>
              <li>Service 讓 Ingress 能導流到它</li>
              <li>public / local 只差 image 來源與 pullPolicy</li>
            </ul>
          </div>
        </div>

        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300 text-center">
          看到 API / Frontend rollout 都成功，Service 都存在，才進下一張。
        </div>
      </div>
    ),
    notes: `這張補 API 和 Frontend 的流程。延續 47/54 的選擇：public path 用 k8s/05-api.yaml 和 k8s/06-frontend.yaml；local fallback 用 k8s-local/05-api.yaml 和 k8s-local/06-frontend.yaml。投影片上的 $MANIFEST_DIR 就是在幫學生避免手動改錯路徑。

API 是短網址產品的邏輯層，負責建立短網址、查 DB、redirect。API 從 ConfigMap 拿 DB host、port、database、user，從 Secret 拿 DB password。這裡要提醒學生，前面建立的 Secret 和 ConfigMap 到這裡才真的被用到。

API local YAML 使用 url-shortener-api:lab，imagePullPolicy: Never。它也有 probes 和 resources，resources.requests.cpu 後面 HPA 會用到。

Frontend 是畫面層，也是無狀態服務，所以用 Deployment 加 Service。Ingress 的根路徑會導到 frontend service。

到這裡產品內部三層已經有了：PostgreSQL 是資料、API 是邏輯、Frontend 是畫面。下一張處理對外入口和自動擴縮。

[▶ PPT 51/54：HPA 與 Ingress]`,
  },

  // ── [51/54] HPA 與 Ingress ──
  {
    title: '[51/54] 對外入口與擴縮：HPA + Ingress',
    subtitle: '貼上第四段：建立 HPA、Ingress，讓 Windows 瀏覽器找得到 short.local',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 7：建立 HPA</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s/07-hpa.yaml
kubectl get hpa -n url-shortener
kubectl describe hpa url-api-hpa -n url-shortener`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 8：建立 Ingress</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s/08-ingress.yaml
kubectl get ingress -n url-shortener`}</code></pre>
            <p className="text-cyan-300 font-semibold mt-2">貼到 Windows PowerShell：設定 hosts</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-1 overflow-x-auto"><code>{`Add-Content -Path "C:\\Windows\\System32\\drivers\\etc\\hosts" \`
  -Value "<NODE-IP> short.local"`}</code></pre>
          </div>
        </div>

        <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
          <p className="text-cyan-300 font-semibold mb-2 text-center">Ingress path routing</p>
          <div className="flex items-center justify-center gap-2 flex-wrap">
            <span className="bg-slate-800 border border-slate-600 px-2 py-1 rounded font-mono">short.local/</span>
            <span className="text-cyan-400">→</span>
            <span className="bg-cyan-900/40 border border-cyan-500/50 px-2 py-1 rounded">Frontend Service</span>
            <span className="text-slate-500">|</span>
            <span className="bg-slate-800 border border-slate-600 px-2 py-1 rounded font-mono">/api /r /health /ready</span>
            <span className="text-cyan-400">→</span>
            <span className="bg-purple-900/40 border border-purple-500/50 px-2 py-1 rounded">API Service</span>
          </div>
        </div>

        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded text-amber-300 text-center">
          <span className="font-mono">&lt;NODE-IP&gt;</span> 要換成 Windows 連得到的 VM IP；Windows 瀏覽器要開 short.local，就要改 Windows hosts。
        </div>
      </div>
    ),
    notes: `這張補對外入口和擴縮。

HPA 掛在 API 上，因為短網址主要壓力在 API 查詢和 redirect。HPA 能不能算 CPU 百分比，取決於 API Pod 有沒有 resources.requests.cpu，以及 metrics-server 是否正常。

Ingress 是對外入口。同一個 short.local，根路徑導到 frontend，/api、/r、/health、/ready 導到 API。這裡讓學生看到，同一個 domain 可以依 path 導到不同 Service。

hosts 也要講。學生的瀏覽器要知道 short.local 指向哪個 node IP，所以要加 hosts。這次是 Windows + VMware Linux VM 的教學環境，如果學生要用 Windows 瀏覽器開 http://short.local，就必須改 Windows 的 hosts，不是只改 Linux VM 裡的 /etc/hosts。只有在 Linux VM 裡用 curl 測試時，才需要改 VM 裡的 /etc/hosts。

到這裡才算部署完整：有資料、邏輯、畫面、擴縮規則、對外入口。
...
[▶ PPT 52/54：YAML 大複習]`,
  ...
  // ── [52/54] YAML 大複習 ──
  {
    title: '[52/54] YAML 大複習：每份檔案到底做了什麼',
    subtitle: '做完後回頭對照：你剛剛貼的每段到底建立了什麼',
    section: '7-12 手動部署',
    duration: '7',
    content: (
      <div className="space-y-2 text-[11px]">
        ...
        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300 text-center">
          如果哪一步失敗，就回到對應檔案看它建立的物件；不要盲目重跑全部。
        </div>
      </div>
    ),
...
[▶ PPT 53/54：驗收清單]`,
  ...
  {
    title: '[53/54] 驗收清單',
    subtitle: '貼上第五段：先驗 Windows 入口，再驗產品功能與自我修復',
    section: '7-13 驗收',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="bg-gradient-to-br from-green-900/30 to-cyan-900/30 border border-green-500/40 p-3 rounded">
          <p className="text-green-300 font-semibold mb-2">功能驗收：Windows + VMware</p>
          <ol className="text-slate-300 space-y-1 list-decimal list-inside">
            <li>從 <span className="font-mono text-cyan-300">kubectl get ingress</span> 或 VM IP 找到 <span className="font-mono text-cyan-300">&lt;NODE-IP&gt;</span></li>
            <li>Windows hosts 加入 <span className="font-mono text-cyan-300">&lt;NODE-IP&gt; short.local</span></li>
            <li>Windows 測 <span className="font-mono text-cyan-300">ping short.local</span> / <span className="font-mono text-cyan-300">Test-NetConnection short.local -Port 80</span></li>
            <li>瀏覽器開 <span className="font-mono text-cyan-300">http://short.local</span>，建立短網址並測 redirect</li>
          </ol>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 9：K8s 驗收</p>
...
kubectl get hpa -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">貼上 10：故障驗收</p>
...
kubectl delete pod postgres-0 -n url-shortener`}</code></pre>
        ...
        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300">
          Windows 打不開時不要重 deploy；先查 hosts、VM IP、Port 80，再看 Ingress / Pod。
        </div>
...
產品驗收很清楚，但這裡要講清楚測試位置。學生如果是在 Windows 上開瀏覽器，就要先在 Windows hosts 加一行「node IP short.local」。這個 node IP 通常是 VMware Linux VM 的 control plane IP，或任何一台 Traefik 可以接到 80 port 的 node IP。只在 Linux VM 裡加 /etc/hosts，不會影響 Windows 瀏覽器。

教學流程可以這樣帶。第一步，在 Linux VM 裡用 kubectl get ingress -n url-shortener 看 ADDRESS；如果 ADDRESS 沒出來，就先用 control plane VM 的 IP。第二步，回到 Windows，用系統管理員開 PowerShell，把「NODE-IP short.local」寫進 C:\\Windows\\System32\\drivers\\etc\\hosts。第三步，在 Windows 測 ping short.local 和 Test-NetConnection short.local -Port 80。第四步，才開瀏覽器測產品。

接著在 Windows 瀏覽器打開 short.local，輸入 kubernetes.io，建立短網址，點短網址，瀏覽器真的跳過去。這代表 Windows host 可以連到 VM、Ingress、Frontend、API、DB 都串起來了。

如果學生打不開，不要直接叫他重 deploy。照順序排：short.local 有沒有解析到正確 IP、Windows 到 VM 的 80 port 通不通、kubectl get ingress 有沒有規則、frontend/api Pod 是否 Ready。這樣學生會學到，問題可能在 Windows hosts、VMware 網路、Ingress，也可能才是 Pod。
...
[▶ PPT 54/54：Helm 一個指令]`,
  ...
  // ── [54/54] Helm 一個指令 ──
  {
    title: '[54/54] 最後收尾：Helm 一個指令',
    subtitle: '貼上最後一段：先清掉手動部署，再用 Helm 重建同一套產品',
    ...
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold mb-1">1. 先清掉手動部署</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto"><code>{`kubectl delete namespace url-shortener
kubectl wait --for=delete namespace/url-shortener --timeout=180s`}</code></pre>
            <p className="text-slate-400 mt-2">清掉前面 apply 的 resources，避免 Helm 撞到同名物件。</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold mb-1">2. Helm 一鍵部署也有兩條路</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto text-[10px] leading-relaxed"><code>{`# Public image path（先試）
helm install url-shortener ./helm/url-shortener \\
  -n url-shortener \\
  --create-namespace

# Local image fallback（pull 不過才用）
helm install url-shortener ./helm/url-shortener \\
  -n url-shortener \\
  --create-namespace \\
  -f ./helm/url-shortener/values-local.yaml`}</code></pre>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto"><code>{`helm upgrade url-shortener ./helm/url-shortener \\
  -n url-shortener \\
...
示範 Helm 前，先清掉剛才手動 apply 的 namespace。原因是 Helm install 會建立同名的 Secret、ConfigMap、Service、Deployment、Ingress。如果 namespace 裡已經有這些物件，Helm 會撞到既有資源，學生會以為 Helm 壞掉。這裡可以直接說：我們不是因為手動部署錯了才清掉，而是要把環境重置，讓 Helm 從零交付同一套產品。

清理指令是 kubectl delete namespace url-shortener，然後用 kubectl wait --for=delete namespace/url-shortener --timeout=180s 等 namespace 完整刪除。這會刪掉剛剛測試用的 PostgreSQL PVC 和短網址資料；但不會刪掉已經 import 到 k3s node 的 images，所以 Helm local workflow 還是可以用。

一個 helm install，就能把剛剛所有東西都建起來：Secret、ConfigMap、PostgreSQL StatefulSet、PVC、Migration Job、API、Frontend、Service、HPA、Ingress。

Helm 也有兩條 image 路徑。public image path 先用 chart 預設 values，讓 Kubernetes 從 Docker Hub pull image。如果現場 Docker Hub pull 不過，才改用 local image fallback，加上 -f values-local.yaml，讓 Helm render 出 url-shortener-api:lab、url-shortener-frontend:lab 和 pullPolicy Never。
...
但 Helm 不會幫你 build image，也不會幫你 import image。public path 依賴 Docker Hub 可以 pull；local fallback 依賴每台 node 已經有 image。values-local.yaml 只是告訴 Helm 使用 url-shortener-api:lab、url-shortener-frontend:lab，pullPolicy 用 Never。
