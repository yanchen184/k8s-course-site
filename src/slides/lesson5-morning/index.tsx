  ...
  // ── 5-1（2/2）：minikube 的問題 + 為什麼需要多節點 + k3s 介紹 ──
  {
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">環境方案：Multipass + k3s</p>
          ...
          <p className="text-slate-400 text-xs text-center mt-2">Multipass = Canonical 出品，一行指令開 Ubuntu VM</p>
        </div>
      </div>
    ),
...
所以結論很明確：我們需要多節點。不是可選的，是必修的。沒有多節點，今天一半以上的內容你只能用想像的，沒辦法親手驗證。

那怎麼辦？再裝幾個 minikube 嗎？不行，minikube 就是設計給單節點用的。我們需要一個真正的多節點 K8s 叢集。

這就要介紹 k3s 了。k3s 是 Rancher Labs 開發的輕量版 Kubernetes。名字為什麼叫 k3s？K8s 的 K 到 s 之間有 8 個字母，k3s 只有 3 個字母，意思是它砍掉了很多不必要的東西，但核心功能全部保留。它跟完整版 K8s 的 API 是完全相容的，你之前學的所有 kubectl 指令、所有 YAML 格式，在 k3s 上通通一樣。差別只是它更輕、更快、安裝更簡單。

k3s 跟 minikube 的定位完全不同。minikube 是在你的筆電上模擬一個單節點叢集，用來學習和開發測試。k3s 是一個真正的 K8s 發行版，可以跑多節點，甚至可以用在生產環境。很多邊緣運算、IoT 的場景都用 k3s，因為它資源佔用少，一台樹莓派都跑得動。
...
[▶ 下一頁]`,
  ...
  // ── 5-2（1/2）：Multipass 建 VM + k3s 安裝 ──
  {
    title: 'k3s 安裝實作：Multipass 建 VM + 安裝 k3s',
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">Step 1-3：建 VM + 裝 master</p>
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            <li>Multipass 安裝：brew / choco / snap</li>
            <li>建兩台 VM（各 2 CPU / 2G RAM / 10G Disk）</li>
            <li>Master 上一行 curl 裝 k3s（30 秒搞定）</li>
          </ul>
        </div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          ...
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            ...
            <li><code className="text-green-400">kubectl get nodes</code> → 兩個 Ready</li>
          </ul>
        </div>
      </div>
    ),
    code: `# Step 1：安裝 Multipass
...
kubectl get pods -o wide   # 看 NODE 欄位`,
    notes: `第七步，設定 kubeconfig。現在你每次操作都要打 multipass exec k3s-master 開頭，很麻煩。我們把 master 的 kubeconfig 複製到本機，這樣直接在本機打 kubectl 就能操作叢集。
...
[▶ 下一頁]`,
  ...
  // ── 5-3（2/2）：縮容 + 水平 vs 垂直 + Docker 對照 + HPA 預告 ──
  {
...
[▶ 下一頁]`,
  ...
  {
    title: '學員實作：擴縮容練習',
    subtitle: '建 httpd Deployment → scale 5 → 觀察 → scale 1',
    section: '5-4：擴縮容實作',
    duration: '8',
    content: (
      <div className="space-y-4">
        <div className="bg-green-900/30 border border-green-500/30 p-4 rounded-lg">
          ...
          <ol className="text-slate-300 text-sm space-y-1 list-decimal list-inside">
            ...
            <li><code className="text-green-400">kubectl get pods -o wide</code> 觀察 Pod 分散在哪些 Node</li>
            <li><code className="text-green-400">kubectl scale</code> 回 <strong className="text-white">1</strong></li>
          </ol>
        </div>

        <div className="bg-amber-900/30 border border-amber-500/40 p-4 rounded-lg">
          <p className="text-amber-400 font-semibold mb-3">挑戰題</p>
          <p className="text-slate-300 text-sm">嘗試 <code className="text-green-400">kubectl scale --replicas=0</code>，會怎樣？</p>
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside mt-2">
            <li>Pod 全砍，但 <strong className="text-white">Deployment 還在</strong></li>
            <li><code className="text-green-400">kubectl get deploy</code> → READY: 0/0</li>
...
kubectl get pods         # 3 個新 Pod`,
    notes: `接下來是你們的實作時間。螢幕上有題目，我唸一遍。
