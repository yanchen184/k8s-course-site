...
Docker Compose 裡面也是一樣的做法，在 services 底下的 environment 區塊寫 MYSQL_ROOT_PASSWORD 冒號 my-secret。概念完全一樣，就是用環境變數把設定傳給容器。
...
這個實作我們從故意做錯開始。在真實世界裡，你很可能就是先碰到這個錯誤，然後才去查原因。
