...
版本確認是重要步驟，讓學員知道他們裝了什麼版本。今天跑的是 v3.20.2。 [▶ 下一頁]`,
  ...
  {
    title: 'Step 2：helm repo add 新增 Chart 來源',
...
3. CHART VERSION vs APP VERSION：很多學員會搞混，用 ingress-nginx 4.15.1 裝的是 Nginx 1.15.1 [▶ 下一頁]`,
  ...
  {
    title: 'Step 3：helm install 一鍵安裝 Ingress Controller',
...
k3s 為什麼用 NodePort：因為 k3s 內建 Traefik，已經在 80/443 上了，LoadBalancer 會 pending，NodePort 最簡單 [▶ 下一頁]`,
  ...
  {
    title: 'Step 4：helm upgrade → history → rollback',
...
再次強調 helm rollback 和 kubectl rollout undo 的差異。 [▶ 下一頁]`,
  ...
  {
    title: 'Step 5：helm show values 看 Chart 有哪些參數',
...
拼錯不會報錯這個要特別強調，因為很多學員以為設了沒效是 bug，其實是打錯參數名。 [▶ 下一頁]`,
  ...
  {
    title: 'Step 6：多環境部署 values-dev + values-prod',
...
重點強調：「同一個 Chart 安裝多次，每次是獨立的 Release」，這和之前講的概念呼應。 [▶ 下一頁]`,
  ...
  {
    title: 'Step 7：kube-prometheus-stack 一鍵安裝監控全家桶',
...
安裝比較慢（要 pull 很多 image），可以先下指令讓它跑，一邊講下一頁，等等再回來看結果。 [▶ 下一頁]`,
  ...
  {
    title: 'Step 8：helm create my-app 自建 Chart',
    subtitle: '四步走完：建骨架 → 改參數 → 安裝 → 升級',
    section: '6-18B：Helm 實作 Part 2',
    duration: '5',
    content: (
      <div className="space-y-2">
        <div className="flex items-start gap-3 bg-slate-800/50 p-3 rounded-lg">
          <span className="bg-cyan-600 text-white text-xs font-bold px-2 py-0.5 rounded shrink-0">① 建</span>
          <div>
            <p className="text-slate-200 text-sm font-mono">helm create my-app</p>
            <p className="text-slate-400 text-xs mt-0.5">自動產生 Chart 骨架（deployment / service / values.yaml）</p>
          </div>
        </div>
        <div className="flex items-start gap-3 bg-slate-800/50 p-3 rounded-lg">
          <span className="bg-yellow-600 text-white text-xs font-bold px-2 py-0.5 rounded shrink-0">② 改</span>
          <div>
            <p className="text-slate-200 text-sm font-mono">cat {'>'} my-app/values.yaml</p>
            <p className="text-slate-400 text-xs mt-0.5">把 image 改成你自己的 <span className="text-yellow-300 font-mono">nginx:alpine</span>，關掉 serviceAccount / autoscaling</p>
          </div>
        </div>
        <div className="flex items-start gap-3 bg-slate-800/50 p-3 rounded-lg">
          <span className="bg-green-600 text-white text-xs font-bold px-2 py-0.5 rounded shrink-0">③ 裝</span>
          <div>
            <p className="text-slate-200 text-sm font-mono">helm install my-app ./my-app</p>
            <p className="text-slate-400 text-xs mt-0.5">從本地目錄安裝，<span className="text-green-400">./my-app</span> 就是你剛建的資料夾</p>
          </div>
        </div>
        <div className="flex items-start gap-3 bg-slate-800/50 p-3 rounded-lg">
          <span className="bg-purple-600 text-white text-xs font-bold px-2 py-0.5 rounded shrink-0">④ 升</span>
          <div>
            <p className="text-slate-200 text-sm font-mono">helm upgrade my-app ./my-app --set image.tag=1.25</p>
            <p className="text-slate-400 text-xs mt-0.5">換一個 image tag，跟之前 upgrade 一樣用法</p>
          </div>
        </div>
        <div className="bg-blue-900/30 border border-blue-500/50 p-2 rounded-lg">
          <p className="text-blue-300 text-xs">模板原理：<span className="font-mono text-yellow-300">{'{{ .Values.image.tag }}'}</span> → Helm 把 values.yaml 的值填進 YAML 模板，生成最終的 K8s 資源</p>
        </div>
      </div>
    ),
    code: `# ① 建骨架
...
不需要深講模板語法，只要讓學員知道：改 values.yaml → helm upgrade → K8s 資源自動更新。 [▶ 下一頁]`,
  },

  // ── 6-18B 實作（5/5）：清理 ──
  {
    title: 'Step 9：清理 helm uninstall',
