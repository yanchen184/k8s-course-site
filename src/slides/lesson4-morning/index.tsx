...
[▶ 下一頁]`,
  ...
  // ── 4-1（3/3）：K8s 登場 + 五問題對應解法 + 宣告式管理 ──
  {
    ...
    notes: `Kubernetes，簡稱 K8s，K 和 s 之間有 8 個字母所以叫 K8s。它原本是 Google 內部的系統 Borg。Google 從 2003 年開始用 Borg 管容器，管了超過十五年。規模是什麼等級？Google 自己說過，他們每週啟動超過二十億個容器。二十億，不是二十個。這個系統經過了十幾年、數十億容器的驗證。2014 年，Google 把 Borg 的設計理念拿出來，用 Go 語言重新實作並開源，這就是 Kubernetes。後來捐給了 CNCF，雲端原生運算基金會。到今天，K8s 是容器編排領域的行業標準，AWS、GCP、Azure、阿里雲、騰訊雲，全部都提供 K8s 的託管服務。
