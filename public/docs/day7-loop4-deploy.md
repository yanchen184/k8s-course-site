...
`stringData` 填明文，K8s 幫你 base64 encode。這個 YAML 不要推進 git。輸出顯示 `secret/app-secrets created`，`DATA 3` 代表三個欄位（mysql-password、redis-password、jwt-secret）都存進去了。
...
看到 `redis-xxxxx` Pod Running 就好。Redis 是 Deployment，不需要 PVC，啟動比 mysql 快很多。
