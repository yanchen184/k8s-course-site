...
[▶ PPT 12/45：PostgreSQL 為什麼 StatefulSet]`,
  },

  // ── [12/45] PostgreSQL 為什麼 StatefulSet ──
  {
    title: '[12/45] PostgreSQL — 為什麼不用 Deployment',
    subtitle: '有穩定身份的三件事',
    section: '7-9 第一段：基礎層',
    duration: '3',
    content: (
...
[▶ PPT 14/45：StatefulSet YAML]`,
  ...
  {
    title: '[14/45] PostgreSQL StatefulSet YAML',
...
[▶ PPT 17/45：巡堂驗收 #1]`,
  ...
  // ── [17/45] 巡堂驗收 #1 ──
  {
    title: '[17/45] ⏸ 巡堂驗收 — 第一段',
    subtitle: '確認 00~03 全部跑成功再繼續',
    section: '⏸ 巡堂驗收 #1',
    duration: '2',
    content: (
      <div className="space-y-3">
        ...
        <div className="bg-red-900/20 border border-red-500/40 p-3 rounded-lg text-red-300 text-sm text-center">
          全部確認才喊繼續，有任何一個不對先停下來
        </div>
      </div>
    ),
    notes: `第一段跑完了，停一下做驗收。這是我上課的習慣，每跑完一段我會停下來掃一眼，學員也有時間確認自己的機器跑到哪裡。

六個指令一次打完。Namespace Active、Secret DATA 3、ConfigMap DATA 7、StatefulSet READY 1/1、PVC Bound、postgres-0 Running 1/1。

六個全對才繼續。有任何一個不對先停下來，當場把它修好。常見錯誤：PVC 卡 Pending——確認有 StorageClass，k3s 內建 local-path；postgres-0 沒 Running——看 describe 的 Events 或 logs。
...
[▶ PPT 28/45：巡堂驗收 #2]`,
  ...
  // ── [28/45] 巡堂驗收 #2 ──
  {
    title: '[28/45] ⏸ 巡堂驗收 — 第二段',
    subtitle: '確認 04~07 全部跑成功再繼續',
    section: '⏸ 巡堂驗收 #2',
    duration: '2',
    content: (
      <div className="space-y-3">
        ...
        <div className="bg-red-900/20 border border-red-500/40 p-3 rounded text-red-300 text-sm text-center">
          有任何一個不對先停下來
        </div>
      </div>
    ),
    notes: `第二段驗收，七個指令一次打完。

前面幾個是看狀態：Redis 和 Backend Pod 都 Running，Job 完成一次，三個 RBAC 資源都存在。

最關鍵是兩個 can-i：backend-sa get configmaps 要回 yes，delete pods 要回 no。這兩個一起過，代表 RBAC 不只有生效、而且權限剛好只開給該給的。

最後 curl /health 回 ok，代表 Backend 真的在跑、真的能處理請求。
...
[▶ PPT 31/45：Task Runner 為什麼沒 Service]`,
  ...
  // ── [31/45] Task Runner 沒有 Service ──
  {
    title: '[31/45] Task Runner — 為什麼沒有 Service',
    subtitle: '方向是反的',
    section: '7-9 第三段：應用層',
    duration: '2',
    content: (
      <div className="space-y-3">
        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded">
          <p className="text-amber-300 font-semibold">問題</p>
          <p className="text-slate-300 mt-1">Task Runner 為什麼沒有 Service？前面每個組件都有 Service，它為什麼沒有？</p>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-3 text-sm text-center">流量方向</p>
          <div className="space-y-3 text-xs">
            <div className="flex items-center gap-2 justify-center">
              ...
              <span className="text-green-400">✓ 有 Service</span>
            </div>
            <div className="flex items-center gap-2 justify-center">
              ...
              <span className="text-amber-400">✗ 自己去拉，不需要別人連它</span>
            </div>
          </div>
        </div>

        <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-3 rounded text-xs">
          <p className="text-cyan-300 font-semibold">Service 的作用是讓別人連進來</p>
          <p className="text-slate-400 mt-1">Task Runner 主動去 Redis Queue 拉任務，沒有人要連到它。方向是反的，不需要 Service。</p>
        </div>
      </div>
    ),
    notes: `Task Runner 很特別，它沒有 Service。前面每個組件都有 Service，它為什麼沒有？

Service 的作用是什麼？讓別人連進來。Frontend 要連 Backend，所以 Backend 要有 Service。Backend 要連 Redis，所以 Redis 要有 Service。

Task Runner 呢？它不是被動等人連，它是主動去 Redis Queue 拉任務。別人不會連它，是它連別人。方向是反的。

既然沒人要連它，Service 就沒必要。

這個觀念很重要。Service 不是「所有 Pod 都要配一個」，是「有人要連你的時候才配」。

[▶ PPT 32/45：Task Runner 不是 DaemonSet]`,
  },

  // ── [32/45] Task Runner 不是 DaemonSet ──
  {
    title: '[32/45] Task Runner — 為什麼不是 DaemonSet',
    subtitle: '跟 Node 數量綁定才用 DaemonSet',
    section: '7-9 第三段：應用層',
    duration: '2',
    content: (
      <div className="space-y-3">
        <div className="bg-slate-800/50 p-3 rounded-lg text-xs">
          <p className="text-cyan-400 font-semibold mb-2">DaemonSet 是什麼</p>
          <p className="text-slate-300">保證每個 Node 上都跑一個 Pod。Node 數量決定 Pod 數量。</p>
        </div>

        <div className="bg-slate-800/50 p-3 rounded-lg text-xs">
          <p className="text-cyan-400 font-semibold mb-2">適合 DaemonSet 的場景</p>
          <ul className="text-slate-300 space-y-1 list-disc list-inside">
            <li>日誌收集 Agent（收每個 Node 的本機 log）</li>
            <li>node-exporter（每個 Node 的 metrics）</li>
            <li>kube-proxy（每個 Node 管 iptables）</li>
          </ul>
        </div>

        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded text-xs">
          <p className="text-amber-300 font-semibold">判斷原則</p>
          <p className="text-slate-300 mt-1">跟 Node 數量綁定就用 DaemonSet，不綁定就用 Deployment。</p>
        </div>

        <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-3 rounded text-xs">
          <p className="text-cyan-300 font-semibold">Task Runner 的情況</p>
          <p className="text-slate-400 mt-1">
            消費 Redis Queue，跟 Node 數量無關。3 個 Node 不代表要 3 個 Task Runner。擴到 100 Node 也不應該有 100 Task Runner。
          </p>
        </div>
      </div>
    ),
    notes: `另一個好問題：Task Runner 要跑在每個 Node 上嗎？要不要用 DaemonSet？

DaemonSet 的定義：保證每個 Node 上都跑一個 Pod。Node 數量決定 Pod 數量。

什麼情況適合 DaemonSet？幾個典型：日誌收集 agent，例如 Fluent Bit，每個 Node 要收自己本機的 log，所以每個 Node 都要跑一個。node-exporter，Prometheus 監控 Node 的硬體指標，每個 Node 都要一個。kube-proxy，每個 Node 都要管本機的 iptables 規則。

共同點是：跟 Node 綁定。

Task Runner 呢？它消費 Redis Queue，跟 Node 完全無關。3 個 Node 不代表要 3 個 Task Runner，叢集擴到 100 Node 也不應該有 100 Task Runner。

判斷原則：跟 Node 數量綁定就用 DaemonSet，不綁定就用 Deployment。

[▶ PPT 33/45：Task Runner YAML]`,
  },

  // ── [33/45] Task Runner YAML ──
  {
    title: '[33/45] Task Runner — Deployment YAML',
...
[▶ PPT 34/45：CronJob YAML]`,
  },

  // ── [34/45] CronJob YAML ──
  {
    title: '[34/45] CronJob — 定時排程觸發',
...
[▶ PPT 36/45：concurrencyPolicy]`,
  },

  // ── [36/45] concurrencyPolicy ──
  {
    title: '[36/45] concurrencyPolicy 三個選項',
...
[▶ PPT 37/45：Ingress k3s Traefik]`,
  },

  // ── [37/45] Ingress Traefik ──
  {
    title: '[37/45] Ingress — k3s 用 Traefik',
    subtitle: '不是 nginx',
    section: '7-9 第三段：應用層',
    duration: '2',
    content: (
...
[▶ PPT 38/45：Middleware + Ingress YAML]`,
  },

  // ── [38/45] Middleware + Ingress YAML ──
  {
    title: '[38/45] Middleware + Ingress YAML',
...
[▶ PPT 42/45：巡堂驗收 #3]`,
  ...
  // ── [42/45] 巡堂驗收 #3 ──
  {
    title: '[42/45] ⏸ 巡堂驗收 — 第三段',
    subtitle: '確認 08~12 全部跑成功再繼續',
    section: '⏸ 巡堂驗收 #3',
    duration: '2',
    content: (
      <div className="space-y-3">
...
五個指令：pods 看應用層的 Pod 都 Running，cronjob 等一分鐘後 LAST SCHEDULE 有時間，hpa TARGETS 有數字，ingress ADDRESS 有 Node IP，task-runner log 有 started waiting 那行訊息。
      ...
      <div className="space-y-2 text-xs">
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">1. 狀態總覽</p>
            <p className="text-slate-400 font-mono text-xs">kubectl get all -n tasks</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">2. PG tasks table</p>
            <p className="text-slate-400 font-mono text-xs">psql -c &apos;\dt&apos;</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">3. Redis 密碼</p>
            <p className="text-slate-400 font-mono text-xs">redis-cli -a ... ping → PONG</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">4. Migration log</p>
            <p className="text-slate-400 font-mono text-xs">Migration complete</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">5. RBAC 最小權限</p>
            <p className="text-slate-400 text-xs">get cm yes / delete pods no / get secrets no</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">6. CronJob 觸發</p>
            <p className="text-slate-400 text-xs">task-scheduler-xxx 每分鐘 Complete</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">7. Task Runner</p>
            <p className="text-slate-400 text-xs">三個 Pod waiting for tasks</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">8. Ingress + API</p>
            <p className="text-slate-400 text-xs">curl /api/tasks 回 JSON</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">9. Frontend 畫面</p>
            <p className="text-slate-400 text-xs">瀏覽器 http://task.local 看到 UI</p>
          </div>
        </div>

        <div className="bg-green-900/20 border border-green-500/40 p-3 rounded text-green-300 text-sm text-center mt-3">
          九個全綠 → 整套系統活了 🎉
        </div>
      </div>
    ),
    notes: `現在做最終驗收，九個檢查點，從組件狀態一路驗到端到端使用者體驗。

第一，get all -n tasks 看整體，Pod、Deployment、StatefulSet、Service、Job、HPA、CronJob 都列出來，沒有紅色就好。

第二，進 postgres-0 打 psql 的 \\dt，看到 tasks 表存在，代表 migration 真的執行了。

第三，進 redis 容器 redis-cli 加密碼 ping，回 PONG 代表 Redis 活著、密碼正確。

第四，看 migrate Job 的 log，Migration complete 那行。

第五，RBAC 三個 can-i：get configmaps yes、delete pods no、get secrets no。最小權限落實。

第六，get job 看到每分鐘有新的 task-scheduler Job Complete，CronJob 正常觸發。

第七，task-runner log 三個 Pod 都 waiting，連上 Redis 了。

第八，curl 加 Host 打到 /api/tasks，回 JSON，代表 Ingress、Backend、DB 三者都通。

第九，瀏覽器直接開 http://task.local，看到 Frontend 畫面。

九個全綠，恭喜，整套系統從零建起來了。

[▶ PPT 44/45：7-10 QA 四題]`,
  },
