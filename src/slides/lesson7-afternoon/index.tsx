...
[▶ PPT 46/54：先把 image 放進每台 node]`,
  },

  // ── [46/54] Image 準備 ──
  {
    title: '[46/54] 先把 image 放進每台 node',
    subtitle: '這一步完成後，後面的 kubectl apply / helm install 才會順',
    section: '7-11 Image 準備',
    duration: '5',
    content: (
      <div className="space-y-2 text-[11px]">
        <div className="bg-gradient-to-r from-amber-900/30 to-red-900/30 border border-amber-500/50 p-2 rounded">
        ...
        <div className="bg-slate-900/60 border border-slate-700 p-2 rounded">
          <p className="text-cyan-300 font-semibold mb-1">先進入 Lab 目錄</p>
          <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto"><code>{`cd ~/k8s-course-labs/lesson7/url-shortener`}</code></pre>
        </div>

        <div className="bg-slate-900/60 border border-slate-700 p-2 rounded">
          <p className="text-cyan-300 font-semibold mb-1 text-center">上課流程</p>
          <div className="flex items-center justify-center gap-2 flex-wrap">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">1. 下載 tar</p>
            <p className="text-slate-400 mt-1">Windows 瀏覽器開 Google Drive：</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 whitespace-pre-wrap break-all"><code>{`drive.google.com/file/d/1LAvKkpENmTtQjvxxrivgoHDbuJWzcJH-`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">2. 傳到 control plane VM</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`ssh user@<control-plane-ip> "mkdir -p ~/Downloads"
  ...
  user@<control-plane-ip>:~/Downloads/url-shortener-k3s-images.tar`}</code></pre>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-1 overflow-x-auto"><code>{`sha256sum ~/Downloads/url-shortener-k3s-images.tar`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-2 rounded">
            <p className="text-cyan-300 font-semibold">4. 匯入並檢查每台 node</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-1 overflow-x-auto"><code>{`IMAGE_TAR=~/Downloads/url-shortener-k3s-images.tar \\
K3S_NODES="user@<control-plane-ip> user@<worker-ip>" \\
  ./scripts/load-images-to-k3s-ssh.sh
K3S_NODES="user@<control-plane-ip> user@<worker-ip>" \\
  ./scripts/check-k3s-images-ssh.sh`}</code></pre>
            <p className="text-slate-400 mt-1">把 &lt;control-plane-ip&gt; / &lt;worker-ip&gt; 換成自己的 VM IP。</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <div className="bg-cyan-900/20 border border-cyan-500/40 p-2 rounded text-cyan-300 text-center">
          ...
          <div className="bg-amber-900/20 border border-amber-500/40 p-2 rounded text-amber-200 text-center">
            sudo password required：每台 VM 先讓 sudo -n k3s 可免密碼執行。
          </div>
        </div>

        <div className="bg-emerald-900/20 border border-emerald-500/40 p-2 rounded text-emerald-300 text-center font-semibold">
...
現場可以請學生先在每台 VM 跑 sudo -n k3s ctr images list -q。如果出現 sudo: a password is required，就用 sudo visudo -f /etc/sudoers.d/k3s-lab-user 加入 user ALL=(ALL) NOPASSWD: /usr/local/bin/k3s。帳號或 k3s 路徑不同時要替換成自己的值，最後 chmod 440 後再重測。
...
[▶ PPT 47/54：手動部署流程]`,
  ...
  // ── [47/54] 手動部署流程 ──
  {
    title: '[47/54] 一起把產品部署起來',
    subtitle: '照順序完成 9 個部署問題',
    section: '7-12 手動部署',
    duration: '6',
    content: (
      <div className="space-y-3 text-xs">
        ...
        <pre className="bg-slate-950 text-slate-100 p-3 rounded overflow-x-auto"><code>{`kubectl apply -f k8s/00-namespace.yaml
...
kubectl apply -f k8s/08-ingress.yaml`}</code></pre>
      </div>
    ),
    notes: `image 準備和檢查通過後，才開始 apply YAML。流程分成九個檔案。每個檔案只負責一層，這樣學生比較不會迷路。
...
[▶ PPT 48/54：環境檢查與基礎資源]`,
  ...
  // ── [48/54] 環境檢查與基礎資源 ──
  {
    ...
    subtitle: '不要一開始就全部 apply，先確認 node、system pod、namespace、設定',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 0：確認叢集狀態</p>
...
kubectl get storageclass`}</code></pre>
            <p className="text-slate-400 mt-2">確認 node Ready、Traefik/metrics/storage 都正常。</p>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 1-2：建立產品邊界與設定</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s/00-namespace.yaml
kubectl apply -f k8s/01-secret.yaml
kubectl apply -f k8s/02-configmap.yaml`}</code></pre>
          </div>
        </div>
        ...
        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300 text-center">
          這一步還沒有跑產品，先把「產品放哪裡」和「產品需要哪些設定」準備好。
        </div>
      </div>
    ),
...
[▶ PPT 49/54：資料庫與 Migration]`,
  ...
  // ── [49/54] PostgreSQL 與 Migration ──
  {
    ...
    subtitle: '資料庫要能保存資料，資料表要由 Job 建立',
    section: '7-12 手動部署',
    duration: '6',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 3：PostgreSQL StatefulSet + PVC</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s-local/03-postgres.yaml
...
kubectl get pod postgres-0 -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 4：Migration Job</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s-local/04-migrate-job.yaml
...
kubectl logs job/db-migrate -n url-shortener`}</code></pre>
        ...
        <div className="bg-emerald-900/20 border border-emerald-500/40 p-3 rounded text-emerald-300 text-center">
          local YAML 裡 `postgres:15`、`busybox:1.36`、`url-shortener-api:lab` 都使用 `imagePullPolicy: Never`。
        </div>
      </div>
    ),
    notes: `這張把資料層流程補完整。

先跑 PostgreSQL，因為 API 和 migration 都依賴 DB。這裡使用 local 版 03-postgres.yaml，原因是 postgres:15 也在 tar 裡，必須用 imagePullPolicy: Never，避免現場又去 Docker Hub 拉。
...
[▶ PPT 50/54：API 與 Frontend]`,
  ...
  // ── [50/54] API 與 Frontend ──
  {
    ...
    subtitle: 'API 負責邏輯，Frontend 負責畫面，Service 提供穩定入口',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 5：API Deployment + Service</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s-local/05-api.yaml
...
kubectl get pods -l app=url-api -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 6：Frontend Deployment + Service</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f k8s-local/06-frontend.yaml
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-slate-900/60 border border-slate-700 p-3 rounded">
            ...
            <ul className="text-slate-400 mt-2 space-y-1 list-disc list-inside">
              ...
              <li>local image 不依賴 Docker Hub</li>
            </ul>
          </div>
        </div>

        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300 text-center">
          到這裡產品已經有資料層、邏輯層、畫面層，下一步才是對外入口。
        </div>
      </div>
    ),
    notes: `這張補 API 和 Frontend 的流程。
...
[▶ PPT 51/54：HPA 與 Ingress]`,
  ...
  // ── [51/54] HPA 與 Ingress ──
  {
    ...
    subtitle: 'HPA 管 API 擴縮，Ingress 讓瀏覽器進產品',
    section: '7-12 手動部署',
    duration: '5',
    content: (
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 7：HPA</p>
...
kubectl describe hpa url-api-hpa -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 8：Ingress</p>
...
kubectl get ingress -n url-shortener`}</code></pre>
            <p className="text-cyan-300 font-semibold mt-2">Windows hosts</p>
  ...
  -Value "<NODE-IP> short.local"`}</code></pre>
        ...
        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded text-amber-300 text-center">
          HPA `TARGETS` 如果是 unknown，先看 API 是否有 CPU requests，再看 metrics-server 是否正常。
        </div>
      </div>
    ),
    notes: `這張補對外入口和擴縮。
...
kubectl delete pod postgres-0 -n url-shortener`}</code></pre>
        ...
        <div className="bg-cyan-900/20 border border-cyan-500/40 p-3 rounded text-cyan-300">
          如果 Windows 打不開：先看 Windows hosts，再看 Windows 是否連得到 VM IP，最後才看 Ingress / Pod。
        </div>
...
kubectl wait --for=delete namespace/url-shortener --timeout=180s`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold mb-1">2. 再用 Helm 一鍵部署</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto"><code>{`helm install url-shortener ./helm/url-shortener \\
  -n url-shortener \\
  --create-namespace \\
