...
好，上午我們完成了一件很重要的事情，從零開始寫出了第一個 Pod 的 YAML，然後做了完整的 CRUD 操作。建立、查看、進容器、刪除，整個流程跑了一遍。如果你上午的練習都有跟上，你已經會在 K8s 裡面跑容器了。
...
這次 STATUS 應該會變成 Running 了。如果是 ContainerCreating，等幾秒鐘再看。看到 Running，第一個排錯成功了。
...
建一個新檔案叫 pod-crash.yaml。apiVersion v1，kind Pod，metadata 底下 name 寫 crash-pod。spec 底下 containers 列表的第一個項目，name 寫 crash-test，image 寫 busybox:1.36。到這裡都沒問題，Image 名字是對的，一定拉得到。但是我要加一個 command 欄位。command 是一個陣列，裡面放三個字串：第一個是 "/bin/sh"，第二個是 "-c"，第三個是 "echo hello && exit 1"。

這個 command 的意思是什麼？容器啟動之後，啟動一個 shell，先印出 hello，然後以 exit 1 的錯誤狀態退出。
...
用 kubectl logs crash-pod，你會看到 hello 這個字。因為我們的 command 是 echo hello 再 exit 1，所以容器有印東西才退出。如果你把 echo 拿掉只留 exit 1，log 就會是空的，因為程式什麼都沒印就退出了。在真實的場景中，如果你的 Java 程式啟動就 crash，logs 裡面通常會有 Exception 的堆疊資訊，那就是你排錯的關鍵線索。

你也可以試試加 --previous 參數。

指令：kubectl logs crash-pod --previous

--previous 的意思是看上一個已經結束的容器的日誌。因為 K8s 一直在重啟容器，當前的容器可能剛建好還沒有任何日誌，但上一個 crash 掉的容器可能有留下一些東西。這個參數在真實場景非常有用，一定要記住。
...
第二個排錯練習，CrashLoopBackOff。我們用 lab repo 裡已經準備好的 pod-crash.yaml。

指令：kubectl apply -f pod-crash.yaml

這個 Pod 用的 image 是 busybox:1.36，command 設成 /bin/sh -c "echo hello && exit 1"。意思是：容器啟動後先印 hello，然後以 exit code 1 結束，也就是故意失敗。

指令：kubectl get pods --watch

你會看到 STATUS 從 Running 變成 Error，然後變成 CrashLoopBackOff。RESTARTS 欄位會一直增加：1、2、3...而且每次重啟的間隔越來越長，10 秒、20 秒、40 秒，這就是退避策略。按 Ctrl+C 停止觀察。

現在用 logs 看看容器印了什麼。

指令：kubectl logs crash-pod

你會看到 hello。因為我們的 command 是 echo hello && exit 1，所以容器有印東西再退出的。

但如果 command 只有 exit 1，沒有 echo 呢？那 logs 就會是空的。因為 kubectl logs 看的是 stdout 和 stderr 的輸出，exit 1 只是回傳一個錯誤碼，什麼都沒印，所以 log 是空的。這在實際排錯的時候很重要：如果 logs 是空的，不代表沒有問題，而是程式在印任何東西之前就掛了。這時候你要靠 describe 的 Events 來找原因。

如果你想看上一次（已經死掉的那次）的 log，可以加 --previous。
...
第一題，故意寫錯 YAML 欄位。建一個 Pod YAML，故意把 kind 從 Pod 打成 Podd。apply 看看報什麼錯。再把 kind 改回 Pod，然後把 apiVersion 從 v1 改成 v2，apply 看看報什麼錯。比較兩種錯誤訊息有什麼不同。你會發現格式很像，都是 no matches for kind "xxx" in version "yyy"，差別在一個是 kind 錯、一個是 version 錯。修正之後確認 Pod 可以正常 Running。

指令：cp pod.yaml pod-field-broken.yaml
指令：（編輯 pod-field-broken.yaml，把 kind: Pod 改成 kind: Podd）
指令：kubectl apply -f pod-field-broken.yaml
指令：（改回 kind: Pod，把 apiVersion: v1 改成 apiVersion: v2）
指令：kubectl apply -f pod-field-broken.yaml
指令：（改回 apiVersion: v1，apply 確認 Running）
指令：kubectl delete pod my-nginx
指令：rm pod-field-broken.yaml

第二題，觀察 CrashLoopBackOff 和 Completed 的差別。建一個 Pod，image 用 busybox:1.36，command 設成 ["/bin/sh", "-c", "echo 'K8s is awesome' && sleep 5 && exit 1"]。

指令：（建一個 pod-crash-test.yaml，image busybox:1.36，command 如上）
指令：kubectl apply -f pod-crash-test.yaml
指令：kubectl get pods --watch

觀察 STATUS 從 Running 變成 Error 再變成 CrashLoopBackOff。用 kubectl logs 看到 K8s is awesome。然後把 exit 1 改成 exit 0，重新 apply，觀察這次 Pod 的 STATUS 會變成 Completed 而不是 CrashLoopBackOff。因為 exit 0 代表正常結束，K8s 不會把它當成 crash。觀察完記得清理。

指令：kubectl logs crash-test-pod
指令：（把 exit 1 改成 exit 0，delete 再 apply）
指令：kubectl get pods --watch
指令：kubectl delete pod crash-test-pod

做完的同學，給你一個探索建議。試試用 kubectl run 搭配 --restart=Never 來快速建一個一次性的 Pod。指令是 kubectl run one-shot --image=busybox:1.36 --restart=Never -- /bin/sh -c "echo done && sleep 3"。觀察這個 Pod 跑完之後 STATUS 會變成 Completed，不會重啟。跟我們剛才的 CrashLoopBackOff 對比一下：正常退出的 Pod 安安靜靜地結束，不正常退出的 Pod 才會一直被 K8s 重啟。觀察完 kubectl delete pod one-shot 清理掉。

指令：kubectl run one-shot --image=busybox:1.36 --restart=Never -- /bin/sh -c "echo done && sleep 3"
指令：kubectl get pods --watch
指令：kubectl delete pod one-shot
...
這種模式有一個專門的名字，叫 Sidecar 模式。Sidecar 就是「邊車」的意思，主容器負責核心業務，輔助容器像掛在旁邊的邊車，負責額外的功能。邊車不是主角，但它讓主角的工作更完整。
...
最後是 spec 最底下的 volumes 區塊。volumes 欄位跟 containers 是同一層的，都在 spec 底下。不是在某個容器裡面，而是在 Pod 層級定義的。這很合理，因為 Volume 是給整個 Pod 裡面的容器共享的，所以要定義在 Pod 層級。你可以用 kubectl explain pod.spec.volumes 查看所有支援的 Volume 類型。

這裡定義了 shared-logs 這個 Volume，類型是 emptyDir 後面接一對大括號。大括號是空的，代表用預設設定，不需要額外的參數。emptyDir 就是一個空目錄，Pod 建立的時候自動建立，Pod 刪除的時候自動消失。
