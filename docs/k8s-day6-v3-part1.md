...
- 部署兩個 Service：frontend（jasonmel/k8s-demo-app）+ api（jasonmel/k8s-demo-app）
...
- 挑戰：改成 host routing + 第三個服務（admin，用 jasonmel/k8s-demo-app，MESSAGE=Hello from admin）
...
Controller 確認了，來部署我們的應用。我準備了一個 ingress-basic.yaml，裡面包含四個東西：frontend 的 Deployment 和 Service，用 jasonmel/k8s-demo-app image，MESSAGE 設為 Hello from frontend；api 的 Deployment 和 Service，用 jasonmel/k8s-demo-app image，MESSAGE 設為 Hello from api；還有一個 Ingress 規則，path-based routing。
...
學員實作時間。必做題：照著剛才的步驟，部署兩個服務加上 path routing 的 Ingress，用 curl 驗證斜線 frontend 看到 Message: Hello from frontend、斜線 api 看到 Message: Hello from api。挑戰題：改成 host routing，再加一個第三個服務，用 jasonmel/k8s-demo-app 加上 MESSAGE=Hello from admin 做一個 admin 服務，admin.myapp.local 導到它。記得 /etc/hosts 也要加上 admin.myapp.local。
