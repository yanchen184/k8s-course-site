...
我們用一個完整的流程來串起來。當你在終端輸入 kubectl create deployment nginx --replicas=3，這個請求首先到達 API Server，API Server 驗證你的權限，通過之後把「要 3 個 nginx Pod」這個期望狀態記錄到 etcd。接著 Scheduler 發現有 3 個 Pod 還沒有分配到 Node，它看了看各個 Node 的資源狀況，決定分配到哪些 Node。對應 Node 上的 kubelet 收到通知後，呼叫 Container Runtime 把容器跑起來。之後 Controller Manager 持續監控，只要有 Pod 掛了，就會觸發整個流程重新來一遍，確保始終有 3 個 Pod 在運行。`,
  ...
  // ── Slide 10 K8s 架構圖 ───────────────────────────
  {
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-3">Master Node</p>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-900/30 border border-blue-500/30 p-3 rounded-lg text-center">
              <p className="text-blue-400 font-semibold text-sm">API Server</p>
              <p className="text-slate-400 text-xs">大門 + 保全</p>
            </div>
            <div className="bg-blue-900/30 border border-blue-500/30 p-3 rounded-lg text-center">
              <p className="text-blue-400 font-semibold text-sm">etcd</p>
              <p className="text-slate-400 text-xs">叢集的大腦</p>
            </div>
            <div className="bg-blue-900/30 border border-blue-500/30 p-3 rounded-lg text-center">
              <p className="text-blue-400 font-semibold text-sm">Scheduler</p>
              <p className="text-slate-400 text-xs">調度器</p>
            </div>
            <div className="bg-blue-900/30 border border-blue-500/30 p-3 rounded-lg text-center">
              <p className="text-blue-400 font-semibold text-sm">Controller Manager</p>
              <p className="text-slate-400 text-xs">監控 + 自動修復</p>
            </div>
          </div>
        </div>

        <div className="text-center text-slate-400 text-sm">kubectl / API</div>

        <div className="grid grid-cols-3 gap-3">
          {["Worker 1", "Worker 2", "Worker 3"].map((name) => (
            <div key={name} className="bg-slate-800/50 p-3 rounded-lg">
              <p className="text-cyan-400 font-semibold text-sm mb-2 text-center">{name}</p>
              ...
              <div className="flex gap-1 mt-2">
                <span className="bg-green-900/40 text-green-400 text-xs px-2 py-0.5 rounded">Pod</span>
                <span className="bg-green-900/40 text-green-400 text-xs px-2 py-0.5 rounded">Pod</span>
              </div>
            </div>
          ))}
