...
[▶ PPT 4/47：前置確認 Node Ready]`,
  ...
  // ── [4/47] 前置確認 Node Ready ──
  {
    title: '[4/47] 前置確認：兩個 Node 都要 Ready',
...
[▶ PPT 5/47：IP 飄移修復]`,
  ...
  // ── [5/47] IP 飄移修復 ──
  {
    title: '[5/47] 修復 worker NotReady（IP 飄移）',
...
[▶ PPT 8/47：Secret YAML]`,
  },

  // ── [8/47] Secret YAML ──
  {
    title: '[8/47] Secret — 機密資料',
...
[▶ PPT 10/47：ConfigMap YAML]`,
  },

  // ── [10/47] ConfigMap YAML ──
  {
    title: '[10/47] ConfigMap — 非機密設定',
...
[▶ PPT 11/47：ConfigMap 說明]`,
  },

  // ── [11/47] ConfigMap 說明 ──
  {
    title: '[11/47] ConfigMap — 為什麼值是 Service 名稱',
...
[▶ PPT 12/47：PostgreSQL 為什麼 StatefulSet]`,
  },

  // ── [12/47] PostgreSQL 為什麼 StatefulSet ──
  {
    title: '[12/47] PostgreSQL — 為什麼不用 Deployment',
    subtitle: '有穩定身份的三件事',
    section: '7-9 第一段：基礎層',
    duration: '3',
    content: (
...
[▶ PPT 13/47：Operator 演進路線]`,
  },

  // ── [13/47] Operator 演進 ──
  {
    title: '[13/47] 現代生產環境：用 Operator',
...
[▶ PPT 14/47：StatefulSet YAML]`,
  },

  // ── [14/47] StatefulSet YAML ──
  {
    title: '[14/47] PostgreSQL StatefulSet YAML',
...
[▶ PPT 15/47：Headless Service]`,
  },

  // ── [15/47] Headless Service ──
  {
    title: '[15/47] Headless Service — StatefulSet 的搭檔',
...
[▶ PPT 16/47：Pod DNS 命名]`,
  },

  // ── [16/47] Pod DNS 命名 ──
  {
    title: '[16/47] Pod DNS 命名規則',
...
[▶ PPT 17/47：基礎層 checklist]`,
  },

  // ── [17/47] 基礎層 checklist ──
  {
    title: '[17/47] 基礎層告一段落 — 來對一遍',
    subtitle: '六項都綠才繼續往下',
    section: '7-9 第一段：基礎層',
    duration: '2',
    content: (
...
[▶ PPT 19/47：Redis Deployment]`,
  },

  // ── [19/47] Redis Deployment YAML ──
  {
    title: '[19/47] Redis — 為什麼用 Deployment',
...
[▶ PPT 20/47：Service 三種類型]`,
  },

  // ── [20/47] Service 三類型 ──
  {
    title: '[20/47] Service 三種類型怎麼選',
...
[▶ PPT 21/47：Job YAML]`,
  ...
  // ── [21/47] Job YAML ──
  {
    title: '[21/47] Job — 一次性 DB Migration',
...
[▶ PPT 25/47：Backend Deployment YAML]`,
  },

  // ── [25/47] Backend YAML ──
  {
    title: '[25/47] Backend API — Deployment + Service',
...
readinessProbe 打 /health，回 200 才加進 Service 後端列表。這個 readinessProbe 是什麼、為什麼存在？下面三張專題講完再回來看 Backend。

[▶ PPT 26/47：Running 在騙你]`,
  },

  // ── [26/47] Probe 是什麼 — Running 在騙你 ──
  {
    title: '[26/47] Running 在騙你 — 為什麼需要 Probe',
    subtitle: 'Running 只代表主行程還活著，不代表在做事',
    section: '7-9 第二段：Probe 健康檢查',
    duration: '3',
    content: (
      <div className="space-y-3">
        <div className="bg-amber-900/20 border border-amber-500/40 p-3 rounded text-xs">
          <p className="text-amber-300 font-semibold mb-1">你一直以為的判斷方式</p>
          <p className="text-slate-300"><span className="font-mono">kubectl get pods</span> → STATUS 是 Running → 覺得沒事</p>
          <p className="text-red-300 mt-1">但 Running 在騙你。它只代表容器主行程還在跑，process 活著就算 Running。</p>
        </div>

        <div className="bg-slate-800/50 p-3 rounded-lg text-xs">
          <p className="text-cyan-400 font-semibold mb-2">三種會騙你的情境</p>
          <div className="space-y-2">
            <div className="bg-slate-800/60 border-l-4 border-red-500 p-2 rounded">
              <p className="text-red-300 font-semibold">① API 死鎖</p>
              <p className="text-slate-400">process 還活著，但不回任何請求</p>
            </div>
            <div className="bg-slate-800/60 border-l-4 border-red-500 p-2 rounded">
              <p className="text-red-300 font-semibold">② 連線池滿</p>
              <p className="text-slate-400">回 500 錯誤，K8s 照樣顯示 Running</p>
            </div>
            <div className="bg-slate-800/60 border-l-4 border-red-500 p-2 rounded">
              <p className="text-red-300 font-semibold">③ 啟動還沒完成</p>
              <p className="text-slate-400">Pod 剛 Running 就接流量，前幾十秒請求全炸</p>
            </div>
          </div>
        </div>

        <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-3 rounded text-xs">
          <p className="text-cyan-300 font-semibold">🧠 核心概念</p>
          <p className="text-slate-300 mt-1">K8s 預設不知道你的服務「真的好了沒」。你必須告訴它「怎麼判斷我是真的好」。這就是 Probe 要解決的問題。</p>
        </div>
      </div>
    ),
    notes: `講 Probe 之前，先拆一個大家都誤會的東西。你怎麼確認你的服務正常？九成的人會說：kubectl get pods 看 STATUS 是 Running 就好。

但 Running 這個狀態在騙你。Running 只代表一件事——容器裡面的主行程還在跑。process 活著，K8s 就認為你是 Running。

三種情境讓你秒懂問題在哪。

第一，API 死鎖。你 Java 程式死鎖了，process 還活著在跑，但完全不處理請求。K8s 看你 Running。

第二，連線池滿了。資料庫連線池爆，每個請求都回 500，但 process 沒崩掉。K8s 看你 Running。

第三，啟動還沒完成。Java 應用啟動要 60 秒初始化，這 60 秒 Pod 一啟動就顯示 Running，K8s 就把流量導過來，前 60 秒的請求全部失敗。

這三個情境的共同點是：Running 騙了你。K8s 預設不知道你的服務真的好了沒，它只會問 process 在不在。你必須給它一個方法——「這樣打我，打通才算好」。這個方法就是 Probe。

K8s 用三種 Probe 來解這件事，下一張講三種分別問什麼問題。

[▶ PPT 27/47：三種 Probe]`,
  },

  // ── [27/47] 三種 Probe + 三種檢查方式 ──
  {
    title: '[27/47] 三種 Probe — 各問各的問題',
    subtitle: 'liveness / readiness / startup + 三種檢查方式',
    section: '7-9 第二段：Probe 健康檢查',
    duration: '4',
    content: (
      <div className="space-y-3">
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-slate-800 text-amber-300">
                <th className="border border-slate-700 p-2 text-left">Probe</th>
                <th className="border border-slate-700 p-2 text-left">問的問題</th>
                <th className="border border-slate-700 p-2 text-left">失敗怎麼辦</th>
                <th className="border border-slate-700 p-2 text-left">什麼時候用</th>
              </tr>
            </thead>
            <tbody className="text-slate-300">
              <tr>
                <td className="border border-slate-700 p-2 font-mono text-cyan-300">livenessProbe</td>
                <td className="border border-slate-700 p-2">你還活著嗎？</td>
                <td className="border border-slate-700 p-2 text-red-300">重啟容器</td>
                <td className="border border-slate-700 p-2">解死鎖、掛掉回不了</td>
              </tr>
              <tr className="bg-slate-800/30">
                <td className="border border-slate-700 p-2 font-mono text-cyan-300">readinessProbe</td>
                <td className="border border-slate-700 p-2">準備好接流量了嗎？</td>
                <td className="border border-slate-700 p-2 text-amber-300">從 Service 移除</td>
                <td className="border border-slate-700 p-2">啟動中、暫時不能服務</td>
              </tr>
              <tr>
                <td className="border border-slate-700 p-2 font-mono text-cyan-300">startupProbe</td>
                <td className="border border-slate-700 p-2">啟動完了沒？</td>
                <td className="border border-slate-700 p-2 text-red-300">重啟容器</td>
                <td className="border border-slate-700 p-2">慢啟動（Java 啟動 60 秒）</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-slate-800/50 p-3 rounded-lg text-xs">
          <p className="text-cyan-400 font-semibold mb-2">怎麼「戳」？三種檢查方式</p>
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-slate-800/60 border-l-4 border-green-500 p-2 rounded">
              <p className="text-green-300 font-semibold font-mono">httpGet</p>
              <p className="text-slate-400 mt-1">打 URL，200~399 成功</p>
              <p className="text-amber-300 mt-1">Web API（最常用）</p>
            </div>
            <div className="bg-slate-800/60 border-l-4 border-blue-500 p-2 rounded">
              <p className="text-blue-300 font-semibold font-mono">tcpSocket</p>
              <p className="text-slate-400 mt-1">連 port，連上成功</p>
              <p className="text-amber-300 mt-1">DB、Redis、無 HTTP 的服務</p>
            </div>
            <div className="bg-slate-800/60 border-l-4 border-purple-500 p-2 rounded">
              <p className="text-purple-300 font-semibold font-mono">exec</p>
              <p className="text-slate-400 mt-1">執行指令，exit 0 成功</p>
              <p className="text-amber-300 mt-1">自訂檢查邏輯</p>
            </div>
          </div>
        </div>

        <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-3 rounded text-xs text-slate-300">
          Docker HEALTHCHECK 只貼 unhealthy 標籤，不重啟也不切流量。K8s 三種 Probe 各司其職，而且會<span className="text-amber-300">真的動作</span>。
        </div>
      </div>
    ),
    notes: `K8s 用三種 Probe 解 Running 不可靠的問題。每一種問不同的問題，失敗也做不同的事。

livenessProbe：問你還活著嗎？失敗就把容器整個重啟。注意是重啟容器，不是刪 Pod。什麼時候用？解死鎖、解掛死那種回不來的狀況。

readinessProbe：問準備好接流量了嗎？失敗 K8s 不會重啟容器，而是把這個 Pod 從 Service 的後端列表拔掉，不再給流量。等 Pod 恢復了自動加回來。什麼時候用？啟動中還在連 DB、暫時連線池爆了、過載想拒接新流量。

startupProbe：專門給啟動特別慢的應用。Java Spring Boot 啟動要 60 秒很常見。startupProbe 通過之前，liveness 跟 readiness 都不執行。通過之後 startupProbe 就退場，交棒給另外兩個。沒有 startupProbe 的話 liveness 會在啟動中判定失敗，Pod 就一直被重啟，永遠起不來。

三種 Probe 都要決定「怎麼戳」。三個選項：httpGet 打 URL，200~399 算通過，Web API 最常用。tcpSocket 連上 port 就算通過，適合 PostgreSQL、Redis 這種沒有 HTTP 介面的服務。exec 執行指令，exit code 0 算通過，邏輯自己寫。

對照 Docker HEALTHCHECK 一下。Docker 的 HEALTHCHECK 只有一種，失敗只會在容器上貼個 unhealthy 標籤，不會幫你重啟、不會切流量。K8s 三種 Probe 各管各的，而且會真的採取動作——重啟或拔流量。

[▶ PPT 28/47：Probe YAML 四個參數]`,
  },

  // ── [28/47] Probe YAML 四個參數 ──
  {
    title: '[28/47] Probe 的四個關鍵參數',
    subtitle: 'initialDelay、period、failureThreshold、timeout',
    section: '7-9 第二段：Probe 健康檢查',
    duration: '3',
    content: (
      <div className="space-y-3">
        <div className="bg-slate-800/50 p-3 rounded-lg">
          <pre className="bg-slate-950 text-slate-100 p-3 rounded text-xs"><code>{`readinessProbe:
  httpGet:
    path: /health
    port: 3000
  initialDelaySeconds: 5     # 啟動後先等幾秒再開始檢查
  periodSeconds: 10           # 每幾秒戳一次
  failureThreshold: 3         # 連續失敗幾次才判定壞掉
  timeoutSeconds: 1           # 每次戳等幾秒沒回應算超時`}</code></pre>
        </div>

        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-2 rounded">
            <p className="text-cyan-300 font-semibold font-mono">initialDelaySeconds</p>
            <p className="text-slate-400 mt-1">設太小 → 啟動中誤判失敗；設太大 → 出問題反應慢</p>
          </div>
          <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-2 rounded">
            <p className="text-cyan-300 font-semibold font-mono">periodSeconds</p>
            <p className="text-slate-400 mt-1">readiness 通常比 liveness 短（好了趕快接流量）</p>
          </div>
          <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-2 rounded">
            <p className="text-cyan-300 font-semibold font-mono">failureThreshold</p>
            <p className="text-slate-400 mt-1">避免偶發抖動就重啟。一般 3 次比較穩</p>
          </div>
          <div className="bg-slate-800/40 border-l-4 border-cyan-500 p-2 rounded">
            <p className="text-cyan-300 font-semibold font-mono">timeoutSeconds</p>
            <p className="text-slate-400 mt-1">超時算失敗。預設 1 秒對慢服務可能不夠</p>
          </div>
        </div>

        <div className="bg-gradient-to-br from-green-900/20 to-cyan-900/20 border border-green-500/40 p-3 rounded text-xs">
          <p className="text-green-300 font-semibold mb-1">🧠 回到 Loop 3：每個組件該用哪種？</p>
          <ul className="text-slate-300 space-y-1 list-disc list-inside">
            <li><b>Backend</b>：readiness（/health httpGet）← 下一張就會看到</li>
            <li><b>PostgreSQL</b>：readiness（tcpSocket 5432）— 沒 HTTP，連上就算好</li>
            <li><b>Redis</b>：readiness（exec redis-cli ping）— 要確認回 PONG</li>
            <li><b>Task Runner</b>：不用（沒人連它、沒對外服務）</li>
          </ul>
        </div>
      </div>
    ),
    notes: `Probe 的 YAML 只有四個參數需要記。

initialDelaySeconds，容器啟動後等幾秒再開始檢查。你的程式啟動要多久，這個值就設多少加一點緩衝。設太小 Pod 啟動中就被判定失敗會一直重啟。設太大出問題要等很久 K8s 才發現。

periodSeconds，每幾秒戳一次。readiness 通常設短一點像 5 秒，好了趕快接流量。liveness 可以長一點像 10 秒，沒必要那麼急。

failureThreshold，連續失敗幾次才判定壞掉。這個很重要，不要設 1。網路偶爾抖一下就誤判，Pod 就一直被重啟，災難。一般設 3 比較穩，連續三次都失敗才動作。

timeoutSeconds，每次戳等幾秒沒回應算超時。預設是 1 秒。對慢一點的服務可能不夠，啟動中偶爾會超過 1 秒，調成 3 或 5 看情況。

講完概念回到 Loop 3，我們這套系統每個組件該用哪種 Probe？

Backend 最適合 httpGet 打 /health，Web API 標準做法，下一張看 YAML 實際怎麼寫。

PostgreSQL 沒有 HTTP 介面，用 tcpSocket 連 5432 port，連得上就當它活著。

Redis 有自己的協定，TCP 連上不代表 Redis 有回應，用 exec 執行 redis-cli ping，回 PONG 才算真的好。

Task Runner 呢？要不要 Probe？Task Runner 是 worker，沒人連它、沒對外服務。liveness 可以設（解死鎖），readiness 沒意義（readiness 是控制流量進來的，Task Runner 根本沒流量進來）。所以通常 worker 只需要 liveness 或乾脆不設。

這個判斷原則你記住——Probe 不是每個 Pod 都要配滿，是看它的角色決定要哪幾種。

[▶ PPT 29/47：Backend 四個重點]`,
  },

  // ── [29/47] Backend 四個重點 ──
  {
    title: '[29/47] Backend 四個重點',
...
[▶ PPT 30/47：Backend 驗證]`,
  },

  // ── [30/47] Backend 驗證 ──
  {
    title: '[30/47] Backend 驗證 /health',
...
[▶ PPT 31/47：資料層 checklist]`,
  },

  // ── [31/47] 資料層 checklist ──
  {
    title: '[31/47] 資料層告一段落 — 來對一遍',
    subtitle: '七項都綠才繼續往下',
    section: '7-9 第二段：資料層',
    duration: '2',
    content: (
...
[▶ PPT 33/47：Frontend]`,
  ...
  // ── [33/47] Frontend ──
  {
    title: '[33/47] Frontend — Deployment + Service',
...
[▶ PPT 34/47：Task Runner 為什麼沒 Service]`,
  },

  // ── [34/47] Task Runner 用 Deployment ──
  {
    title: '[34/47] Task Runner 用 Deployment 就好 — 兩個判斷',
...
[▶ PPT 35/47：Task Runner YAML]`,
  },

  // ── [35/47] Task Runner YAML ──
  {
    title: '[35/47] Task Runner — Deployment YAML',
...
[▶ PPT 36/47：CronJob YAML]`,
  },

  // ── [36/47] CronJob YAML ──
  {
    title: '[36/47] CronJob — 定時排程觸發',
...
[▶ PPT 38/47：concurrencyPolicy]`,
  },

  // ── [38/47] concurrencyPolicy ──
  {
    title: '[38/47] concurrencyPolicy 三個選項',
...
[▶ PPT 39/47：Ingress k3s Traefik]`,
  },

  // ── [39/47] Ingress Traefik ──
  {
    title: '[39/47] Ingress — k3s 用 Traefik',
    subtitle: '不是 nginx',
    section: '7-9 第三段：應用層',
    duration: '2',
    content: (
...
[▶ PPT 40/47：Middleware + Ingress YAML]`,
  },

  // ── [40/47] Middleware + Ingress YAML ──
  {
    title: '[40/47] Middleware + Ingress YAML',
