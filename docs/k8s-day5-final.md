...
這堂課的 Lab 用 minikube 就能完成。如果你想看到 Pod 真的分散跑在不同 Node 上，可以跟著下一段把環境升級成 k3s 多節點叢集；如果你的電腦記憶體不夠（需要至少 8GB），跳過也完全不影響學習，minikube 一樣能做所有實作。

---

## 第 1.5 頁 | （可選）多節點環境搭建：Multipass + k3s（15min）

### PPT 上的內容

**（可選章節 — 記憶體不夠可跳過，minikube 一樣能完成所有 Lab）**
...
好，在進入 Deployment 之前，我們先來看一個可選的環境升級。如果你的電腦記憶體不到 8GB，或者你只是想趕快學 Deployment，可以直接跳到下一段，minikube 一樣能做所有實作。

上一堂課我們用的是 minikube，它很方便、一行指令就能啟動，但有一個限制 — 它只有一個 Node。所有的 Pod 都跑在同一台機器上，你看不到「Pod 分散到不同 Node」的效果。

如果你想體驗真正的多節點叢集，可以升級到 k3s。k3s 是 Rancher Labs 開源的一個輕量版 Kubernetes，功能跟 K8s 一樣但安裝超級快、資源佔用少很多。名字為什麼叫 k3s？因為 K8s 有 8 個字母（K-u-b-e-r-n-e-t-e-s），k3s 只有 3 個字母，代表它砍掉了很多肥肉，但核心功能全部保留。
