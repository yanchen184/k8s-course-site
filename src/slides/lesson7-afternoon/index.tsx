...
[▶ PPT 46/54：先把 image 放進每台 node]`,
  ...
  user@192.168.56.10:~/Downloads/`}</code></pre>
            <p className="text-slate-400 mt-1">把帳號和 IP 換成自己的 VM。</p>
          </div>
        </div>
...
kubectl get storageclass`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener/00-namespace.yaml
kubectl apply -f apps/k8s/url-shortener/01-secret.yaml
kubectl apply -f apps/k8s/url-shortener/02-configmap.yaml`}</code></pre>
          </div>
        </div>
      ...
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener-local/03-postgres.yaml
...
kubectl get pod postgres-0 -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener-local/04-migrate-job.yaml
      ...
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener-local/05-api.yaml
...
kubectl get pods -l app=url-api -n url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener-local/06-frontend.yaml
kubectl get deploy,svc -n url-shortener
      ...
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener/07-hpa.yaml
kubectl get hpa -n url-shortener
kubectl describe hpa url-api-hpa -n url-shortener`}</code></pre>
          </div>
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            <p className="text-cyan-300 font-semibold">Step 8：Ingress + hosts</p>
            <pre className="bg-slate-950 text-slate-100 p-2 rounded mt-2 overflow-x-auto"><code>{`kubectl apply -f apps/k8s/url-shortener/08-ingress.yaml
...
Add-Content -Path "C:\\Windows\\System32\\drivers\\etc\\hosts" \`
  -Value "<NODE-IP> short.local"`}</code></pre>
            <p className="text-slate-400 mt-2">Windows 瀏覽器要開 short.local，就改 Windows hosts。</p>
          </div>
        </div>
...
kubectl get namespace url-shortener`}</code></pre>
          ...
          <div className="bg-slate-800/40 border border-slate-700 p-3 rounded">
            ...
            <pre className="bg-slate-950 text-slate-100 p-2 rounded overflow-x-auto"><code>{`helm install url-shortener ./apps/helm/url-shortener \\
  -n url-shortener \\
  --create-namespace \\
...
清理指令是 kubectl delete namespace url-shortener，然後用 kubectl get namespace url-shortener 確認 namespace 消失。這會刪掉剛剛測試用的 PostgreSQL PVC 和短網址資料；但不會刪掉已經 import 到 k3s node 的 images，所以 Helm local workflow 還是可以用。
