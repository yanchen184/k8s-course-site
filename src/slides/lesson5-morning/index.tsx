...
【③ 題目】
1. Master Node 四個元件的名稱和各自職責？
2. kubelet 和 kube-proxy 的差異是什麼？
3. 你執行 kubectl apply -f pod.yaml 後，指令依序經過哪些元件？

【④ 解答】
1. apiserver（唯一入口）/ etcd（資料庫）/ scheduler（調度決策）/ controller-manager（確保實際=期望）
2. kubelet 負責啟動容器（跟 Container Runtime 互動）；kube-proxy 負責網路規則（讓 Service 能轉發流量）
3. kubectl → apiserver → etcd（寫入期望狀態）→ scheduler（選 Node）→ 目標 Node 的 kubelet（啟動容器）
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="bg-amber-900/30 border border-amber-500/40 p-3 rounded-lg">
          <p className="text-amber-400 font-semibold text-sm mb-2">Docker 對照 + HPA 預告（第七堂）</p>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="bg-slate-900/40 p-2 rounded text-center">
              <p className="text-slate-400 mb-1">Docker Compose</p>
              <code className="text-slate-300">--scale web=10</code>
              <p className="text-red-400 mt-1">只能單機</p>
            </div>
            <div className="bg-slate-900/40 p-2 rounded text-center">
              <p className="text-slate-400 mb-1">kubectl scale</p>
              <code className="text-green-400">--replicas=10</code>
              <p className="text-green-400 mt-1">跨 Node 分散</p>
            </div>
            <div className="bg-slate-900/40 p-2 rounded text-center">
              <p className="text-slate-400 mb-1">HPA（第七堂）</p>
              <code className="text-blue-300">CPU &gt; 70% 自動加</code>
              <p className="text-blue-400 mt-1">全自動</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-slate-900/70 p-2 rounded text-xs font-mono">
              ...
              <p className="text-slate-300">nginx-deploy-xxx-aaa &nbsp;Running &nbsp;k3s-worker1</p>
              <p className="text-slate-300">nginx-deploy-xxx-bbb &nbsp;Running &nbsp;k3s-worker2</p>
              <p className="text-slate-300">nginx-deploy-xxx-ccc &nbsp;Running &nbsp;k3s-master</p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="bg-slate-900/80 p-3 rounded-lg text-xs font-mono leading-5">
            ...
            <div className="pl-4 bg-red-900/30 rounded px-1"><span className="text-purple-400">replicas</span><span className="text-slate-300">: </span><span className="text-red-400 font-bold">1</span> <span className="text-red-500 text-xs">← ?</span></div>
            ...
            <div className="pl-10 bg-red-900/30 rounded px-1"><span className="text-red-400 font-bold">app: backend</span> <span className="text-red-500 text-xs">← ?</span></div>
...
[▶ 下一頁]`,
  ...
  // ── 5-7（1/2）：滾動更新 + 回滾操作示範 ──
  {
    ...
    subtitle: 'nginx-deploy 完整流程：更新 → 觀察 → 補記錄 → 故意失敗 → 回滾',
    section: '5-7：滾動更新實作',
    duration: '8',
    content: (
      <div className="space-y-3">
  ...
  // ── 5-10（1/2）：自我修復實測 + Labels 操作 ──
  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="bg-slate-800/50 p-3 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">Step 3 — 批次刪除 + describe 看 Selector</p>
          <div className="bg-slate-900/60 p-2 rounded font-mono text-xs mb-1">
...

  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="bg-amber-900/30 border border-amber-500/40 p-3 rounded-lg">
          <p className="text-amber-400 font-semibold mb-2">上午小測驗（5 題）</p>
          <div className="space-y-1.5 text-xs text-slate-300">
            <div className="flex gap-2">
              <span className="text-amber-400 font-bold shrink-0">Q1</span>
              <span>nginx:1.25 → 1.28 的完整流程（至少 3 個指令，依序排列）</span>
            </div>
            <div className="flex gap-2">
              <span className="text-amber-400 font-bold shrink-0">Q2</span>
              <span><code className="text-green-400">rollout undo</code> 連跑兩次，最後停在哪個版本？為什麼？</span>
            </div>
            <div className="flex gap-2">
              <span className="text-amber-400 font-bold shrink-0">Q3</span>
              <span>這個 YAML 有 bug，找出來並說明原因：<br /><code className="text-red-400">selector.matchLabels.app=web</code> 但 <code className="text-red-400">template.labels.app=website</code></span>
            </div>
            <div className="flex gap-2">
              <span className="text-amber-400 font-bold shrink-0">Q4</span>
              <span>一次刪掉 10 個 <code className="text-green-400">app=api</code> 的 Pod，並驗證 Deployment 已補回來</span>
            </div>
            <div className="flex gap-2">
              <span className="text-amber-400 font-bold shrink-0">Q5</span>
              <span><code className="text-green-400">pod-template-hash</code> 是什麼？是誰加的？可以手動刪除嗎？</span>
            </div>
          </div>
        </div>
