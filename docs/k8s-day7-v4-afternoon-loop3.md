...
> ▶ **PPT 3/45：12 個 K8s 組件 — 每個用在哪裡、為什麼**
...
> ▶ **PPT 4/45：五個判斷心法 — 接下來反覆用到**
> 這張先不細講，接下來 13 步會反覆用到這五條判斷。看到一次就對應一次。
...
> ▶ **PPT 5/45：Step 1：Namespace — 隔離這套系統**
...
> ▶ **PPT 6/45：複習：Secret vs ConfigMap — 分界線只有一條**
> 先停在這張，講完分界線再翻下一頁看 YAML。
...
> ▶ **PPT 9/45：複習：StatefulSet — 為什麼不用 Deployment？**
...
> ▶ **PPT 10/45：Step 4：PostgreSQL StatefulSet**
...
> ▶ **PPT 11/45：Headless Service — StatefulSet 的搭檔**
...
> ▶ **PPT 12/45：Redis — 為什麼這裡不用 StatefulSet？**
...
> ▶ **PPT 13/45：Step 5：Redis Deployment + ClusterIP Service**
...
> ▶ **PPT 15/45：複習：Job — 為什麼一次性任務不用 Deployment？**
...
> ▶ **PPT 16/45：Step 6：DB Migration Job**
...
> ▶ **PPT 17/45：複習：RBAC — 給 Pod 操作 K8s API 的身份**
...
> ▶ **PPT 18/45：Step 7：Backend RBAC — 只給 get/list configmaps**
...
> ▶ **PPT 19/45：複習：readinessProbe — Pod Running ≠ 可以接流量**
> ▶ **PPT 20/45：觀念：envFrom vs env — 為什麼 Secret 不能用 envFrom？**
> 這兩張是講 Backend YAML 前的鋪陳，翻到 PPT 20 再開始寫 YAML。

> ▶ **PPT 21/45：Step 8：Backend Deployment + Service**
...
> ▶ **PPT 22/45：驗證 RBAC 真的在擋 — 拿掉 Role 會怎樣**
...
> ▶ **PPT 23/45：Step 9：Frontend Deployment + Service**
...
> ▶ **PPT 24/45：Step 10：Task Runner — 為什麼不需要 Service？**
...
> ▶ **PPT 25/45：為什麼 Task Runner 不用 DaemonSet？**
...
> ▶ **PPT 26/45：複習：CronJob — 定時觸發**

CronJob 每分鐘掃一次資料庫，把到期的任務撈出來丟進 Queue：

> ▶ **PPT 27/45：Step 11：CronJob Task Scheduler**
...
> ▶ **PPT 28/45：複習：Ingress — 為什麼不用 NodePort？**
...
> ▶ **PPT 29/45：Step 12：Ingress（nginx）— path rewrite**
...
> ▶ **PPT 30/45：複習：HPA — 自動擴縮**
> ▶ **PPT 31/45：Step 13：HPA — Backend 2~10 副本**
> 先翻 30 講觀念（上午學過的快速 recap），再翻 31 看實際 YAML。
...
> ▶ **PPT 33/45：影片 7-10：QA + Helm + 學員題目**

好，先回答幾個剛才操作過程中常見的問題。

> ▶ **PPT 34/45：QA #1：PVC 一直是 Pending 怎麼辦？**
...
> ▶ **PPT 35/45：QA #2：Pod 一直 CrashLoopBackOff**
...
> ▶ **PPT 36/45：QA #3：HPA TARGETS 一直顯示 unknown**
...
> ▶ **PPT 37/45：QA #4：Task Runner 不消費 Queue 的任務**
...
> ▶ **PPT 38/45：Helm 示範 — 同一套系統，用 Helm 管理**
...
> ▶ **PPT 41/45：Step 3 — helm rollback**
...
> ▶ **PPT 42/45：Step 4 — --set 覆蓋單一 value**
