...
[▶ 下一頁]`,
  ...
  // ── 7-4 學員實作解答：Probe ──
  {
    title: '解答：livenessProbe 讓 RESTARTS 加 1',
    subtitle: 'Loop 1 完成',
    section: '7-4：回頭操作 Loop 1',
    duration: '3',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">必做解答：livenessProbe YAML</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            <p className="text-slate-500">containers:</p>
            <p>{'- '}name: nginx</p>
            <p>{'  '}image: nginx:1.27</p>
            <p className="text-green-400">{'  '}livenessProbe:</p>
            <p>{'    '}httpGet:</p>
            <p>{'      '}path: /</p>
            <p>{'      '}port: 80</p>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">觸發重啟：刪掉 index.html</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            <p className="text-slate-500"># 取得 Pod 名稱</p>
            <p>kubectl get pods -l app=nginx-probe</p>
            <p className="text-slate-500 mt-1"># 刪除 index.html → 讓 httpGet / 回 403/404 → 觸發重啟</p>
            <p>kubectl exec {'<pod>'} -- rm /usr/share/nginx/html/index.html</p>
            <p className="text-slate-500 mt-1"># 觀察 RESTARTS 增加</p>
            <p>kubectl get pods -l app=nginx-probe -w</p>
            <p className="text-slate-500 mt-1"># 清理</p>
            <p>kubectl delete deployment nginx-probe-demo</p>
          </div>
        </div>
        ...
        <div className="bg-green-900/30 border border-green-500/30 p-3 rounded-lg">
          ...
          <p className="text-xs text-slate-300 mt-1">RESTARTS 從 0 → 1（livenessProbe 偵測到失敗後自動重啟）</p>
        </div>
      </div>
    ),
    notes: `好，來看解答。

必做題的關鍵在 livenessProbe 那幾行。httpGet path 是斜線，port 是 80。initialDelaySeconds 設 5，給 nginx 一點啟動時間。periodSeconds 10 表示每 10 秒檢查一次。failureThreshold 3 表示連續 3 次失敗才重啟。

觸發的方式：kubectl exec 進去把 index.html 刪掉。nginx 的根路徑就會回 403 或 404，livenessProbe 判定失敗，連續 3 次後重啟 Pod。你用 kubectl get pods -w 監看，會看到 RESTARTS 從 0 變成 1。

清理指令：kubectl delete deployment nginx-probe-demo。

如果你的 RESTARTS 沒有加，最常見的原因是 failureThreshold 還沒達到，或者 path 設錯了，probe 根本沒有在檢查正確的路徑。 [▶ 下一頁]`,
  },
...
[▶ 下一頁]`,
  ...
  // ── 7-7 學員實作解答：Resource limits ──
  {
    title: '解答：OOMKilled → 改 limits → Running',
    subtitle: 'Loop 2 完成',
    section: '7-7：回頭操作 Loop 2',
    duration: '3',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">必做解答：oom-demo Pod YAML</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            <p className="text-slate-500">containers:</p>
            <p>{'- '}name: stress</p>
            <p>{'  '}image: polinux/stress</p>
            <p>{'  '}args: ["--vm", "1", "--vm-bytes", "256M"]</p>
            <p className="text-green-400">{'  '}resources:</p>
            <p>{'    '}limits:</p>
            <p>{'      '}cpu: "200m"</p>
            <p className="text-red-400">{'      '}memory: "128Mi"{'  '}# 故意設太小 → OOMKilled</p>
          </div>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">觀察 OOMKilled → 修正 → Running</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            <p className="text-slate-500"># 部署後觀察狀態（OOMKilled）</p>
            <p>kubectl get pod oom-demo</p>
            <p className="text-slate-500 mt-1"># 看詳細原因</p>
            <p>kubectl describe pod oom-demo</p>
            <p className="text-slate-500 mt-1"># 修正：把 memory limits 改成 512Mi，重新 apply</p>
            <p className="text-green-400">{'      '}memory: "512Mi"{'  '}# 改大 → Running</p>
            <p className="text-slate-500 mt-1"># 清理</p>
            <p>kubectl delete pod oom-demo</p>
          </div>
        </div>
        ...
        <div className="bg-green-900/30 border border-green-500/30 p-3 rounded-lg">
          <p className="text-green-400 font-semibold text-sm">預期結果</p>
          <p className="text-xs text-slate-300 mt-1">128Mi → OOMKilled / CrashLoopBackOff；改成 512Mi → Running</p>
        </div>
      </div>
    ),
    notes: `來看解答。

oom-demo 的重點是 resources.limits.memory 設了 128Mi，但 stress 工具吃了 256Mi。超過 limits 就被 OOMKilled。kubectl describe pod oom-demo 的 Events 欄位會看到 OOMKilled 的記錄，Last State 欄位也會顯示 OOMKilled。

修正方法就是把 memory limits 改成 512Mi，讓 Pod 有足夠的空間。改完重新 apply，Pod 就會是 Running。

清理：kubectl delete pod oom-demo。

關鍵觀念：limits 是天花板，超過 memory limits 就被作業系統殺掉，沒有緩衝，直接 OOMKilled。所以設 limits 的時候要有餘裕，一般建議程式實際用量的 1.5 到 2 倍。 [▶ 下一頁]`,
  },
...
[▶ 下一頁]`,
  ...
  // ── 7-10 學員實作解答：HPA ──
  {
    ...
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">必做解答：建立 HPA</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            ...
            <p className="text-slate-500"># nginx-resource-demo-hpa  Deployment/nginx.. 5%/50%    2        5</p>
          </div>
        </div>

        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">挑戰解答：壓測讓 REPLICAS 上升</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-2 rounded space-y-1">
            <p className="text-slate-500"># 開一個 busybox 一直打請求</p>
            <p>kubectl run -it load-gen --image=busybox --rm -- sh</p>
            <p className="text-slate-500 mt-1"># 在 busybox 裡面跑：</p>
            <p>while true; do wget -q -O- http://nginx-resource-svc; done</p>
            <p className="text-slate-500 mt-1"># 另一個 terminal 觀察 HPA</p>
            <p>kubectl get hpa -w</p>
            <p className="text-slate-500 mt-1"># 清理</p>
            <p>kubectl delete hpa nginx-resource-demo-hpa</p>
          </div>
        </div>

        <div className="bg-green-900/30 border border-green-500/30 p-3 rounded-lg">
          <p className="text-green-400 font-semibold text-sm">預期結果</p>
          <p className="text-xs text-slate-300 mt-1">壓測後 TARGETS 超過 50% → REPLICAS 從 2 自動增加到 3、4、5</p>
        </div>
      </div>
    ),
    notes: `來看解答。
...
清理：kubectl delete hpa nginx-resource-demo-hpa。

上午三個 Loop 全部完成。Probe 管健康、Resource limits 管資源、HPA 管容量。吃個午飯，下午繼續。 [▶ 下一頁]`,
  },
]
