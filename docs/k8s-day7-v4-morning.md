...
指令：kubectl autoscale deployment nginx-resource-demo --min=2 --max=10 --cpu-percent=50
...
指令：kubectl autoscale deployment nginx-resource-demo --min=2 --max=10 --cpu-percent=30
...
下午我們進入最後的部分：從零建一套完整的系統，把四堂課學的所有組件全部串在一起。今天的系統是一個任務排程系統，前後端加資料庫加 Redis，用到 Deployment、StatefulSet、PVC、ConfigMap、Secret、Service、Ingress、HPA、RBAC、CronJob。幾乎把課程學過的所有東西都用上了。
