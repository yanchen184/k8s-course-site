...
下一堂不再繼續講理論，而是直接進入實戰。會做三件事：

1. 打包一個 Hello World API，確認 Dockerfile 基礎能獨立完成
2. 把它改寫成生產級版本，補上 `.dockerignore`、multi-stage、非 root 與 healthcheck
3. 做一次 Code Review，練習從症狀回推 Dockerfile 問題

今天先記住三個最重要的檢查點：
- `.dockerignore` 要先補齊，不要把 `.env`、`node_modules`、`.git` 打進 Image
- Multi-stage 要把建構環境和運行環境拆開
- 生產環境的 Dockerfile 至少要考慮非 root、healthcheck、映像大小與發佈流程
