...
# 影片 6-3（0）：今天的 Demo App 介紹（~2min）

## 本集重點

在開始 Ingress 實作之前，先介紹今天整堂課要用的 demo app。

## 逐字稿

在開始 Ingress 實作之前，先介紹今天整堂課要用的 demo app。

這個 app 叫做 yanchen184/k8s-demo-app。它就是一個很簡單的 PHP 應用，收到 HTTP 請求，就把自己的狀態印出來。印四行：

指令：curl http://\<NODE-IP\>/frontend

```
Server: 10.42.x.x:80 (frontend-deploy-abc)
Message: Hello from frontend
Username: admin
Password: mypassword
```

Server 是自己的 Pod IP 和名稱，Message 是 MESSAGE 這個環境變數的值，Username 是 USERNAME 的值，Password 是 PASSWORD 的值。
...
更重要的是，這個 app 貫穿今天整堂課：

- Ingress：curl 不同路徑，看到不同的 Message，確認路由正確
- ConfigMap：把 USERNAME 存進 ConfigMap，curl 看到 Username 欄位有沒有套進去
- Secret：把 PASSWORD 存進 Secret，curl 看到 Password 欄位
- Helm：values.yaml 換一個值，curl 驗證整套都跟著變了

同一個 app，從頭用到尾。你只要記得 curl 看四行輸出就好。

IMAGE 不變，環境變數不同，行為就不同。這本身就是 K8s 的核心概念之一：把設定和程式分開。等一下學 ConfigMap 和 Secret 的時候，你會更深刻體會這件事。
...
kubectl get svc demo   # 看 NodePort
...
- 部署兩個 Service：frontend（yanchen184/k8s-demo-app）+ api（yanchen184/k8s-demo-app）
- 寫 ingress.yaml（path-based）：/frontend → frontend-svc，/api → api-svc
- curl NODE-IP/frontend → 看到 Message: Hello from frontend；curl NODE-IP/api → 看到 Message: Hello from api
- 改成 host-based：www.myapp.local → frontend，api.myapp.local → api
- 修改 /etc/hosts 加入域名映射
- curl www.myapp.local/frontend → 看到 Message: Hello from frontend；curl api.myapp.local/api → 看到 Message: Hello from api
...
curl http://<NODE-IP>/frontend   # → Server: 10.42.x.x:80 / Message: Hello from frontend
curl http://<NODE-IP>/api        # → Server: 10.42.x.x:80 / Message: Hello from api
```
...
curl http://www.myapp.local/frontend   # → Message: Hello from frontend
curl http://api.myapp.local/api        # → Message: Hello from api
```
...
**學員實作：**
- 必做：部署兩個服務 + path routing Ingress → curl 驗證（看到 Message: Hello from frontend 和 Message: Hello from api）
- 挑戰：改成 host routing + 第三個服務（admin，用 yanchen184/k8s-demo-app，MESSAGE=Hello from admin）
...
Controller 確認了，來部署我們的應用。我準備了一個 ingress-basic.yaml，裡面包含四個東西：frontend 的 Deployment 和 Service，用 yanchen184/k8s-demo-app image，MESSAGE 設為 Hello from frontend；api 的 Deployment 和 Service，用 yanchen184/k8s-demo-app image，MESSAGE 設為 Hello from api；還有一個 Ingress 規則，path-based routing。
...
kubectl describe ingress app-ingress。看 Rules 那一段，會列出你定義的路由規則。斜線 frontend 對應 frontend-svc 的 80 Port，斜線 api 對應 api-svc 的 80 Port。
...
你應該看到類似這樣的輸出：Server: 10.42.x.x:80 / Message: Hello from frontend。

curl http://192.168.1.100/api

你應該看到類似這樣的輸出：Server: 10.42.x.x:80 / Message: Hello from api。
...
curl http://www.myapp.local/frontend
...
學員實作時間。必做題：照著剛才的步驟，部署兩個服務加上 path routing 的 Ingress，用 curl 驗證斜線 frontend 看到 Message: Hello from frontend、斜線 api 看到 Message: Hello from api。挑戰題：改成 host routing，再加一個第三個服務，用 yanchen184/k8s-demo-app 加上 MESSAGE=Hello from admin 做一個 admin 服務，admin.myapp.local 導到它。記得 /etc/hosts 也要加上 admin.myapp.local。
...
Path-based routing 的部分。kubectl apply -f ingress-basic.yaml，一次建好 Deployment、Service、Ingress。kubectl get ingress 確認規則有建好。curl NODE-IP/frontend 看到 Message: Hello from frontend，curl NODE-IP/api 看到 Message: Hello from api。

Host-based routing 的部分。kubectl apply -f ingress-host.yaml。改 /etc/hosts 加上域名映射。curl www.myapp.local/frontend 看到 Message: Hello from frontend，curl api.myapp.local/api 看到 Message: Hello from api。
...
- Pod 用 envFrom 讀 ConfigMap → curl 看到注入的值
- Volume 掛載 nginx.conf → 修改 ConfigMap → 等自動更新（subPath 不更新的坑）
- Secret：kubectl create secret generic
- 整合：Deployment 同時 envFrom ConfigMap + Secret，curl 看到完整輸出
- 驗證方式：`curl http://<NODE-IP>/frontend`

**Step 1：ConfigMap — 環境變數注入**
```bash
kubectl apply -f configmap-literal.yaml
# ConfigMap 存 MESSAGE: "Hello from ConfigMap"、USERNAME: "admin"
# Deployment 用 envFrom 引用，Service 是 frontend-svc
...
**Step 4：整合 — Deployment 同時 envFrom ConfigMap + Secret**
```bash
kubectl apply -f secret-db.yaml
# secret-db.yaml：Secret 存 PASSWORD: "mypassword"
# Deployment 同時 envFrom ConfigMap + Secret，Service 是 frontend-svc
...
**學員實作：**
- 必做：建 ConfigMap（MESSAGE + USERNAME）+ Secret（PASSWORD）→ Deployment 用 envFrom 引用兩個 → curl 看到完整輸出
...
這個 YAML 裡面有一個 ConfigMap 叫 app-config，存了 MESSAGE 等於 "Hello from ConfigMap"，還有 USERNAME 等於 "admin" 兩個設定。還有一個 Deployment，用 envFrom 把整個 ConfigMap 注入為環境變數，Service 名稱是 frontend-svc。

等 Pod 跑起來，我們用 curl 驗證一下。先拿 Node 的 IP。
...
你應該看到這樣的輸出。Server 後面是 Pod 的 IP，Message 是 Hello from ConfigMap，Username 是 admin。ConfigMap 的值成功注入，而且透過 HTTP 回應直接看到了。

現在來做一個重要的實驗。我們改 ConfigMap，看看 Pod 會不會自動更新。

kubectl edit configmap app-config

找到 MESSAGE，從 Hello from ConfigMap 改成 Hello Updated，存檔。

現在再 curl 一下。你猜結果是什麼？

還是 Hello from ConfigMap。沒有更新。
...
等新的 Pod 起來，再 curl，現在才是 Hello Updated。
...
第四步，整合。Deployment 同時 envFrom ConfigMap 和 Secret。

kubectl apply -f secret-db.yaml

這個 YAML 裡面有一個 Secret 存了 PASSWORD 等於 mypassword，一個 Deployment 用 envFrom 同時引用 ConfigMap 和 Secret，把三個值全部注入為環境變數，Service 名稱還是 frontend-svc。
...
你現在應該看到四行輸出。Server 是 Pod 的 IP，Message 是 Hello from ConfigMap，Username 是 admin，Password 是 mypassword。

ConfigMap 的 MESSAGE 和 USERNAME 都進來了，Secret 的 PASSWORD 也進來了。三個值同時靠 envFrom 注入，Pod 拿到之後透過 HTTP 回應秀出來。

這就是 ConfigMap 和 Secret 搭配使用的標準模式。非敏感的設定放 ConfigMap，密碼放 Secret，Pod 用 envFrom 一次把兩個都引用進來。驗證的時候也不用進容器，一個 curl 就看到所有東西。

學員實作時間。必做題：建一個 ConfigMap 存 MESSAGE 和 USERNAME，建一個 Secret 存 PASSWORD，然後部署 Deployment 用 envFrom 同時引用這兩個，curl 看到完整輸出包含 Message、Username、Password 三行。挑戰題：用 Volume 掛載方式自訂一個 nginx.conf，然後修改 ConfigMap，等 30 到 60 秒觀察檔案是否自動更新。
...
- 帶做 ConfigMap 環境變數 + Volume 掛載 + Secret 整合
- 驗證方式：curl http://<NODE-IP>/frontend 看到完整輸出
...
ConfigMap 環境變數注入。kubectl apply -f configmap-literal.yaml。curl http://<NODE-IP>/frontend 看到 Message 和 Username 有注入。kubectl edit configmap 改 MESSAGE，Pod 不會自動更新，curl 還是舊值。kubectl rollout restart，重啟之後 curl 才拿到新值。
...
整合。kubectl apply -f secret-db.yaml。等 Pod 起來，curl http://<NODE-IP>/frontend 看到 Message、Username、Password 全部出現。
