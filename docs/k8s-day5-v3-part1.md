...
學員實作題目：
- 必做 1：建 httpd Deployment replicas:2 → scale 到 5 → kubectl get pods -o wide 觀察分散 → scale 回 1
- 必做 2：scale 到 0 → kubectl get deploy 看到 READY 0/0（Pod 全砍但 Deployment 還在）→ scale 回 3 → Pod 又回來了。這在維護服務的時候很有用，暫停不用刪 Deployment
- 必做 3（預告體驗）：開兩個終端
  - 終端 1：kubectl get pods -w（持續觀察，不要關）
  - 終端 2：快速連打，每次等 5 秒看終端 1 的變化
    - kubectl scale deployment nginx-deploy --replicas=5
    - kubectl scale deployment nginx-deploy --replicas=8
    - kubectl scale deployment nginx-deploy --replicas=10
    - kubectl scale deployment nginx-deploy --replicas=3
  - 在終端 1 看到 Pod 快速增加、快速減少
  - 這就是第七堂 HPA 自動幫你做的事：流量大自動加 Pod，流量小自動砍 Pod
