...
建一個新檔案叫 pod-crash.yaml。apiVersion v1，kind Pod，metadata 底下 name 寫 crash-pod。spec 底下 containers 列表的第一個項目，name 寫 crash-test，image 寫 nginx:1.27。到這裡都沒問題，Image 名字是對的，一定拉得到。但是我要加一個 command 欄位。command 是一個陣列，裡面放三個字串：第一個是 "/bin/sh"，第二個是 "-c"，第三個是 "echo hello && exit 1"。

這個 command 的意思是什麼？容器啟動之後，不要執行 nginx 的預設指令，而是啟動一個 shell，先印出 hello，然後以 exit 1 的錯誤狀態退出。
...
感受一下這個退避的節奏。，感受一下這個退避的節奏。等到後面可能要等好幾分鐘才會重啟一次。這就是指數退避策略在起作用。
