...
Kubernetes，簡稱 K8s，K 和 s 之間有 8 個字母所以叫 K8s。它原本是 Google 內部的系統 Borg。Google 從 2003 年開始用 Borg 管容器，管了超過十五年。規模是什麼等級？Google 自己說過，他們每週啟動超過二十億個容器。二十億，不是二十個。這個系統經過了十幾年、數十億容器的驗證。2014 年，Google 把 Borg 的設計理念拿出來，用 Go 語言重新實作並開源，這就是 Kubernetes。後來捐給了 CNCF，雲端原生運算基金會。到今天，K8s 是容器編排領域的行業標準，AWS、GCP、Azure、阿里雲、騰訊雲，全部都提供 K8s 的託管服務。
...
Docker 的世界裡，跑一個容器就是 docker run nginx，一行指令。K8s 不一樣，K8s 不直接管容器，它管的最小單位叫做 Pod。

Pod 是什麼？你就把它想成容器的一層包裝。就像便利商店的飯糰，米飯和餡料是容器本身，外面那層塑膠膜就是 Pod。一個 Pod 裡面可以放一個或多個容器，這些容器共享同一個網路和儲存空間。也就是說，同一個 Pod 裡的容器可以用 localhost 互相溝通，就像跑在同一台機器上一樣。

那為什麼要多包這一層？因為有些場景下，主程式需要搭配一個輔助程式一起跑，而且它們之間非常緊密。舉個例子，你有一個 API 容器把日誌寫到檔案裡，旁邊放一個日誌收集的容器讀這個檔案，然後把日誌傳到 Elasticsearch。這兩個容器需要共享同一個目錄，放同一個 Pod 裡最方便。這種模式叫 Sidecar，邊車模式，就像摩托車旁邊掛一個邊車，主容器是摩托車，輔助容器是邊車。

不過絕大多數情況下，一個 Pod 就放一個容器。你現在就把 Pod 當成容器來理解就好，只是 K8s 所有的調度、管理、監控都是以 Pod 為單位的。Pod 是 K8s 的原子單位，最小的不可再分的調度單位。

順便提一下 Node。Node 就是一台機器，實體機或虛擬機。一個叢集有很多 Node，一個 Node 上跑很多 Pod。Node 是機器，Pod 是跑在機器上的容器包裝。

好，我們知道怎麼在 K8s 跑容器了。docker run nginx 在 K8s 就是建一個 nginx 的 Pod。
