...
好，第一個錯誤修好了。先清理一下，kubectl delete pod broken-pod。 [▶ 下一頁]`,
  },
...
kubectl delete pod sidecar-pod`,
    notes: `好，YAML 寫完了，存檔。部署它，kubectl apply -f pod-sidecar.yaml。
...
kubectl delete pod mysql-pod`,
    notes: `好，我們來動手做。大家把終端機打開，進到 ~/k8s-labs。
...
挑戰題給有餘力的同學。第一個是跑一個 Redis Pod，image 用 redis:7，Redis 不需要設環境變數就能啟動。跑起來之後用 kubectl exec -it redis-pod 兩個減號 redis-cli ping，如果回 PONG 就成功了。第二個是跑一個 Python HTTP Server，image 用 python:3.12，但你要在 YAML 裡面加 command 欄位覆蓋掉預設的啟動指令，設成 python -m http.server 8000。然後 port-forward 到 8000，瀏覽器會看到一個目錄列表的頁面。 [▶ 下一頁 — 學員開始做，你去巡堂]`,
  ...
  // ── 4-23 總結（1/2）：今日知識清單 + Docker 對照表 ──
  {
    ...
    notes: `好，來回顧今天的內容。上午的部分我們在影片 4-11 已經回顧過了，這裡一句話帶過：上午我們從 Docker 的五個瓶頸出發，一路認識了 K8s 的八個核心概念和 Master-Worker 架構，最後動手跑了第一個 Pod 的 CRUD。
...
還有一個 Docker 對照表，大家截圖存起來當速查卡。docker run 對應 kubectl apply，docker run -p 對應 port-forward，docker run -e 對應 env 欄位，docker ps 對應 kubectl get pods，docker logs 對應 kubectl logs，docker exec 對應 kubectl exec，docker stop rm 對應 kubectl delete pod，docker inspect 對應 describe 或 -o yaml，Docker Compose 的 YAML 對應 K8s 的 YAML，Docker Compose 的 environment 對應 K8s 的 env。你會發現，K8s 的很多操作跟 Docker 是一一對應的。你在 Docker 學的東西沒有白學，K8s 只是把同樣的概念放到了一個更大的框架裡面。 [▶ 下一頁]`,
  ...
  // ── 4-23 總結（2/2）：回家作業 + 下堂課預告 ──
  {
    ...
    notes: `回家作業三個。第一，把今天的 Pod 練習從頭到尾做一遍，不看筆記。如果能憑記憶完成整個流程，今天的內容就完全內化了。第二，跑不同的 Image 觀察行為差異。redis 會一直 Running。python:3.12 如果你不給它 command，它會啟動之後直接退出變成 Completed。busybox 也是。想想為什麼。第三，進階作業，跑 MySQL Pod 加環境變數，進去建資料庫。
