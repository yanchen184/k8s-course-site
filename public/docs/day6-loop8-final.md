...
進入這個 section 前，先確認叢集現在的狀態：

```bash
kubectl get all        # 看目前叢集裡有哪些資源跑著
helm list              # 看目前已安裝的 Helm Release
```
...
**Q：為什麼 MySQL 要用 StatefulSet 而不是 Deployment？**

A：Deployment 的 Pod 之間沒有固定身份——刪了建一個新的，名字也不一樣。MySQL 這類資料庫需要固定的 Pod 名稱和穩定的 DNS（mysql-0.mysql.default.svc.cluster.local），而且每個 Pod 要有自己獨立的 PVC（mysql-0 的資料不能跑到 mysql-1 去）。StatefulSet 提供固定序號、獨立 PVC、有序啟動這三個特性，Deployment 全部做不到。

**Q：Helm 和手寫 YAML 的適用場景各是什麼？**

A：手寫 YAML 的情境：需要高度客製化的部署邏輯、或者你在學原理，需要知道每一行在做什麼。套 Helm Chart 的情境：安裝已知的開源軟體（WordPress、Prometheus、Redis），不需要改內部邏輯，只需要調參數——這時候用 Helm 是正確的工程判斷，不是偷懶。

---

### ④ 學員實作

（本節為引導觀摩節，無操作題。實作在 6-24。）
...
**目標：用 Helm 一行安裝 WordPress，親手感受「整套服務一鍵起來」是什麼感覺。**

必做題用 `--set` 方式快速安裝 WordPress + MariaDB，驗收看到瀏覽器歡迎頁。挑戰題改用 `values.yaml` 管理參數並設定自訂域名，進階挑戰驗證資料持久化。
...
照 ② 的步驟執行：`helm repo add` → `helm repo update` → `helm install` → `kubectl get pods -w` → `kubectl get svc` → 瀏覽器驗收。
...
用完整因果鏈回顧第六堂為什麼每個概念會出現，整理今日新增指令清單，對照 Docker → K8s 概念表，最後是回家作業和第七堂預告。
...
```bash
helm list                     # 確認還有哪些 Release 沒清掉
helm uninstall my-blog        # 刪除 WordPress Release
kubectl get pvc               # PVC 不會自動刪，確認一下
kubectl delete pvc --all      # 清掉所有 PVC
kubectl get all               # 確認叢集回到乾淨狀態
```
