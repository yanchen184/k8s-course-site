...
service/my-ingress-ingress-nginx-controller   LoadBalancer   ...
...
```bash
helm install dev-ingress ingress-nginx/ingress-nginx -f values-dev.yaml
```
...
執行：
```bash
helm install my-ingress ingress-nginx/ingress-nginx --set controller.replicaCount=1
```
然後執行：
```bash
...
觀察 upgrade 後 replicaCount 是否被重設回預設值。用 `helm history my-ingress` 確認 revision，然後試試加 `--reuse-values` 有什麼差別。

---

**必做 2：dev / prod 兩套 Ingress**

你的團隊要在同一個叢集跑兩套 Nginx Ingress：
- dev：1 個副本
- prod：2 個副本
...
不帶 `--set controller.replicaCount` 的 upgrade 會讓 replicaCount 被重設回 Chart 的預設值（通常是 1）。`helm history` 看到 REVISION 2。
...
```yaml
# dev-ingress-values.yaml
controller:
  replicaCount: 1
```
```yaml
# prod-ingress-values.yaml
controller:
  replicaCount: 2
```
```bash
helm install dev-ingress ingress-nginx/ingress-nginx -f dev-ingress-values.yaml
helm install prod-ingress ingress-nginx/ingress-nginx -f prod-ingress-values.yaml
