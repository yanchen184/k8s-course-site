...
今天全程用 `yanchen184/k8s-demo-app:latest`，是我自己包的 PHP 小程式，一個 curl 就能看到結果。
...
這四行是今天所有 demo 的觀察視窗：
- **Server** → Pod IP，確認 Ingress 是真的分流到不同 Pod
- **Message** → 來自 `MESSAGE` 環境變數，Ingress 示範時用 ConfigMap 注進來
- **Username** → 來自 `USERNAME` 環境變數，ConfigMap 章節注進來
- **Password** → 來自 `PASSWORD` 環境變數，Secret 章節注進來
...
`-n kube-system`：namespace 是 K8s 用來隔離資源的空間，就像資料夾。系統元件都跑在 `kube-system`，你自己建的資源預設在 `default`。
...
- **`traefik-xxx`**：Ingress Controller 本體，讀 Ingress YAML、做路由
- **`svclb-traefik-xxx`**：k3s 特有的 **Klipper Load Balancer**。標準 K8s 沒有地端 LoadBalancer 實作，k3s 內建 Klipper，用 DaemonSet 在每個 Node 上跑一個 Pod，把 Node 的 80/443 port 接給 Traefik，讓 Traefik 能監聽標準 port。你在雲端（AWS/GCP）看到的 LoadBalancer 就是雲端版的 Klipper。

確認 Traefik 拿到 EXTERNAL-IP：
...
預期輸出：
```
traefik   LoadBalancer   10.43.x.x   192.168.64.10   80:xxxxx/TCP,443:xxxxx/TCP   2d
```

`EXTERNAL-IP`（192.168.64.10）就是你 VM 的 IP，等等 curl 用這個。

`READY 1/1`、`STATUS Running`：可以繼續。
- 沒有輸出 → k3s 沒裝好
- `CrashLoopBackOff` → `kubectl logs -n kube-system <traefik-pod>` 看原因
...
spec:
  ...
  rules:
    - http:
        paths:
          - path: /frontend
            pathType: Prefix         # 前綴匹配，/frontend 開頭都符合
            backend:
              service:
                name: frontend-svc
                port:
