...
你可能會想：k3s 不就是用來建 K8s 叢集的嗎？兩個有什麼差？
...
簡單說：今天的課我們用的還是 k3s，但是把 k3s 叢集 import 進 Rancher 來管。真正的企業環境如果從零建叢集，可能會用 RKE。
...
本節以概念為主，指令集中於 6-21 實作。

---

### ③ 題目

1. k3s 和 RKE 各自適合什麼場景？說出至少兩點差異。
2. 為什麼有了 Rancher GUI，還需要保留 kubectl 的操作能力？

---

### ④ 解答

1. k3s 適合輕量環境、邊緣運算、學習開發——資源佔用低，curl 一行安裝，快速上手。RKE 適合企業生產環境——功能最完整、與 Rancher 深度整合、適合有多個節點和複雜網路需求的環境。

2. kubectl 是自動化的基礎。CI/CD Pipeline 要執行 apply、rollout、scale 這些操作，只能靠指令，不能靠 GUI 點按鈕。腳本、定時任務、GitOps 流程都是靠 kubectl。Rancher 解決的是「人工巡檢和日常操作」，不是「自動化流程」。兩者分工，缺一不可。
...
1. 用 Docker 把 Rancher 跑起來
2. 瀏覽器登入，設定初始密碼
3. 把 k3s 叢集 import 進 Rancher
4. GUI 導覽：看 Deployment、看 Pod 日誌、開 Web Terminal、做 scale
...
- `-p 80:80 -p 443:443`：把主機的 80 和 443 對應到 Rancher 容器的 80 和 443（Rancher Web 介面）
...
> **如果 Port 衝突**：k3s 的 Traefik 可能已經佔了 80 和 443。這種情況改用：
> ```bash
> docker run -d --restart=unless-stopped \
...
5. Rancher 會給你一段 `kubectl apply` 指令，像這樣：
   ```
   kubectl apply -f https://<rancher-IP>/v3/import/xxxxxxxxxxxxxxxx.yaml
   ```
6. 複製這段指令，**回到 k3s master 節點**，執行它
...
以下是 Demo 操作清單（老師邊說邊操作，學員看畫面）：

**① Cluster Dashboard**

點進 `my-k3s-cluster`，看 Overview 頁：
- CPU 使用率、記憶體使用率（百分比 + 實際數值）
- Node 列表：每個 Node 的狀態和資源使用量
- Pod 列表：目前跑了幾個 Pod
...
左側選單「Workloads」→「Deployments」：

- 看到今天建的所有 Deployment（之前幾個 section 建的）
- 每個 Deployment 顯示：名稱、namespace、Pod 數量、Ready 狀態、映像版本

點進某個 Deployment，看到 Pod 列表，點 Pod 名稱右邊的「⋮」→「View Logs」：

- 直接在瀏覽器看 Pod 的 stdout log
- 等同於打 `kubectl logs <pod-name>`，但不用記 Pod 名稱

---

**③ Web Terminal（Execute Shell）**

在 Pod 列表，點某個 Pod 右邊的「⋮」→「Execute Shell」：

- 瀏覽器直接開出 shell，進入容器
- 等同於打 `kubectl exec -it <pod-name> -- /bin/sh`

可以在裡面打指令，確認容器狀態、查環境變數。
...
1. 點 Deployment 名稱進去
2. 右上角有 replicas 數字，點「Edit Config」
3. 找到 Replicas 欄位，直接改數字
4. 點「Save」
5. 回到 Pod 列表，看 Pod 數量變化
...
左側「Service Discovery」→「Services」：
- 所有 Service 列表：ClusterIP、NodePort、LoadBalancer
- 看到 Port 對應關係

左側「Service Discovery」→「Ingresses」：
- 所有 Ingress 列表
- 看到哪些服務對外開放、對應哪個 hostname

---

**⑥ Storage**

左側「Storage」→「PersistentVolumes」：看所有 PV 的狀態和 Reclaim Policy

左側「Storage」→「PersistentVolumeClaims」：看哪些 PVC 已 Bound、哪些 Pending
...
1. 安裝 Rancher → 導入 k3s 叢集，確認叢集狀態變 Active
2. 用 Rancher GUI 找到之前建的某個 Deployment，用 GUI 改 replicas（不用打 kubectl）

**挑戰（有餘力再做）：**

在 Rancher GUI 上從頭建一個新的 Deployment，完全不用 kubectl（Workloads → Deployments → Create）

---

### ③ 題目

1. Rancher 的 `Execute Shell` 功能等同於哪個 kubectl 指令？完整寫出指令格式。
2. 操作題：在 Rancher GUI 上把某個 Deployment 的 replicas 改成 4，然後用 kubectl 確認數量真的改了。
3. 挑戰題：在 Rancher GUI 上建一個用 `nginx:1.25` 映像、replicas=2、名為 `gui-test` 的 Deployment，建完用 `kubectl get deploy gui-test` 確認存在。

---

### ④ 解答

1. `kubectl exec -it <pod-name> -- /bin/sh`（或 `/bin/bash`，看容器裡有什麼 shell）。`-it` 是 interactive + tty，讓終端可以互動輸入。

2. 操作步驟：
   - Rancher：Workloads → Deployments → 點 Deployment → Edit Config → 改 Replicas 為 4 → Save
   - kubectl 確認：
     ```bash
...
3. 操作步驟：
   - Workloads → Deployments → 右上角 Create
   - Name：`gui-test`
   - Namespace：`default`
...
這個 section 目的是確認所有人的 Rancher 都跑起來了，叢集正確 import 進去。

**確認清單（帶著學員做）：**

1. 瀏覽器開 `https://<master-IP>`，能進 Rancher 介面
2. 看到 `my-k3s-cluster` 狀態是 `Active`（不是 Pending 或 Unavailable）
3. 點進叢集，看到 Node 列表正確（master + worker 都在）
4. 任意點一個 Deployment，能看到 Pod 列表
...
k3s 預設裝了 Traefik 作為 Ingress Controller，Traefik 會佔用 80 和 443。如果你直接跑 Rancher：
```bash
-p 80:80 -p 443:443
```

Docker 會回報 port 已被佔用。
...
**坑 3：Import 指令要在 k3s master 上執行**

Rancher 給你的那段指令：
```bash
kubectl apply -f https://<rancher-IP>/v3/import/xxxxxxxx.yaml
```

要在 k3s master 節點上執行，不是在你的本機打。原因：這個指令要連到 k3s 的 API server，你的本機可能沒有設定 k3s 的 kubeconfig。

執行前先確認你在正確的節點上：
```bash
kubectl get nodes
# 應該看到 k3s-master 和 worker 節點
```
