...
到目前為止 Ingress、ConfigMap、Secret 都學了。接下來把這三樣串起來，做一個完整的整合實作，有前端、後端、資料庫，接近真實部署。
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          ...
          <ul className="text-slate-300 text-sm space-y-1 list-disc list-inside">
            <li><code className="text-green-400">kubectl apply -f mysql-deploy.yaml -n my-app</code> → MySQL</li>
            <li><code className="text-green-400">kubectl apply -f frontend-deploy.yaml -n my-app</code> → 前端 Nginx</li>
            <li><code className="text-green-400">kubectl apply -f api-deploy.yaml -n my-app</code> → httpd API</li>
            <li><code className="text-green-400">kubectl apply -f app-ingress.yaml -n my-app</code> → 路由規則</li>
          </ul>
        </div>
        <div className="bg-slate-800/50 p-4 rounded-lg">
...
MySQL 的 Deployment 用 envFrom 引用 mysql-secret 和 mysql-config，把密碼和資料庫名稱都注入為環境變數。

kubectl apply -f frontend-deploy.yaml -n my-app

Nginx 的 Deployment 用 Volume 掛載 frontend-config 裡面的 index.html 到 /usr/share/nginx/html/ 目錄。

kubectl apply -f api-deploy.yaml -n my-app

httpd 的 Deployment，我們的 API 後端。
...
kubectl get ingress -n my-app。看到 app-ingress，等 ADDRESS 出現。
...
[▶ 下一頁 — 學員開始做，你去巡堂]`,
  ...
  {
    title: '解答：整合實作 + 上午因果鏈總結',
    subtitle: '必做解答 + 三個坑 + 上午回顧',
    section: '6-10：回頭操作 + 上午總結',
    duration: '5',
    content: (
      <div className="space-y-4">
        <div className="bg-slate-800/50 p-4 rounded-lg">
          <p className="text-cyan-400 font-semibold mb-2">必做題解答（整合實作九步驟）</p>
          <div className="font-mono text-xs text-slate-300 bg-slate-900 p-3 rounded space-y-0.5">
            <p>kubectl create namespace my-app</p>
            <p>kubectl create secret generic mysql-secret --from-literal=MYSQL_ROOT_PASSWORD=rootpassword123 -n my-app</p>
            <p>kubectl create configmap mysql-config --from-literal=MYSQL_DATABASE=myappdb -n my-app</p>
            <p>kubectl apply -f configmap-frontend.yaml -n my-app</p>
            ...
            <p className="text-slate-500"># /etc/hosts 加 myapp.local → curl 驗收</p>
          </div>
        </div>
...
kubectl apply -f mysql-deploy.yaml -n my-app。部署 MySQL。

kubectl apply -f frontend-deploy.yaml -n my-app。部署前端 Nginx。

kubectl apply -f api-deploy.yaml -n my-app。部署 httpd API。

kubectl apply -f app-ingress.yaml -n my-app。建 Ingress。
