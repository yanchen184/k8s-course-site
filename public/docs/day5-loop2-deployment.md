...
## 5-5 回頭操作 Loop 1 — Lab 1 學生情境練習（15 min）

### ① 課程內容

本節為學生獨立練習時間，對應 PPT 5-4（Lab 1：你被叫去救火）。學生照 PPT 做，不需要老師逐步帶。

**Lab 設計邏輯：**
- Bug 1：`selector.matchLabels: app: api` vs `template.labels: app: backend`（selector/label 不一致，READY 永遠 0/3）
- Bug 2：`replicas: 1`（老闆要求至少 3 個）
- 修好後還要 scale 到 5 確認 Pod 分散，把 scale 指令也練進去

**老師角色：**
- 宣布開始後去巡堂
- 結束前帶大家看答案，說明兩個 bug 的原理

> 📋 學生看的是 PPT 投影片（5-4 第二張「Lab 1：你被叫去救火」），上面有完整 YAML 和任務說明。
...
你是新入職的工程師。同事部署了一個 API 服務但設定有問題，你要接手修好。這份 YAML 裡有**兩個 bug**，自己找。
...
2. 觀察發生什麼事（用學過的指令自己查）
3. 找出兩個 bug，各自說明問題是什麼
4. 修好 YAML，重新 apply，讓 READY 變成 **3/3**
5. 用 `kubectl scale` 擴到 **5**，確認 Pod 分散在不同 Node

**驗收：**
- `kubectl get deploy` → READY: 3/3
- `kubectl scale` 到 5 後 `kubectl get pods -o wide` → NODE 欄位有分散
...
```bash
# 修改 api-service.yaml（兩個 bug 都修）後重新 apply
...
**老師補充說明重點：**
- K8s `apply` 時做 validation，selector 和 template labels 不一致**直接報錯**，讓學生看一下 Error 訊息並解讀
- Bug 1 是最常見的 YAML 坑，熟記三處 labels 的關係就能預防
- Bug 2 提醒學生：`replicas` 不是寫完就不管，要符合實際需求

> 📌 **Lab 2「版本事故」** 在 Loop 3（5-8）做，需要先學 rollout 系列指令。
