...
你在這門課前面可能用過 minikube，我們課程本身用的是 k3s，真實公司用的是 RKE2。RKE2 是 SUSE 出的企業版 K8s，跟 k3s 同一家公司，但多了安全合規認證，專門給銀行、政府這類有稽核需求的環境用。三個都是 K8s，但定位完全不一樣。
...
| 多節點 | 不行 | 可以 | 可以 |
| 安全合規 | 無 | 無 | FIPS 140-2、CIS Benchmark（註） |
| 預設 Ingress | 要手動 enable | Traefik | Nginx |
| 適合場景 | 學習、本機測試 | 小公司 / 課程 | 銀行 / 政府 / 稽核環境 |

minikube 不上生產，跑在筆電上練習用。k3s 和 RKE2 都是真實伺服器，差別在有沒有合規需求。

> **註**：FIPS 140-2 是美國聯邦政府的密碼模組安全標準，政府機關採購系統常要求通過這個認證。CIS Benchmark 是網路安全中心（CIS）發布的 K8s 安全設定基準，企業合規稽核常用來檢查叢集設定是否符合最佳安全實踐。
...
kubeconfig 是 kubectl 連線叢集用的設定檔，裡面存了 API server 的位址、使用者憑證、叢集資訊。不同的 K8s 發行版，kubeconfig 放的位置不一樣，切換叢集就是換這個檔案：
```bash
# minikube — 自動設定，不用管
minikube start
...
- `-n cattle-system`：指定 namespace，Rancher agent 裝在這個 namespace。cattle-system 這個名字來自 Rancher 的牧場命名哲學——Rancher 是牧場主，cattle 是牛群，所以 Rancher 的 agent 就跑在「牛群系統」裡
