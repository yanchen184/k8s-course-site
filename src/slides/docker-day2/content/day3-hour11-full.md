...
下一堂不再繼續講理論，而是直接進入實戰。整堂課只走一條主線：**Node.js API 的 Dockerfile 實作**，目標是讓大家真的把今天的觀念用出來，而不是再看一堆新範例。

會做三件事：

1. 先打包一個最小可跑的 Hello API，確認 Dockerfile 基礎已經能獨立完成
2. 再把它升級成 production-ready 版本，補上 `.dockerignore`、multi-stage、non-root、healthcheck
3. 最後做一次 Code Review，從錯誤 Dockerfile 反推問題在哪裡

今天先把三個檢查點記住：
- **先準備 lockfile**，後面才有辦法用 `npm ci` 做可重現的 build
- **快取邊界在 `package.json` / `package-lock.json`**，不是在整個專案目錄
- **生產級 Dockerfile 至少要考慮 non-root、healthcheck、entrypoint 與 image size**
...
2. 善用 Build Cache：**依賴先裝，程式碼後放**
3. 不要用 root：建立 non-root 使用者，用 `USER` 切換
...
下一堂課，我們會把今天這些觀念全部落地，從最小可跑版本一路改到 production-ready。重點不是再看新的語言範例，而是把同一個 API 的 Dockerfile 一步一步寫對。到時候見！
